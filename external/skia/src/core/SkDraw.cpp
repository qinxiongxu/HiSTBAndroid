...
#ifdef SKIA_ACCELERATION_ENABLE
    SkHSBlitter *blit = SkHSBlitter::getInstance();
#endif
...
void SkDraw::drawRect(const SkRect& rect, const SkPaint& paint) const {
    ...
    while (looper.next()) {
        ...
        switch (rtype) {
            case kFill_RectType:
#ifdef SKIA_ACCELERATION_ENABLE
                if(blit->checkProcessName()>0 && fBitmap->getPixels() == blit->getSurfaceVirtualAddress()&& paint.getShader() != NULL
					&& (matrix.getType() & (SkMatrix::kScale_Mask | SkMatrix::kTranslate_Mask)))
                {
                    SkIRect dump = clip.bwRgn().getBounds();
                    SkBitmapProcShader *bmShader = (SkBitmapProcShader*)paint.getShader();
                    SkBitmap srcBmp;

                    bmShader->asABitmap(&srcBmp, NULL, NULL);

                    int x = SkScalarRound(localDevRect.fLeft);
                    int y =  SkScalarRound(localDevRect.fTop);
                    int width =  SkScalarRound(localDevRect.width());
                    int height = SkScalarRound(localDevRect.height());

                    SkIRect ir = SkIRect::MakeXYWH(x, y, width, height);
                    SkIRect clipBounds = SkIRect::MakeLTRB(dump.fLeft, dump.fTop, dump.fRight, dump.fBottom);

                    blit->blitRect(fBitmap, blit->getSurfacePhysicalAddress(), &srcBmp, ir, clipBounds);
                    break;
                }
#endif
...
#ifdef SKIA_ACCELERATION_ENABLE
    if (blit->checkProcessName()>0 && fBitmap->getPixels() == blit->getSurfaceVirtualAddress()
		&& (matrix.getType() & (SkMatrix::kScale_Mask | SkMatrix::kTranslate_Mask)))
    {
        SkIRect clipBounds = fClip->getBounds();
        SkRect r = SkRect::MakeXYWH(0,0,bitmap.width(), bitmap.height());

        matrix.mapRect(&r);

        int x = SkScalarRound(r.fLeft);
        int y = SkScalarRound(r.fTop);
        int width = SkScalarRound(r.fRight)-SkScalarRound(r.fLeft);
        int height = SkScalarRound(r.fBottom)-SkScalarRound(r.fTop);

        SkIRect ir = SkIRect::MakeXYWH(x, y, width, height);
        blit->blitRect(fBitmap, blit->getSurfacePhysicalAddress(), &bitmap, ir, clipBounds);
        return;
    }
#endif
