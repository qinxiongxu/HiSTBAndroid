...
void ScreenRecoveryUI::draw_background_locked(Icon icon)
{
    ...
    if(buffer != NULL && (strcmp(buffer,"shcmcc")==0)) {
        gr_fill(0, -110, gr_fb_width(), gr_fb_height());
        ...
        if (icon) {
            ...
            int iconY = (gr_fb_height() - (iconHeight+40)) / 2;
...
{
    if (currentIcon == ERROR || currentIcon == ERROR_MOBILE || currentIcon == ERROR_CHECK) return;
    ...
    if (progressBarType != EMPTY) {
        int iconHeight = 0;
        if(buffer != NULL && (strcmp(buffer,"shcmcc")==0)) {
            int iconHeight = gr_get_height(backgroundIcon[INSTALLING_UPDATE_MOBILE]);
        } else {
        ...
        int dy = 0;
        if(buffer != NULL && (strcmp(buffer,"shcmcc")==0)) {
            dy = (3*gr_fb_height() + iconHeight - 2*height)/5;
        } else {
            dy = (3*gr_fb_height() + iconHeight - 2*height)/4;
        }
        ...
        if (progressBarType == INDETERMINATE) {
            if(buffer != NULL && (strcmp(buffer,"shcmcc")==0)) {
                ...
                static int j = 0;
                static int k = 1;
                int width_block = gr_get_width(progressBarIndeterminate[frame]);

                gr_blit(progressBarEmpty, 0, 0, width, height, dx, dy);
                if (!rtl_locale) {
                    if (length < (width - width_block) && (j == 0)){
                        length = 8 * i;
                        gr_blit(progressBarIndeterminate[frame], 0, 0, width_block, height, dx + length, dy);
                        i += 1;
                        k = 1;
                    } else {
                        j = 1;
                        length = width - width_block - 2 * k;
                        k += 1;
                        if (length > 0 ) {
                            gr_blit(progressBarIndeterminate[frame], 0, 0, width_block, height, dx + length, dy);
                        } else {
                ...
                if (rtl_locale) {
                    frame = (frame + indeterminate_frames - 2) % indeterminate_frames;
                } else {
                    frame = (frame + 1) % (indeterminate_frames-1);
                }
            }
        }
    }
...
void ScreenRecoveryUI::progress_loop() {
    ...
    for (;;) {
        ...
        if ((currentIcon == INSTALLING_UPDATE || currentIcon == ERASING || currentIcon == ERASING_MOBILE) &&
...
void ScreenRecoveryUI::Init()
{
    ...
    property_get("ro.product.target", buffer, "ott");
    if(buffer != NULL && (strcmp(buffer,"shcmcc")==0)) {
        LoadBitmap("icon_installing_mobile", &backgroundIcon[INSTALLING_UPDATE_MOBILE]);
        LoadBitmap("erasing_mobile", &backgroundIcon[ERASING_MOBILE]);
        LoadBitmap("icon_error_mobile", &backgroundIcon[ERROR_MOBILE]);
        LoadBitmap("progress_empty_mobile", &progressBarEmpty);
        LoadBitmap("progress_fill_mobile", &progressBarFill);
    } else {
        LoadBitmap("icon_installing", &backgroundIcon[INSTALLING_UPDATE]);
        backgroundIcon[ERASING] = backgroundIcon[INSTALLING_UPDATE];
        LoadBitmap("icon_error", &backgroundIcon[ERROR]);
        backgroundIcon[NO_COMMAND] = backgroundIcon[ERROR];
        LoadBitmap("icon_error_check",&backgroundIcon[ERROR_CHECK]);
        ...
        LoadLocalizedBitmap("error_text", &backgroundText[ERROR_CHECK]);
    }
    int i;
