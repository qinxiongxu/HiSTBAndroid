...
//    fs_type="ubifs"  partition_type="MTD"     location=partition
...
Value* MountFn(const char* name, State* state, int argc, Expr* argv[]) {
    ...
    if (strcmp(partition_type, "MTD") == 0) {
        ...
        if (strcmp(fs_type,"ubifs") ==0) {
            ...
            if (ret == -1) {
                fprintf(stderr,"no ubi num to %s\n",location);
                result = strdup("");
                goto done;
            }
            char device[20];
        ...
        } else if (mtd_mount_partition(mtd, mount_point, fs_type, 0 /* rw */) != 0) {
...
//    fs_type="ubifs"  partition_type="MTD"     location=partition fs_size=<bytes> mount_point=<location>
...
Value* FormatFn(const char* name, State* state, int argc, Expr* argv[]) {
    ...
    if (strcmp(fs_type,"raw") == 0 && strcmp(partition_type,"EMMC") == 0) {
        ...
        if (strstr(location,"/dev/block/platform/hi_mci.1/by-name") != NULL) {
            sprintf(partition_path,"%s",location);
        } else {
            sprintf(partition_path,"/dev/block/platform/hi_mci.1/by-name/%s",location);
        }

        dir = opendir("/dev/block/platform/hi_mci.1/by-name/");
        bool_bit = 0;
        if (dir) {
            while((dirent = readdir(dir))) {
                memset(dir_buf,0,sizeof(dir_buf));
                strcpy(dir_buf,"/dev/block/platform/hi_mci.1/by-name/");
                strcat(dir_buf,dirent->d_name);
                if (!strcmp(dir_buf,partition_path)){
                    bool_bit = 1 ;
                    break;
                }
        ...
        if ( bool_bit == 0) {
            printf("There is no %s: %s", partition_path,strerror(errno));
            result = strdup("");
            goto done;
    ...
    if (strcmp(partition_type, "MTD") == 0) {
        ...
        if((strcmp(fs_type, "ubifs") == 0)){
            int status = ubi_deatch_volume(mtd->device_index);
            if (status != 0) {
                fprintf(stderr, "%s: ubi deatch failed (%d) on %s, mtd->device_index=%d \n",
                        name, status, mount_point, mtd->device_index);
                if(errno == EBUSY) {
                    result = strdup("");
                    goto done;
                }
        ...
        if(strcmp(fs_type, "ubifs") == 0){
            ...
            int status = ubi_attach_volume(mtd->device_index);

            if (status != 0) {
                fprintf(stderr, "%s: ubi attach failed (%d) on %s, mtd->device_index=%d\n",
                        name, status, mount_point, mtd->device_index);
                result = strdup("");
                goto done;
            }

            status = ubi_make_volume(mtd->device_index);
            if (status != 0) {
                fprintf(stderr, "%s: ubi make volume failed (%d) on %s\n",
                        name, status, mount_point);
                result = strdup("");
                goto done;
            }
        }
...
Value* PackageExtractFileFn(const char* name, State* state,
    ...
    if (argc == 2) {
        ...
        if (strstr(zip_path,"fastboot")) {
            printf(" SECURITY_L2 fastboot begin\n");
        ...
        fastCaDone:
            ...
            printf(" SECURITY_L2 fastboot end\n");
            return v;
        }
    #endif
...
Value* GetPropFn(const char* name, State* state, int argc, Expr* argv[]) {
    ...
    if (strcmp(key, "persist.sys.packagetype.qfp") == 0) {
        HI_CHIP_PACKAGE_TYPE_E penPackageType = HI_CHIP_PACKAGE_TYPE_BUTT;
        free(key);
        HI_SYS_Init();
        HI_SYS_GetChipPackageType(&penPackageType);
        HI_SYS_DeInit();
        if (penPackageType == HI_CHIP_PACKAGE_TYPE_QFP_216) {
            printf("Chip package type: QFP \n");
            return StringValue(strdup("true"));
        } else {
            printf("Chip package type: not QFP \n");
...
Value* WriteUbiImageFn(const char* name, State* state, int argc, Expr* argv[]) {
    char* result = NULL;
    ...
    success = true;
    int buffer_size = mtd_info.erasesize;
    char* buffer = malloc(buffer_size);
    if(buffer == NULL)
    {
        printf("buffer malloc failed++!buffer_size=%d\n",buffer_size);
        goto done;
    }
    int read;
    while (success && (read = fread(buffer, 1, buffer_size, f)) > 0) {
        int wrote = mtd_write_ubi_data(ctx, buffer, read);
        success = success && (wrote == read);
        if (!success) {
...
Value* PartChangeFn(const char* name, State* state, int argc, Expr* argv[]) {
    char* result = strdup("");
    if (argc != 2) {
        return ErrorAbort(state, "%s() expects 1 arg, got %d", name, argc);
    }
    char* partition;
    char* partition_type;
    if (ReadArgs(state, argv, 2,&partition_type, &partition) < 0) {
        return NULL;
    }
    if (strlen(partition) == 0 || strlen(partition_type) == 0) {
        ErrorAbort(state, "mount_point argument to unmount() can't be empty");
        result = NULL;
        goto done;
    }
    ...
    int fd;
    char *PARTCHANGENAME="/proc/partchange";
    fd = open(PARTCHANGENAME,O_WRONLY);
    if (fd < 0) {
        fprintf(stderr,"open %s fail %s \n",PARTCHANGENAME,strerror(errno));
        result = NULL;
        goto done;
    }

    fprintf(stderr,"the new partition =%s \n",partition);

    if (write(fd,partition,strlen(partition)) != strlen(partition)) {
        fprintf(stderr,"error write %s to %s , %s\n",partition,PARTCHANGENAME,strerror(errno));
        result = NULL;
    }
    close(fd);
...
Value* SetMiscFn(const char* name, State* state, int argc, Expr* argv[]) {

    char *result = strdup("");
    const int MISC_PAGES = 3;
    const int MISC_COMMAND_PAGE = 1;
    ...
    char* set_bit = "set";
    if (argc == 2) {
        if (ReadArgs(state, argv, 2,&partition_type, &device) < 0) {
            return NULL;
        }
    }

    if (argc == 3) {
        if (ReadArgs(state, argv, 2,&partition_type, &device,&set_bit) < 0) {
            return NULL;
        }
    }
    ...
    if (strcmp(set_bit,"clean") == 0) {
        memset(&boot, 0, sizeof(boot));
    }

    if (strlen(device) == 0 || strlen(partition_type) == 0) {
        ErrorAbort(state, "device and partition_type can't be empty");
        result = NULL;
        goto done;
    }
    ...
    if( strcmp(partition_type,"MTD") == 0) {
        ...
        size_t write_size;
        part = mtd_find_partition_by_name(device);
        if (part == NULL || mtd_partition_info(part, NULL, NULL, &write_size)) {
            fprintf(stderr, "no mtd partition named \"%s\"", device);
            result = NULL;
            goto done;
        }
    ...
    else if (strcmp(partition_type,"EMMC") == 0) {
...
Value* WriteExt4SparseImageFn(const char* name, State* state, int argc, Expr* argv[]) {
    ...
    if (!write_ext4sp(filename,partition)) {
        result = strdup("");
    }
...
int flashled_pid = -1;
Value* OpenLed(const char* name, State* state, int argc, Expr* argv[]) {
   pid_t pid = fork();
    if(pid == -1) {
        return ErrorAbort(state, " the fork return is %d \n", pid);
    } else if (pid == 0) {
        flashled();
        return 0;
    } else {
...
Value* CloseLed(const char* name, State* state, int argc, Expr* argv[]) {
    if ( 0 <= flashled_pid) {
        kill(flashled_pid,SIGKILL);
    }
    return StringValue(strdup("ok"));
}

void RegisterInstallFunctions() {
    RegisterFunction("open_led", OpenLed);
    RegisterFunction("close_led", CloseLed);
    ...
    RegisterFunction("write_ubi_image", WriteUbiImageFn);
    RegisterFunction("write_ext4sp_img", WriteExt4SparseImageFn);
    ...
    RegisterFunction("partchange", PartChangeFn);
    RegisterFunction("setmisc", SetMiscFn);
}
