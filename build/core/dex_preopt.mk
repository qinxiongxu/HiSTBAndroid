...
define _dexpreopt-boot-fastpreloadjar
$(eval _dbj_jar := $(DEXPREOPT_BOOT_JAR_DIR_FULL_PATH)/$(1).jar)
$(eval _dbj_odex := $(DEXPREOPT_BOOT_JAR_DIR_FULL_PATH)/$(1).odex)
$(eval _dbj_jar_no_dex := $(DEXPREOPT_BOOT_JAR_DIR_FULL_PATH)/$(1)_nodex.jar)
$(eval _dbj_src_jar := frameworks/base/cmds/preload/$(1).jar)
$(eval _dbj_framework_odex := $(PRODUCT_OUT)/system/framework/$(1).odex)
$(eval $(_dbj_odex): PRIVATE_DBJ_JAR := $(_dbj_jar))
$(_dbj_odex) : $(_dbj_src_jar) | $(ACP) $(DEXPREOPT) $(DEXOPT)
	@echo "Dexpreopt Boot Jar: $$@"
	$(hide) rm -f $$@
	...
	$(hide) $(ACP) -fp $$< $$(PRIVATE_DBJ_JAR)
	$$(call dexpreopt-one-file,$$(PRIVATE_DBJ_JAR),$$@)
	$(hide) $(ACP) -fp $$@ $(_dbj_framework_odex)
...
$(eval _dbj_jar_no_dex :=)
$(eval _dbj_src_jar :=)
$(eval _dbj_framework_odex :=)
endef

$(foreach b,$(DEXPREOPT_BOOT_JARS_MODULES),$(if $(filter preloadclass ZygoteOptimize,$(b)), \
    $(eval $(call _dexpreopt-boot-fastpreloadjar,$(b))),$(eval $(call _dexpreopt-boot-jar,$(b)))))
