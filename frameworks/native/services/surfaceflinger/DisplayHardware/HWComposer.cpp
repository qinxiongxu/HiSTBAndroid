...
status_t HWComposer::prepare() {
    ...
    if (err == NO_ERROR) {
        ...
        for (size_t i=0 ; i<mNumDisplays ; i++) {
            ...
            if(disp.hasOvComp==true && disp.hasPreFbComp==true && disp.hasFbComp==false){
                //comopose ui to fb && postFb
                disp.hasPreFbComp = disp.hasFbComp;
                disp.hasFbComp = true;
            } else {
                disp.hasPreFbComp = disp.hasFbComp;
            }
...
public:
    ...
    virtual int isSkip() {
          return  getLayer()->flags & HWC_SKIP_LAYER;
    }

    //hisi modified. DIM can not be handled by HWC(TDE)
    ...
    //hisi modified. will be hanlde by HWC(TDE)
    virtual void setVideo(bool video) {
        if (video) {
            getLayer()->flags |= HWC_VIDEO_LAYER;
        } else {
            getLayer()->flags &= ~HWC_VIDEO_LAYER;
        }
    }
 ...
 */
HWComposer::LayerListIterator HWComposer::getLayerIterator(int32_t id, size_t index) const{
    if (uint32_t(id)>31 || !mAllocatedDisplayIDs.hasBit(id)) {
 ...
 */
HWComposer::LayerListIterator HWComposer::begin(int32_t id) const{
 ...
 */
HWComposer::LayerListIterator HWComposer::end(int32_t id) const{
    size_t numLayers = 0;
    if (uint32_t(id) <= 31 && mAllocatedDisplayIDs.hasBit(id)) {
        const DisplayData& disp(mDisplayData[id]);
        if (mHwc && disp.list) {
...
bool HWComposer::supportByHardwareCompose(const sp<const DisplayDevice>& hw) const
{
    const int32_t id = hw->getHwcDisplayId();
    HWComposer::LayerListIterator cur = this->begin(id);
    const HWComposer::LayerListIterator end = this->end(id);
    ...
    if (cur != end) {
        for (size_t i=0 ; i<count && cur!=end ; ++i, ++cur) {
            ...
            if(0 == strcmp("LayerDim",layer->getTypeId()))
            {
                return false;
            }
        }
