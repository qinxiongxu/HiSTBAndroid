...
status_t SurfaceFlinger::getDisplayInfo(const sp<IBinder>& display, DisplayInfo* info) {
    ...
    class Density {
	    ...
	    static int getDensityFromPrivateProperty(char const* propName) {
            char property[PROPERTY_VALUE_MAX];
            int density = 0;
            if (property_get(propName, property, NULL) > 0) {
                density = atoi(property);
    ...
    public:
        static int getEmuDensity() {
            return getDensityFromPrivateProperty("persist.sys.screen_density"); }
            ...
            if (hw->canDraw()) {
                ...
                for (size_t i=0 ; i<count ; i++) {
                    ...
                    if (s.layerStack == hw->getLayerStack()) {
                        ...
                        if (!drawRegion.isEmpty() || (s.flags & layer_state_t::eLayerClearFB)) {
        ...
        for (size_t dpy=0 ; dpy<mDisplays.size() ; dpy++) {
            ...
            if (id >= 0) {
                ...
                for (size_t i=0 ; cur!=end && i<count ; ++i, ++cur) {
                    const sp<Layer>& layer(currentLayers[i]);
                    const Layer::State& s(layer->getDrawingState());
                    if(layer->isUpdate() && !(s.flags & layer_state_t::eLayerClearFB)) {
                        mLayerBufferChange = true;
                    }
...
bool forbidUpdate()
{
    ...
    if(locked)//check the lock
    {
        property_get("persist.sys.swap.enable", buffer, "1");
        bootexit = atoi(buffer);
    }

    if(locked)
    {
        if(bootexit > 0)//relase the lock
        {
            locked = false;
            return false;
        }
...
bool videoExist()
{
    return (access(USER_BOOT_VIDEO, R_OK) == 0);
}

bool SurfaceFlinger::needsUpdate()
{
    //1.getLayerChangeState
    bool ret = true;
    if(mLayerStateChange||mLayerBufferChange) {
        ret = true;
    } else  {
        ret = false;
    }
...
/*just can be called once time in every fresh period*/
void  SurfaceFlinger::resetUpdate()
{
    mLayerStateChange = false;
    mLayerBufferChange = false;
}
        ...
        const Region& inDirtyRegion)
{
    ...
    const int32_t id = hw->getHwcDisplayId();
    HWComposer& hwc(getHwComposer());
    bool haveHwc = hwc.hasHwcComposition(id);
    if(haveHwc && !updateFlag) {
        //Don't update UI When just have video play
    } else {
    ...
    hw->swapRegion.orSelf(dirtyRegion);
    bool hardwareCompose  = hwc.supportByHardwareCompose(hw);
    int layers = mDrawingState.layersSortedByZ.size();
    Rect r;
    if (1 == layers)
    {
        sp<Layer> layer = mDrawingState.layersSortedByZ[0];
        r = layer->getContentCrop();
    }
    if((layers <=0) || (1==layers && !r.isValid()) || (haveHwc && !updateFlag && !hardwareCompose)) {
        //Don't update UI When video play  && UI have no Update && gpuCompose
        ALOGE("do not swapBuffers layers=%d", layers);
    } else {
        if (CC_LIKELY(!mDaltonize)) {
...
void SurfaceFlinger::doComposeSurfaces(const sp<const DisplayDevice>& hw, const Region& dirty)
{
    ...
    const HWComposer::LayerListIterator end = hwc.end(id);
    bool hardwareCompose  = hwc.supportByHardwareCompose(hw);
    static bool preComposetype = false;
    ...
    if (hasGlesComposition) {
        ...
        if (hasHwcComposition || hardwareCompose) {
            // when using overlays, we assume a fully transparent framebuffer
            // NOTE: we could reduce how much we need to clear, for instance
            // remove where there are opaque FB layers. however, on some
            // GPUs doing a "clean slate" clear might be more efficient.
            // We'll revisit later if needed.
            if(preComposetype != hardwareCompose) {
    ...
    if (cur != end) {
        ...
        for (size_t i=0 ; i<count && cur!=end ; ++i, ++cur) {
            ...
            const Layer::State& s(layer->getCurrentState());

            const sp<Layer>& splayer = layer->getLayer();
            PixelFormat format = PIXEL_FORMAT_RGB_565;
            if(splayer != NULL) format = splayer->getLayerFormat();

            const sp<GraphicBuffer>& pactiveBuffer(splayer->getActiveBuffer());

            if(s.flags&layer_state_t::eLayerClearFB
                && (clip.isEmpty()
                    || format == PIXEL_FORMAT_RGBA_8888
                    || (pactiveBuffer != NULL
                        && 1 == pactiveBuffer->getWidth()
                        && 1 == pactiveBuffer->getHeight()))
                && (s.alpha == 0xFF) )
            {
                ...
                Region mediaClip;
                Rect visiblerect(0, 0, s.requested.w, s.requested.h);
                visiblerect = s.transform.transform(visiblerect);
                ...
                if (cmpRet == 0)
                {
                    mediaClip.set(visiblerect);
                }
                else
                {
                    Rect visiblerect2(fbWidth - visiblerect.bottom, visiblerect.left, fbWidth - visiblerect.top, visiblerect.right);
                    mediaClip.set(visiblerect2);
                }

                if(!hardwareCompose)
                {
                    RenderEngine& engine(getRenderEngine());
                    engine.fillRegionWithColor(mediaClip, fbHeight, 0, 0, 0, 0);
                }
            }
                ...
                switch (cur->getCompositionType()) {
                    ...
                    case HWC_FRAMEBUFFER: {
                        if(hardwareCompose)
                            ...
                            layer->draw(hw, clip); //gpu

                        //add for turn off gpu when ui conten have no update
                        layer->setLastBuffer();
                        break;
                    }
        ...
        for (size_t i=0 ; i<count ; ++i) {
            ...
            if(s.flags&layer_state_t::eLayerClearFB && (clip.isEmpty() || format == PIXEL_FORMAT_RGBA_8888) && (s.alpha == 0xFF) )
            {
                Region mediaClip;
                Rect visiblerect(0, 0, s.requested.w, s.requested.h);
                visiblerect = s.transform.transform(visiblerect);
    ...
    // record compose type
    preComposetype = hardwareCompose;
...
void SurfaceFlinger::onInitializeDisplays() {
    ...
    if (!hasFastplay() && videoExist())
    {
        ALOGE("No use fastplay , only boot.ts! ");
    }
    else
    {
...
void SurfaceFlinger::dumpLayerInfoOnVideo(String8& result)
{
    bool colorize = false;
    /*
