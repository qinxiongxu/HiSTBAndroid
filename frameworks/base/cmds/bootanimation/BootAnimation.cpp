...
#define VIDEO_PROP_NAME "persist.sys.bootvideo.exit"
...
void stopMediaPlayer(sp<MediaPlayer> mp)
{
    ...
    if (!use_win_proc){
        if (mp == NULL) {
            ...
            mp->reset();
            mp->setDataSource(USER_BOOT_VIDEO, NULL);
            mp->prepare();
            setStopMode(mp, 1);
            mp->start();
           //usleep(10*1000);
...
bool BootAnimation::video()
{
    ...
    // after this handle, we can update graphic
    // only use when we don't use fastplay and select boot.ts
    if (!hasFastplay() && videoExist())
    {
        if (!isUnblanked) {
            SurfaceComposerClient::unblankDisplay(dtoken);
            isUnblanked = true;
        }
    }
