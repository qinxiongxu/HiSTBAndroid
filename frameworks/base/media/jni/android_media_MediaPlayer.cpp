...
android_media_MediaPlayer_seekTo(JNIEnv *env, jobject thiz, int msec)
{
    ...
    {
        int MESSAGE_SEEK_START = 3;
        jclass cls = env->GetObjectClass(thiz);
        jmethodID sendExtendMessage = env->GetMethodID(cls,"sendExtendMessage","(III)V");
        env->CallVoidMethod(thiz,sendExtendMessage, (jint)MESSAGE_SEEK_START,(jint)msec,(jint)0);
    }
...
android_media_MediaPlayer_setParameter(JNIEnv *env, jobject thiz, jint key, jobject java_request)
{
    ...
    sp<MediaPlayer> mp = getMediaPlayer(env, thiz);
    if (mp == NULL ) {
        ALOGV("setParameter: mp is null, not send exception");
        //jniThrowException(env, "java/lang/IllegalStateException", NULL);
        return false;
    }
