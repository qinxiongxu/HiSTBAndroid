...
JMediaCodec::JMediaCodec(
    ...
    if (property_get("service.media.codec.debug", value, NULL)
...
status_t JMediaCodec::configure(
        ...
        int flags) {
    if (mDebug) {
        ALOGI("configure(format:%#x, surface:%#x, crypto:%#x, flags:%d)",
            format.get(), bufferProducer.get(), crypto.get(), flags);
        if (format != NULL) {
            AString mime;
            format->findString("mime", &mime);
            mIsVideo = !strncasecmp(mime.c_str(), "video/", 6);
            ALOGI("video:%s, format:%s", mIsVideo?"true":"false", format->debugString().c_str());
        }
    }
    sp<Surface> client;
...
status_t JMediaCodec::createInputSurface(
    ...
    if (mDebug) {
        ALOGI("(%s)createInputSurface()", mIsVideo?"vid":"aud");
    }
...
status_t JMediaCodec::start() {
    if (mDebug) {
        ALOGI("(%s)start()", mIsVideo?"vid":"aud");
    }
    return mCodec->start();
}
...
status_t JMediaCodec::flush() {
    if (mDebug) {
        ALOGI("(%s)flush()", mIsVideo?"vid":"aud");
    }
    return mCodec->flush();
}
...
status_t JMediaCodec::queueInputBuffer(
    ...
    if (mDebug) {
        ALOGI("(%s)QIB(idx:%ld, offset:%ld, size:%ld, pts:%lld, flags:%d)",
            mIsVideo?"vid":"aud", index, offset, size, timeUs, flags);
    }
...
status_t JMediaCodec::queueSecureInputBuffer(
        ...
        AString *errorDetailMsg) {
    if (mDebug) {
        ALOGI("(%s)QSIB(idx:%ld, offset:%ld, pts:%lld, flags:%d)",
            mIsVideo?"vid":"aud", index, offset, presentationTimeUs, flags);
    }
...
status_t JMediaCodec::dequeueInputBuffer(size_t *index, int64_t timeoutUs) {
    status_t status = mCodec->dequeueInputBuffer(index, timeoutUs);
    if (mDebug) {
        ALOGI("(%s)DIB(timeout:%lld), idx:%d", mIsVideo?"vid":"aud", timeoutUs, (status == OK)?(*index):-1);
    }
    return status;
}
...
status_t JMediaCodec::dequeueOutputBuffer(
    ...
    if (mDebug) {
        ALOGI("(%s)DOB(idx:%ld, offset:%ld, size:%ld, pts:%lld, flags:%d, timeout:%lld), err:%d",
            mIsVideo?"vid":"aud", *index, offset, size, timeUs, flags, timeoutUs, err);
    }
    if (err != OK) {
        return err;
    }
...
status_t JMediaCodec::releaseOutputBuffer(size_t index, bool render) {
    if (mDebug) {
        ALOGI("(%s)ROB(idx:%ld, render:%s)",
            mIsVideo?"vid":"aud", index, render?"true":"false");
    }
    return render
...
status_t JMediaCodec::signalEndOfInputStream() {
    if (mDebug) {
        ALOGI("(%s)signal input EOS", mIsVideo?"vid":"aud");
    }
...
status_t JMediaCodec::getOutputFormat(JNIEnv *env, jobject *format) const {
    sp<AMessage> msg;
    status_t err;
	err = mCodec->getOutputFormat(&msg);
    if (mDebug) {
        ALOGI("(%s)getOutputFormat(idx:%ld, format:%s)", mIsVideo?"vid":"aud",
            index, (err == OK)?msg->debugString().c_str():"NULL");
    }
   if (err != OK) {
        return err;
    }
...
status_t JMediaCodec::getBuffers(
    ...
    if (mDebug) {
        ALOGI("(%s)getBuffers(%s), count:%ld, err:%d", mIsVideo?"vid":"aud",
            input?"input":"output", (err == OK)?buffers.size():0, err);
    }
    if (err != OK) {
        return err;
    }
...
status_t JMediaCodec::getName(JNIEnv *env, jstring *nameStr) const {
    ...
    if (mDebug) {
        ALOGI("(%s)getName(%s), err:%d", mIsVideo?"vid":"aud",
            name.c_str(), err);
    }
...
status_t JMediaCodec::setParameters(const sp<AMessage> &msg) {
    if (mDebug) {
        ALOGI("(%s)setParameters(%s)", mIsVideo?"vid":"aud",
...
void JMediaCodec::setVideoScalingMode(int mode) {
    if (mDebug) {
        ALOGI("(%s)setVideoScalingMode(mode:%d)", mIsVideo?"vid":"aud", mode);
    }
