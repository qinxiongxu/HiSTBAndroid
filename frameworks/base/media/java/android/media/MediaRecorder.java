...
import com.hisilicon.karaoke.HiKaraokeMediaRecorder;
 ...
 * </div>
 */
public class MediaRecorder implements MediaRecorderEx {
    static {
        System.loadLibrary("media_jni");
        native_init();
    }
    private final static String TAG = "MediaRecorder";
    private static int RECORD_STATE_START = 0;
    private static int RECORD_STATE_PAUSE = 1;
    private static int RECORD_STATE_RESUME = 2;
    private static int RECORD_STATE_STOP = 3;
    private static int RECORD_STATE_RELEASE = 4;
    ...
    //for karaoke
    private IMicphoneService mMicphoneService = null;
    HiKaraokeMediaRecorder mHiKaraokeMediaRecorder;
    boolean mIsKaraokeMode = false;
    /**
     * Default constructor.
     */
    public MediaRecorder() {
        ...
        if(karEnable != null && karEnable.equals("true")){
            try{
                if(getIMicphoneService().isInitialized() == 1)
                {
                     mIsKaraokeMode = true;
                     mHiKaraokeMediaRecorder = new HiKaraokeMediaRecorder();
                     mHiKaraokeMediaRecorder.setup(packageName);
                }
				else
				{
    ...
    public void setPreviewDisplay(Surface sv) {
     ...
     private IMicphoneService getIMicphoneService() {
        if (mMicphoneService == null) {
            IBinder b = ServiceManager.getService("Micphone");
            mMicphoneService = IMicphoneService.Stub.asInterface(b);
        }
        return mMicphoneService;
    }
    /**
    ...
    public void setAudioSource(int audio_source)
        ...
        if(mIsKaraokeMode)
        {
            mHiKaraokeMediaRecorder.setAudioSource(audio_source);
        }
        else
        {
            ...
            throws IllegalStateException
    {
        if(mIsKaraokeMode)
        {
            mHiKaraokeMediaRecorder.setOutputFormat(output_format);
        }
        else
        {
            ...
            throws IllegalStateException
    {
        if(mIsKaraokeMode)
        {
            mHiKaraokeMediaRecorder.setAudioEncoder(audio_encoder);
        }
        else
        {
            _setAudioEncoder(audio_encoder);
        }
    }
    ...
    public void setAudioSamplingRate(int samplingRate) {
        ...
        if(mIsKaraokeMode)
        {
            mHiKaraokeMediaRecorder.setParameter("audio-param-sampling-rate=" + samplingRate);
        }
        else
        {
    ...
    public void setAudioChannels(int numChannels) {
        ...
        if(mIsKaraokeMode)
        {
            mHiKaraokeMediaRecorder.setParameter("audio-param-number-of-channels=" + numChannels);
        }
        else
        {
    ...
    public void setAudioEncodingBitRate(int bitRate) {
        ...
        if(mIsKaraokeMode)
        {
            mHiKaraokeMediaRecorder.setParameter("audio-param-encoding-bitrate=" + bitRate);
        }
        else
        {
    ...
    public void prepare() throws IllegalStateException, IOException
    {
        if (mPath != null) {
            ...
            try {
                if(mIsKaraokeMode)
                {
                    mHiKaraokeMediaRecorder.setOutputFile(fos.getFD(), 0, 0);
                }
                else
                {
        ...
        if(mIsKaraokeMode)
        {
            mHiKaraokeMediaRecorder.prepare();
        }
        else
        {
            _prepare();
    ...
    public void pause() {
        try {
            getIMicphoneService()
                .setAudioMixedDataRecordState(RECORD_STATE_PAUSE);
        } catch (RemoteException e) {
            e.printStackTrace();
        }
    }
    ...
    public void resume() {
        try {
            getIMicphoneService()
                .setAudioMixedDataRecordState(RECORD_STATE_RESUME);
        } catch (RemoteException e) {
            e.printStackTrace();
        }
    }
    ...
    public void release()
    {
        if(mIsKaraokeMode)
        {
            mHiKaraokeMediaRecorder.release();
        }
		else
		{
			_release();
    ...
    private native void _setAudioEncoder(int audio_encoder)
            throws IllegalStateException;
