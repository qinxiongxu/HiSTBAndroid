...
import com.hisilicon.karaoke.HiKaraokeAudioRecord;
...
public class AudioRecord
{
     ...
     * for karaoke
     */
    private boolean mIsKaraokeSource = false;
    private HiKaraokeAudioRecord mHiKaraokeAudioRecord;
    //---------------------------------------------------------
    ...
    throws IllegalArgumentException {
        ...
        if(mIsKaraokeSource){
			mHiKaraokeAudioRecord = new HiKaraokeAudioRecord();
            initResult = mHiKaraokeAudioRecord.setup(
                new WeakReference<AudioRecord>(this), mRecordSource, mSampleRate, mChannelMask, mAudioFormat, mNativeBufferSizeInBytes,
                session);
        }else{
        initResult = native_setup( new WeakReference<AudioRecord>(this),
    ...
    public int getNotificationMarkerPosition() {
        if(mIsKaraokeSource){
            return mHiKaraokeAudioRecord.get_marker_pos();
        }else{
    ...
    public int getPositionNotificationPeriod() {
        if(mIsKaraokeSource){
            return mHiKaraokeAudioRecord.get_pos_update_period();
        }else{
    ...
    throws IllegalStateException {
        ...
        synchronized(mRecordingStateLock) {
            if(mIsKaraokeSource){
                if (mHiKaraokeAudioRecord.start(MediaSyncEvent.SYNC_EVENT_NONE, 0) == SUCCESS) {
    ...
    public int read(byte[] audioData, int offsetInBytes, int sizeInBytes) {
        ...
        if(mIsKaraokeSource){
            return mHiKaraokeAudioRecord.read_in_byte_array(audioData, offsetInBytes, sizeInBytes);
        }else{
        return native_read_in_byte_array(audioData, offsetInBytes, sizeInBytes);
        }
    }
    ...
    public int read(short[] audioData, int offsetInShorts, int sizeInShorts) {
        ...
        if(mIsKaraokeSource){
            return mHiKaraokeAudioRecord.read_in_short_array(audioData, offsetInShorts, sizeInShorts);
        }else{
    ...
    public int read(ByteBuffer audioBuffer, int sizeInBytes) {
        ...
        if(mIsKaraokeSource){
            return mHiKaraokeAudioRecord.read_in_direct_buffer(audioBuffer, sizeInBytes);
        }else{
    ...
    public int setNotificationMarkerPosition(int markerInFrames) {
        ...
        if(mIsKaraokeSource){
            return mHiKaraokeAudioRecord.set_marker_pos(markerInFrames);
        }else{
    ...
    public int setPositionNotificationPeriod(int periodInFrames) {
        ...
        if(mIsKaraokeSource){
            return mHiKaraokeAudioRecord.set_pos_update_period(periodInFrames);
        }else{
