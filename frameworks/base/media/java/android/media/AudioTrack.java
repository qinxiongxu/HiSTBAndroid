...
public class AudioTrack
{
    ...
    private final static String TAG = "AudioTrackJava";
    ...
    throws IllegalArgumentException {
        ...
        if ("true".equals(SystemProperties.get("service.audio.track.debug"))) {
            mDebug = true;
            Log.i(TAG, "enable debug");
        }
        if (mDebug) {
            Log.i(TAG, "AudioTrack(streamType:" + streamType + ", sampleRate:" + sampleRateInHz + ", channelConfig:" + channelConfig
				+ ", audioFormat:" + audioFormat + ", bufferSize:" + bufferSizeInBytes + ", mode:" + mode + ", sessionId:" + sessionId + ")");
        }
    ...
    public void release() {
        if (mDebug) {
            Log.i(TAG, "release, mState:" + mState);
        }
    ...
    public int getSampleRate() {
        if (mDebug) {
            Log.i(TAG, "getSampleRate, return:" + mSampleRate);
        }
        return mSampleRate;
    }
    ...
    public int getPlaybackRate() {
        int rate = native_get_playback_rate();
        if (mDebug) {
            Log.i(TAG, "getPlaybackRate, return:" + rate);
        }
        return rate;
    }
    ...
    public int getAudioFormat() {
        if (mDebug) {
            Log.i(TAG, "getAudioFormat, return:" + mAudioFormat);
        }
        return mAudioFormat;
    }
    ...
    public int getStreamType() {
        if (mDebug) {
            Log.i(TAG, "getStreamType, return:" + mAudioFormat);
        }
        return mStreamType;
    }
    ...
    public int getChannelConfiguration() {
        if (mDebug) {
            Log.i(TAG, "getChannelConfiguration, return:" + mChannelConfiguration);
        }
    ...
    public int getChannelCount() {
        if (mDebug) {
            Log.i(TAG, "getChannelCount, return:" + mChannelCount);
        }
    ...
    public int getState() {
        if (mDebug) {
            Log.i(TAG, "getState, return:" + mState);
        }
        return mState;
    }
    ...
    public int getPlayState() {
        synchronized (mPlayStateLock) {
            if (mDebug) {
                Log.i(TAG, "getPlayState, return:" + mPlayState);
            }
            return mPlayState;
        }
    }
    ...
    protected int getNativeFrameCount() {
        int count = native_get_native_frame_count();
        if (mDebug) {
            Log.i(TAG, "getNativeFrameCount, return:" + count);
        }
        return count;
    }
    ...
    public int getNotificationMarkerPosition() {
        int pos = native_get_marker_pos();
        if (mDebug) {
            Log.i(TAG, "getNotificationMarkerPosition, return:" + pos);
        }
        return pos;
    }
    ...
    public int getPositionNotificationPeriod() {
        int period = native_get_pos_update_period();
        if (mDebug) {
            Log.i(TAG, "getPositionNotificationPeriod, return:" + period);
        }
        return period;
    }
    ...
    public int getPlaybackHeadPosition() {
        int position = native_get_position();
        if (mDebug) {
            Log.i(TAG, "getPlaybackHeadPosition, return:" + position);
        }
        return position;
    }
    ...
    public int getLatency() {
        int latency = native_get_latency();
        if (mDebug) {
            Log.i(TAG, "getLatency, return:" + latency);
        }
        return latency;
    }
    ...
    public int getAudioSessionId() {
        if (mDebug) {
            Log.i(TAG, "getAudioSessionId, return:" + mSessionId);
        }
        return mSessionId;
    }
    ...
    */
    public boolean getTimestamp(AudioTimestamp timestamp)
    {
        ...
        if (mDebug) {
            Log.i(TAG, "getTimestamp, framePosition:" + longArray[0] + ", nanoTime:" + longArray[1] + ", err:" + ret);
        }
        if (ret != SUCCESS) {
            return false;
        }
    ...
    public void setPlaybackPositionUpdateListener(OnPlaybackPositionUpdateListener listener,
                                                    Handler handler) {
        if (mDebug) {
            Log.i(TAG, "setPlaybackPositionUpdateListener(listener:" + listener + ", handler:" + handler + ")");
        }
        if (listener != null) {
    ...
    public int setStereoVolume(float leftVolume, float rightVolume) {
        if (mDebug) {
            Log.i(TAG, "setStereoVolume(leftVolume:" + leftVolume + ", rightVolume:" + rightVolume + "), mState:" + mState);
        }
    ...
    public int setPlaybackRate(int sampleRateInHz) {
        if (mDebug) {
            Log.i(TAG, "setPlaybackRate(sampleRate:" + sampleRateInHz + "), mState:" + mState);
        }
        ...
        int ret = native_set_playback_rate(sampleRateInHz);
        if (mDebug) {
            Log.i(TAG, "setPlaybackRate, return:" + ret);
        }
        return ret;
    }
    ...
    public int setNotificationMarkerPosition(int markerInFrames) {
        if (mDebug) {
            Log.i(TAG, "setNotificationMarkerPosition(marker:" + markerInFrames + "), mState:" + mState);
        }
        if (mState == STATE_UNINITIALIZED) {
            return ERROR_INVALID_OPERATION;
        }
        int ret = native_set_marker_pos(markerInFrames);
        if (mDebug) {
            Log.i(TAG, "setNotificationMarkerPosition, return:" + ret);
        }
        return ret;
    }
    ...
    public int setPositionNotificationPeriod(int periodInFrames) {
        if (mDebug) {
            Log.i(TAG, "setPositionNotificationPeriod(period:" + periodInFrames + "), mState:" + mState);
        }
        if (mState == STATE_UNINITIALIZED) {
            return ERROR_INVALID_OPERATION;
        }
        int ret = native_set_pos_update_period(periodInFrames);
        if (mDebug) {
            Log.i(TAG, "setPositionNotificationPeriod, return:" + ret);
        }
        return ret;
    }
    ...
    public int setPlaybackHeadPosition(int positionInFrames) {
        if (mDebug) {
            Log.i(TAG, "setPlaybackHeadPosition(position:" + positionInFrames + "), mState:" + mState);
        }
        ...
        int ret = native_set_position(positionInFrames);
        if (mDebug) {
            Log.i(TAG, "setPlaybackHeadPosition, return:" + ret);
        }
        return ret;
    }
    ...
    public int setLoopPoints(int startInFrames, int endInFrames, int loopCount) {
        if (mDebug) {
            Log.i(TAG, "setLoopPoints(start:" + startInFrames + ", end:" + endInFrames
                + ", loop:" + loopCount+ ")");
        }
        ...
        int ret = native_set_loop(startInFrames, endInFrames, loopCount);
        if (mDebug) {
            Log.i(TAG, "setLoopPoints, return:" + ret);
        }
        return ret;
    }
    ...
    protected void setState(int state) {
        if (mDebug) {
            Log.i(TAG, "setState(" + state + "), oldState:" + mState);
        }
        mState = state;
    }
    ...
    throws IllegalStateException {
        if (mDebug) {
            Log.i(TAG, "play, mPlayState:" + mPlayState);
        }
        if (mState != STATE_INITIALIZED) {
    ...
    throws IllegalStateException {
        if (mDebug) {
            Log.i(TAG, "stop, mPlayState:" + mPlayState);
        }
        if (mState != STATE_INITIALIZED) {
    ...
    throws IllegalStateException {
        if (mDebug) {
            Log.i(TAG, "pause, mPlayState:" + mPlayState);
        }
        if (mState != STATE_INITIALIZED) {
    ...
    public int write(byte[] audioData, int offsetInBytes, int sizeInBytes) {
        if (mDebug) {
            Log.i(TAG, "write_byte(offset:" + offsetInBytes + ", size:" + sizeInBytes + ")");
        }
    ...
    public int write(short[] audioData, int offsetInShorts, int sizeInShorts) {
        if (mDebug) {
            Log.i(TAG, "write_short(offset:" + offsetInShorts + ", size:" + sizeInShorts + ")");
        }
    ...
    public int attachAuxEffect(int effectId) {
        if (mDebug) {
            Log.i(TAG, "attachAuxEffect(effectId:" + effectId + ")");
        }
        if (mState == STATE_UNINITIALIZED) {
            return ERROR_INVALID_OPERATION;
        }
    ...
    public int setAuxEffectSendLevel(float level) {
        if (mDebug) {
            Log.i(TAG, "setAuxEffectSendLevel(level:" + level + ")");
        }
        if (mState == STATE_UNINITIALIZED) {
            return ERROR_INVALID_OPERATION;
        }
