...
public class MediaPlayer implements SubtitleController.Listener, MediaPlayerEx
{
    ...
    private boolean mBufferFlag = false;
    ...
    private static final int INVOKE_ID_CMD_GET_MEDIA_URL = 5500;
    private static final int INVOKE_ID_GET_CMD_CONNECT_ID = 5501;
    /**
    ...
    public void setDisplay(SurfaceHolder sh) {
        ...
        if (sh != null) {
            ...
            mMediaPlayerContext = surface.getSurfaceContext();
            //add for hiplayer
            sh.setType(SurfaceHolder.SURFACE_TYPE_HISI_TRANSPARENT);
            if(surface != null) {
                surface.mPlayer = this;
                //force update video position here
                Rect videoRect = surface.getVideoRect();
                //Log.w(TAG,"videoRect:"+"left="+videoRect.left+" top="+videoRect.top
                //    +" right="+videoRect.right+" bottom="+videoRect.bottom);
                StringBuilder builder = new StringBuilder();;
                builder.append(".left="+videoRect.left);
                builder.append(".top="+videoRect.top);
                builder.append(".right="+videoRect.right);
                builder.append(".bottom="+videoRect.bottom);
                setParameter(KEY_PARAMETER_VIDEO_POSITION_INFO,builder.toString());
            }
        } else {
            surface = null;
        }
    ...
    public void setSurface(Surface surface) {
        ...
        if (surface != null) {
            surface.mPlayer = this;
            Rect videoRect = surface.getVideoRect();
            Log.e(TAG,"videoRect:"+"left="+videoRect.left+" top="+videoRect.top
                +" right="+videoRect.right+" bottom="+videoRect.bottom);
        }
        updateSurfaceScreenOn();
    }
    ...
    public void setDataSource(String path)
        ...
        if (mSurfaceHolder != null) {
            Log.e(TAG, "call setDataSource function ");
    ...
    private void setDataSource(String path, String[] keys, String[] values)
        ...
        if("true".equals(SystemProperties.get("persist.sys.softdetector.enable", "false"))){
            getConnectID();
            getMediaURL();
            sendExtendMessage(MESSAGE_PLAY_PREPARE,0,0);
        }
    }
    ...
    public  void start() throws IllegalStateException {
        ...
        if("true".equals(SystemProperties.get("persist.sys.softdetector.enable", "false"))){
            if (bPause) {
                sendExtendMessage(MESSAGE_PLAY_RESUME,0,0);
            }
        }
    }
    ...
    public void pause() throws IllegalStateException {
        ...
        _pause();
        if("true".equals(SystemProperties.get("persist.sys.softdetector.enable", "false"))){
            sendExtendMessage(MESSAGE_PLAY_PAUSE,0,0);
        }
    }
    ...
    public CHANNEL switchChannel(CHANNEL channel) {
        int fixed = -1;
        int INVOKE_ID_SET_AUDIO_CHANNEL = 5331; //must equal to CMD_SET_AUDIO_CHANNEL in HiMediaPlayerInvoke.java
        Parcel request = Parcel.obtain();
        Parcel reply = Parcel.obtain();

        try {
            request.writeInterfaceToken(IMEDIA_PLAYER);
            request.writeInt(INVOKE_ID_SET_AUDIO_CHANNEL);
            request.writeInt(channel.ordinal());
            invoke(request, reply);
            reply.setDataPosition(0);
            fixed = reply.readInt();
        } finally {
    ...
    public void release() {
        if("true".equals(SystemProperties.get("persist.sys.softdetector.enable", "false"))){
            sendExtendMessage(MESSAGE_PLAY_STOP,0,0);
        }
        if (mUvmosConnectId != 0) {
            sendUVMOSExtendMessage(UVMOS_MESSAGE_PLAY_COMPLETE,0,0);
        }
        stayAwake(false);
        updateSurfaceScreenOn();
    ...
    private class EventHandler extends Handler
    {
        ...
        public void handleMessage(Message msg) {
            ...
            case MEDIA_SEEK_COMPLETE:
                ...
                if("true".equals(SystemProperties.get("persist.sys.softdetector.enable", "false"))){
                    sendExtendMessage(MESSAGE_SEEK_END,0,0);
                    // fall through
                    if (1 == msg.arg1){
                        mSeekFlag = false;
                        Log.d(TAG, "second seek msg.arg1:"+msg.arg1);
                        sendExtendMessage(MESSAGE_SEEK_START,0,0);
                    }
                }
            ...
            case MEDIA_INFO:
                ...
                case MEDIA_INFO_BUFFERING_START:{
                    if("true".equals(SystemProperties.get("persist.sys.softdetector.enable", "false"))){
                        sendExtendMessage(MESSAGE_BUFFER_START,0,0);
                    }
                    sendUVMOSExtendMessage(UVMOS_MESSAGE_BUFFER_START,0,0);
                    }break;
                case MEDIA_INFO_BUFFERING_END:{
                    if("true".equals(SystemProperties.get("persist.sys.softdetector.enable", "false"))){
                        sendExtendMessage(MESSAGE_BUFFER_END,0,0);
                    }
                    sendUVMOSExtendMessage(UVMOS_MESSAGE_BUFFER_END,0,0);
                    }break;
                case MEDIA_INFO_EXTEND_FIRST_FRAME_TIME:{
                    if("true".equals(SystemProperties.get("persist.sys.softdetector.enable", "false"))){
                        mBitRate = msg.arg2;
                        sendExtendMessage(MESSAGE_PLAY_START,0,0);
                    }
                    sendUVMOSExtendMessage(UVMOS_MESSAGE_PLAY_START,0,0);
                    }break;
                case MEDIA_INFO_EXTEND_BUFFER_LENGTH:{
                    if("true".equals(SystemProperties.get("persist.sys.softdetector.enable", "false"))){
                        mBufferDuration = (long)msg.arg2/1000;
                        sendExtendMessage(MESSAGE_BUFFER_LENGTH,0,0);
                    }
                    }break;
                }
    ...
    private static final int MESSAGE_PLAY_PREPARE = 1;
    private static final int MESSAGE_PLAY_START = 2;
    private static final int MESSAGE_SEEK_START = 3;
    private static final int MESSAGE_SEEK_END = 4;
    private static final int MESSAGE_PLAY_PAUSE = 5;
    private static final int MESSAGE_PLAY_RESUME = 6;
    private static final int MESSAGE_BUFFER_START = 7;
    private static final int MESSAGE_BUFFER_END = 8;
    private static final int MESSAGE_BUFFER_LENGTH = 9;
    private static final int MESSAGE_PLAY_STOP = 10;
    private static final int UVMOS_MESSAGE_PLAY_PREPARE = 11;
    private static final int UVMOS_MESSAGE_PLAY_START = 12;
    private static final int UVMOS_MESSAGE_PLAY_COMPLETE = 13;
    private static final int UVMOS_MESSAGE_BUFFER_START = 14;
    private static final int UVMOS_MESSAGE_BUFFER_END = 15;
    private static final int MEDIA_INFO_EXTEND_BUFFER_LENGTH = 5000;
    private static final int MEDIA_INFO_EXTEND_FIRST_FRAME_TIME = 5001;
    private static final int MEDIA_INFO_EXTEND_SETDATASOURCE = 5002;

    private  boolean  bPause = false;
    private boolean bStart = false;
    private static int mConnectId = 0;
    private int mBitRate = 0;
    private long mBufferDuration = 0;
    private String mMediaUrl = "";

    private long mUvmosCountTime = -1;
    private static int mUvmosConnectId = 0;
    private String mUvmosMediaUrl = "";
    private int mFrameRate = 0;
    private long mVideoRate = 0;
    private long mMediaWidth = 0;
    private long mMediaHeight = 0;
    private String mEncodeType = "";
    private boolean mbLocalMediaUrl = false;
    private long mCountTime = -1;
    private static Context mMediaPlayerContext = null;

    public static final String MEDIA_INFO_EXTEND_STRING_MESSAGE_TYPE = "TYPE";
    public static final String MEDIA_INFO_EXTEND_STRING_PLAY_PREPARE = "PLAY_PREPARE";
    public static final String MEDIA_INFO_EXTEND_STRING_PLAY_START = "PLAY_START";
    public static final String MEDIA_INFO_EXTEND_STRING_SEEK_START = "SEEK_START";
    public static final String MEDIA_INFO_EXTEND_STRING_SEEK_END = "SEEK_END";
    public static final String MEDIA_INFO_EXTEND_STRING_PAUSE_MESSAGE = "PAUSE_MESSAGE";
    public static final String MEDIA_INFO_EXTEND_STRING_RESUME_MESSAGE = "RESUME_MESSAGE";
    public static final String MEDIA_INFO_EXTEND_STRING_BUFFER_START = "BUFFER_START";
    public static final String MEDIA_INFO_EXTEND_STRING_BUFFER_END = "BUFFER_END";
    public static final String MEDIA_INFO_EXTEND_STRING_PLAYABE_REPORT = "PLAYABE_REPORT";
    public static final String MEDIA_INFO_EXTEND_STRING_PLAY_QUIT = "PLAY_QUIT";
    public static final String MEDIA_INFO_EXTEND_STRING_ID = "ID";//int
    public static final String MEDIA_INFO_EXTEND_STRING_URL = "URL";//string
    public static final String MEDIA_INFO_EXTEND_STRING_START_TIME = "START_TIME";//long
    public static final String MEDIA_INFO_EXTEND_STRING_BITRATE = "BITRATE";//int
    public static final String MEDIA_INFO_EXTEND_STRING_PLAY_TIME = "PLAY_TIME";//int
    public static final String MEDIA_INFO_EXTEND_STRING_END_TIME = "END_TIME";//long
    public static final String MEDIA_INFO_EXTEND_STRING_TIME = "TIME";//long
    public static final String MEDIA_INFO_EXTEND_STRING_BUFFER_LENGTH = "SECONDS";//long
    /* UVMOS */
    public static final String MEDIA_INFO_EXTEND_STRING_MESSAGE_STATUS = "PLAYER_STATUS";
    public static final String MEDIA_INFO_EXTEND_STRING_MESSAGE_PLAY_PREPARE = "UVMOS_PLAY_PREPARE";
    public static final String MEDIA_INFO_EXTEND_STRING_MESSAGE_PLAY_START = "UVMOS_PLAY_START";
    public static final String MEDIA_INFO_EXTEND_STRING_MESSAGE_PLAY_COMPLETE = "UVMOS_PLAY_COMPLETE";
    public static final String MEDIA_INFO_EXTEND_STRING_MESSAGE_BUFFER_START = "UVMOS_BUFFER_START";
    public static final String MEDIA_INFO_EXTEND_STRING_MESSAGE_BUFFER_END = "UVMOS_BUFFER_END";
    public static final String MEDIA_INFO_EXTEND_STRING_PLAYER_TYPE = "PlayerType";
    public static final String MEDIA_INFO_EXTEND_STRING_PLAYER = "HiPlayer";
    public static final String MEDIA_INFO_EXTEND_STRING_BUFFER_START_TIME = "StallingHappenTime";
    public static final String MEDIA_INFO_EXTEND_STRING_BUFFER_END_TIME = "StallingRestoreTime";
    public static final String MEDIA_INFO_EXTEND_STRING_UVMOS_ID = "ID";//int
    public static final String MEDIA_INFO_EXTEND_STRING_UVMOS_URL = "VideoURL";//string
    public static final String MEDIA_INFO_EXTEND_STRING_UVMOS_START_TIME = "StartPlayTime";//int
    public static final String MEDIA_INFO_EXTEND_STRING_UVMOS_COMPLETE_TIME = "CompleteTime";
    public static final String MEDIA_INFO_EXTEND_STRING_VIDEO_RATE = "VideoRate";//long
    public static final String MEDIA_INFO_EXTEND_STRING_UVMOS_PLAY_TIME = "DisplayFirstFrameTime";//int
    public static final String MEDIA_INFO_EXTEND_STRING_MEDIA_WIDTH = "MediaWidthResolution";//int
    public static final String MEDIA_INFO_EXTEND_STRING_MEDIA_HEIGHT = "MediaHeightResolution";//int
    public static final String MEDIA_INFO_EXTEND_STRING_FRAME_RATE = "FrameRate";//int
    public static final String MEDIA_INFO_EXTEND_STRING_ENCODE_TYPE = "EncodeType";//string

    private void getUvmosConnectID() {
        mUvmosConnectId = (int)Math.random() * 10 + 1;
    }

    private void getConnectID(){
        String defPlayerFlag = SystemProperties.get("service.media.hiplayer", "");
        if ("false".equals(defPlayerFlag)) {
            return;
        }
        Parcel request = Parcel.obtain();
        Parcel reply = Parcel.obtain();
        try {
            request.writeInterfaceToken(IMEDIA_PLAYER);
            request.writeInt(INVOKE_ID_GET_CMD_CONNECT_ID);
            invoke(request, reply);
            mConnectId = reply.readInt();
        } finally {
            request.recycle();
            reply.recycle();
        }
    ...
    private void getMediaURL(){
        ...
        try {
            request.writeInterfaceToken(IMEDIA_PLAYER);
            request.writeInt(INVOKE_ID_CMD_GET_MEDIA_URL);
            invoke(request, reply);
            mMediaUrl = reply.readString();
        } finally {
            request.recycle();
            reply.recycle();
        }
    ...
    private void setUvMOSInfo() {
        getUvmosConnectID();
        Metadata meta = getMetadata(true, true);
        if (null != meta) {
            if (meta.has(Metadata.VIDEO_CODEC)) {
                mEncodeType = meta.getString(Metadata.VIDEO_CODEC);
            } else {
                mEncodeType = "";
                Log.d(TAG, "EncodeType: "+Metadata.VIDEO_CODEC + "has no this key");
            }
            if (meta.has(Metadata.BIT_RATE)) {
                mVideoRate = meta.getInt(Metadata.BIT_RATE);
            } else {
                mVideoRate = 0;
                Log.d(TAG, "VideoRate: "+Metadata.BIT_RATE + "has no this key");
            }
            if (meta.has(Metadata.VIDEO_FRAME_RATE)) {
                mFrameRate = meta.getInt(Metadata.VIDEO_FRAME_RATE);
            } else {
                mFrameRate = 0;
                Log.d(TAG, "FrameRate: "+Metadata.VIDEO_FRAME_RATE + "has no this key");
            }
            if (meta.has(Metadata.VIDEO_HEIGHT)) {
                mMediaHeight = meta.getInt(Metadata.VIDEO_HEIGHT);
            } else {
                mMediaHeight = 0;
                Log.d(TAG, "VideoHeight: "+Metadata.VIDEO_HEIGHT + "has no this key");
            }
            if (meta.has(Metadata.VIDEO_WIDTH)) {
                mMediaWidth = meta.getInt(Metadata.VIDEO_WIDTH);
            } else {
                mMediaWidth = 0;
                Log.d(TAG, "VideoWidth: "+Metadata.VIDEO_WIDTH + "has no this key");
            }
            meta.releaseParcel();
        } else {
            mEncodeType = "";
        ...
        sendUVMOSExtendMessage(UVMOS_MESSAGE_PLAY_PREPARE,0,0);
    }

    public void sendUVMOSExtendMessage(int what, int arg0, int arg1) {
        String PropertyValueUvmos = "";
        PropertyValueUvmos = SystemProperties.get("persist.media.hiplayer.uvmos");
        if (PropertyValueUvmos != null && PropertyValueUvmos.equals("true")) {
            boolean bSend = true;
            long currenttime = System.currentTimeMillis();
            Intent intent = new Intent("MEDIA_PLAY_UVMOS_MONITOR_MESSAGE");
            SimpleDateFormat formater = new SimpleDateFormat("yyyy-MM-dd-HH:mm:ss");
            Date date;

            switch (what) {
                case UVMOS_MESSAGE_PLAY_PREPARE:
                    mUvmosCountTime = System.currentTimeMillis();
                    intent.putExtra(MEDIA_INFO_EXTEND_STRING_MESSAGE_STATUS, MEDIA_INFO_EXTEND_STRING_MESSAGE_PLAY_PREPARE);
                    intent.putExtra(MEDIA_INFO_EXTEND_STRING_PLAYER_TYPE, MEDIA_INFO_EXTEND_STRING_PLAYER);
                    intent.putExtra(MEDIA_INFO_EXTEND_STRING_UVMOS_ID, mUvmosConnectId);
                    intent.putExtra(MEDIA_INFO_EXTEND_STRING_UVMOS_START_TIME, mUvmosCountTime);
                    intent.putExtra(MEDIA_INFO_EXTEND_STRING_UVMOS_URL, mUvmosMediaUrl);
                    intent.putExtra(MEDIA_INFO_EXTEND_STRING_MEDIA_WIDTH, mMediaWidth);
                    intent.putExtra(MEDIA_INFO_EXTEND_STRING_MEDIA_HEIGHT, mMediaHeight);
                    intent.putExtra(MEDIA_INFO_EXTEND_STRING_VIDEO_RATE, mVideoRate);
                    intent.putExtra(MEDIA_INFO_EXTEND_STRING_FRAME_RATE, mFrameRate);
                    intent.putExtra(MEDIA_INFO_EXTEND_STRING_ENCODE_TYPE, mEncodeType);
                    Log.d(TAG, "broadcast "+MEDIA_INFO_EXTEND_STRING_MESSAGE_STATUS + ": " +MEDIA_INFO_EXTEND_STRING_MESSAGE_PLAY_PREPARE);
                    Log.d(TAG, "--------- "+MEDIA_INFO_EXTEND_STRING_UVMOS_ID + ": " +mUvmosConnectId);
                    Log.d(TAG, "--------- "+MEDIA_INFO_EXTEND_STRING_PLAYER_TYPE + ": " +MEDIA_INFO_EXTEND_STRING_PLAYER);
                    Log.d(TAG, "--------- "+MEDIA_INFO_EXTEND_STRING_UVMOS_START_TIME + ": " +formater.format(mUvmosCountTime));
                    Log.d(TAG, "--------- "+MEDIA_INFO_EXTEND_STRING_UVMOS_URL + ": " +mUvmosMediaUrl);
                    Log.d(TAG, "--------- "+MEDIA_INFO_EXTEND_STRING_MEDIA_WIDTH + ": " +mMediaWidth);
                    Log.d(TAG, "--------- "+MEDIA_INFO_EXTEND_STRING_MEDIA_HEIGHT + ": " +mMediaHeight);
                    Log.d(TAG, "--------- "+MEDIA_INFO_EXTEND_STRING_VIDEO_RATE + ": " +mVideoRate);
                    Log.d(TAG, "--------- "+MEDIA_INFO_EXTEND_STRING_FRAME_RATE + ": " +mFrameRate);
                    Log.d(TAG, "--------- "+MEDIA_INFO_EXTEND_STRING_ENCODE_TYPE + ": " +mEncodeType);
                    break;
                case UVMOS_MESSAGE_PLAY_START:
                    intent.putExtra(MEDIA_INFO_EXTEND_STRING_MESSAGE_STATUS, MEDIA_INFO_EXTEND_STRING_MESSAGE_PLAY_START);
                    intent.putExtra(MEDIA_INFO_EXTEND_STRING_UVMOS_ID, mUvmosConnectId);
                    intent.putExtra(MEDIA_INFO_EXTEND_STRING_UVMOS_PLAY_TIME, currenttime);
                    Log.d(TAG, "broadcast "+MEDIA_INFO_EXTEND_STRING_MESSAGE_STATUS + ": " +MEDIA_INFO_EXTEND_STRING_MESSAGE_PLAY_START);
                    Log.d(TAG, "--------- "+MEDIA_INFO_EXTEND_STRING_UVMOS_ID + ": " +mUvmosConnectId);
                    Log.d(TAG, "--------- "+MEDIA_INFO_EXTEND_STRING_UVMOS_PLAY_TIME + ": " +formater.format(currenttime));
                    break;
                case UVMOS_MESSAGE_PLAY_COMPLETE:
                    intent.putExtra(MEDIA_INFO_EXTEND_STRING_MESSAGE_STATUS, MEDIA_INFO_EXTEND_STRING_MESSAGE_PLAY_COMPLETE);
                    intent.putExtra(MEDIA_INFO_EXTEND_STRING_UVMOS_ID, mUvmosConnectId);
                    intent.putExtra(MEDIA_INFO_EXTEND_STRING_UVMOS_COMPLETE_TIME, currenttime);
                    Log.d(TAG, "broadcast "+MEDIA_INFO_EXTEND_STRING_MESSAGE_STATUS + ": " +MEDIA_INFO_EXTEND_STRING_MESSAGE_PLAY_COMPLETE);
                    Log.d(TAG, "--------- "+MEDIA_INFO_EXTEND_STRING_UVMOS_ID + ": " +mConnectId);
                    Log.d(TAG, "--------- "+MEDIA_INFO_EXTEND_STRING_UVMOS_COMPLETE_TIME + ": " +formater.format(currenttime));
                    break;
                case UVMOS_MESSAGE_BUFFER_START:
                    intent.putExtra(MEDIA_INFO_EXTEND_STRING_MESSAGE_STATUS, MEDIA_INFO_EXTEND_STRING_MESSAGE_BUFFER_START);
                    intent.putExtra(MEDIA_INFO_EXTEND_STRING_UVMOS_ID, mUvmosConnectId);
                    intent.putExtra(MEDIA_INFO_EXTEND_STRING_BUFFER_START_TIME, currenttime);
                    Log.d(TAG, "broadcast "+MEDIA_INFO_EXTEND_STRING_MESSAGE_STATUS + ": " +MEDIA_INFO_EXTEND_STRING_MESSAGE_BUFFER_START);
                    Log.d(TAG, "--------- "+MEDIA_INFO_EXTEND_STRING_UVMOS_ID + ": " +mUvmosConnectId);
                    Log.d(TAG, "--------- "+MEDIA_INFO_EXTEND_STRING_BUFFER_START_TIME + ": " +formater.format(currenttime));
                    break;
                case UVMOS_MESSAGE_BUFFER_END:
                    intent.putExtra(MEDIA_INFO_EXTEND_STRING_MESSAGE_STATUS, MEDIA_INFO_EXTEND_STRING_MESSAGE_BUFFER_END);
                    intent.putExtra(MEDIA_INFO_EXTEND_STRING_UVMOS_ID, mUvmosConnectId);
                    intent.putExtra(MEDIA_INFO_EXTEND_STRING_BUFFER_END_TIME, currenttime);
                    Log.d(TAG, "broadcast "+MEDIA_INFO_EXTEND_STRING_MESSAGE_STATUS + ": " +MEDIA_INFO_EXTEND_STRING_MESSAGE_BUFFER_END);
                    Log.d(TAG, "--------- "+MEDIA_INFO_EXTEND_STRING_UVMOS_ID + ": " +mUvmosConnectId);
                    Log.d(TAG, "--------- "+MEDIA_INFO_EXTEND_STRING_BUFFER_END_TIME + ": " +formater.format(currenttime));
                    break;
                default:
                    bSend = false;
                    Log.d(TAG, "error, unkown message index ");
                    break;
            }
            Context mContext = ActivityThread.currentApplication();
            if (null != mContext && bSend) {
            mContext.sendBroadcast(intent);
            } else {
                if (null == mContext) {
                    Log.d(TAG, "error, unkown state Context:"+mContext);
                }
            }
        } else {
            Log.d(TAG, "has no set the property of uvmos!!! ");
        }
    }

    void sendExtendMessage(int what, int arg0, int arg1){
        String defPlayerFlag = SystemProperties.get("service.media.hiplayer", "");
        if ("false".equals(defPlayerFlag)) {
            return;
        }
        ...
        switch(what){
            case MESSAGE_PLAY_PREPARE:{
                ...
                intent.putExtra(MEDIA_INFO_EXTEND_STRING_MESSAGE_TYPE, MEDIA_INFO_EXTEND_STRING_PLAY_PREPARE);
                intent.putExtra(MEDIA_INFO_EXTEND_STRING_ID, mConnectId);
                intent.putExtra(MEDIA_INFO_EXTEND_STRING_URL, mMediaUrl);
                intent.putExtra(MEDIA_INFO_EXTEND_STRING_START_TIME, mCountTime);
                intent.putExtra(MEDIA_INFO_EXTEND_STRING_MEDIA_WIDTH, mMediaWidth);
                intent.putExtra(MEDIA_INFO_EXTEND_STRING_MEDIA_HEIGHT, mMediaHeight);
                intent.putExtra(MEDIA_INFO_EXTEND_STRING_FRAME_RATE, mFrameRate);
                intent.putExtra(MEDIA_INFO_EXTEND_STRING_ENCODE_TYPE, mEncodeType);
                mSeekFlag = false;
                bPause = false;
                if(mMediaUrl.startsWith("http:/") || mMediaUrl.startsWith("rtsp:/")
                    || mMediaUrl.startsWith( "rtp:/") || mMediaUrl.startsWith("mms:/")
                    || mMediaUrl.startsWith("https:/")
                    || mMediaUrl.startsWith("rtmp:/") || mMediaUrl.startsWith( "udp:/")){
                        mbLocalMediaUrl = true;
                }else {
                        mbLocalMediaUrl = false;
                }
                ...
                Log.d(TAG, "--------- "+MEDIA_INFO_EXTEND_STRING_START_TIME + ": " +formater.format(mCountTime));
                Log.d(TAG, "--------- "+MEDIA_INFO_EXTEND_STRING_MEDIA_WIDTH + ": " +mMediaWidth);
                Log.d(TAG, "--------- "+MEDIA_INFO_EXTEND_STRING_MEDIA_HEIGHT + ": " +mMediaHeight);
                Log.d(TAG, "--------- "+MEDIA_INFO_EXTEND_STRING_FRAME_RATE + ": " +mFrameRate);
                Log.d(TAG, "--------- "+MEDIA_INFO_EXTEND_STRING_ENCODE_TYPE + ": " +mEncodeType);
            ...
            case MESSAGE_PLAY_START:{
                ...
                intent.putExtra(MEDIA_INFO_EXTEND_STRING_MESSAGE_TYPE, MEDIA_INFO_EXTEND_STRING_PLAY_START);
                intent.putExtra(MEDIA_INFO_EXTEND_STRING_ID, mConnectId);
                intent.putExtra(MEDIA_INFO_EXTEND_STRING_PLAY_TIME, position);
                intent.putExtra(MEDIA_INFO_EXTEND_STRING_END_TIME, currenttime);
                intent.putExtra(MEDIA_INFO_EXTEND_STRING_BITRATE, mBitRate);
                Log.d(TAG, "broadcast "+MEDIA_INFO_EXTEND_STRING_MESSAGE_TYPE + ": " +MEDIA_INFO_EXTEND_STRING_PLAY_START);
                Log.d(TAG, "--------- "+MEDIA_INFO_EXTEND_STRING_ID + ": " +mConnectId);
                Log.d(TAG, "--------- "+MEDIA_INFO_EXTEND_STRING_PLAY_TIME
                    +" "+(int)(position/3600)
                    +":"+(int)(position/60)
                    +":"+(int)(position %60));
                Log.d(TAG, "--------- "+MEDIA_INFO_EXTEND_STRING_END_TIME + ": " +formater.format(currenttime));
                Log.d(TAG, "--------- "+MEDIA_INFO_EXTEND_STRING_BITRATE + ": " +mBitRate);
            }break;
            case MESSAGE_SEEK_START:{
                int position = arg0/1000;
                intent.putExtra(MEDIA_INFO_EXTEND_STRING_MESSAGE_TYPE, MEDIA_INFO_EXTEND_STRING_SEEK_START);
                intent.putExtra(MEDIA_INFO_EXTEND_STRING_ID, mConnectId);
                intent.putExtra(MEDIA_INFO_EXTEND_STRING_PLAY_TIME, position);
                ...
                Log.d(TAG, "broadcast "+MEDIA_INFO_EXTEND_STRING_MESSAGE_TYPE + ": " +MEDIA_INFO_EXTEND_STRING_SEEK_START);
                Log.d(TAG, "--------- "+MEDIA_INFO_EXTEND_STRING_ID + ": " +mConnectId);
                Log.d(TAG, "--------- "+MEDIA_INFO_EXTEND_STRING_PLAY_TIME
                    +" "+(int)(position/3600)
                    +":"+(int)(position/60)
            ...
            case MESSAGE_SEEK_END:{
                int position = getCurrentPosition()/1000;
                intent.putExtra(MEDIA_INFO_EXTEND_STRING_MESSAGE_TYPE, MEDIA_INFO_EXTEND_STRING_SEEK_END);
                intent.putExtra(MEDIA_INFO_EXTEND_STRING_ID, mConnectId);
                intent.putExtra(MEDIA_INFO_EXTEND_STRING_PLAY_TIME, position);
                intent.putExtra(MEDIA_INFO_EXTEND_STRING_END_TIME, currenttime);
                if (mSeekFlag) {
                ...
                Log.d(TAG, "broadcast "+MEDIA_INFO_EXTEND_STRING_MESSAGE_TYPE + ": " +MEDIA_INFO_EXTEND_STRING_SEEK_END);
                Log.d(TAG, "--------- "+MEDIA_INFO_EXTEND_STRING_ID + ": " +mConnectId);
                Log.d(TAG, "--------- "+MEDIA_INFO_EXTEND_STRING_PLAY_TIME
                    +" "+(int)(position/3600)
                    +":"+(int)(position/60)
            ...
            case MESSAGE_PLAY_PAUSE:{
                int position = getCurrentPosition()/1000;
                bPause = true;
                intent.putExtra(MEDIA_INFO_EXTEND_STRING_MESSAGE_TYPE, MEDIA_INFO_EXTEND_STRING_PAUSE_MESSAGE);
                intent.putExtra(MEDIA_INFO_EXTEND_STRING_ID, mConnectId);
                intent.putExtra(MEDIA_INFO_EXTEND_STRING_PLAY_TIME, position);
                intent.putExtra(MEDIA_INFO_EXTEND_STRING_TIME, currenttime);
                Log.d(TAG, "broadcast "+MEDIA_INFO_EXTEND_STRING_MESSAGE_TYPE + ": " +MEDIA_INFO_EXTEND_STRING_PAUSE_MESSAGE);
                Log.d(TAG, "--------- "+MEDIA_INFO_EXTEND_STRING_ID + ": " +mConnectId);
                Log.d(TAG, "--------- "+MEDIA_INFO_EXTEND_STRING_PLAY_TIME
                    +" "+(int)(position/3600)
                    +":"+(int)(position/60)
                    +":"+(int)(position%60));
                Log.d(TAG, "--------- "+MEDIA_INFO_EXTEND_STRING_TIME + ": " +formater.format(currenttime));
            }break;
            case MESSAGE_PLAY_RESUME:{
                int position = getCurrentPosition()/1000;
                bPause = false;
                intent.putExtra(MEDIA_INFO_EXTEND_STRING_MESSAGE_TYPE, MEDIA_INFO_EXTEND_STRING_RESUME_MESSAGE);
                intent.putExtra(MEDIA_INFO_EXTEND_STRING_ID, mConnectId);
                intent.putExtra(MEDIA_INFO_EXTEND_STRING_PLAY_TIME, position);
                intent.putExtra(MEDIA_INFO_EXTEND_STRING_TIME, currenttime);
                Log.d(TAG, "broadcast "+MEDIA_INFO_EXTEND_STRING_MESSAGE_TYPE + ": " +MEDIA_INFO_EXTEND_STRING_RESUME_MESSAGE);
                Log.d(TAG, "--------- "+MEDIA_INFO_EXTEND_STRING_ID + ": " +mConnectId);
                Log.d(TAG, "--------- "+MEDIA_INFO_EXTEND_STRING_PLAY_TIME
                    +" "+(int)(position/3600)
                    +":"+(int)(position/60)
            ...
            case MESSAGE_BUFFER_START:{
                int position = getCurrentPosition()/1000;
                intent.putExtra(MEDIA_INFO_EXTEND_STRING_MESSAGE_TYPE, MEDIA_INFO_EXTEND_STRING_BUFFER_START);
                intent.putExtra(MEDIA_INFO_EXTEND_STRING_ID, mConnectId);
                intent.putExtra(MEDIA_INFO_EXTEND_STRING_PLAY_TIME, position);
                intent.putExtra(MEDIA_INFO_EXTEND_STRING_START_TIME, currenttime);
                if (!mSeekFlag && bStart) {
                    mBufferFlag = true;
                    Log.d(TAG, "broadcast "+MEDIA_INFO_EXTEND_STRING_MESSAGE_TYPE + ": " +MEDIA_INFO_EXTEND_STRING_BUFFER_START);
                    Log.d(TAG, "--------- "+MEDIA_INFO_EXTEND_STRING_ID + ": " +mConnectId);
                    Log.d(TAG, "--------- "+MEDIA_INFO_EXTEND_STRING_PLAY_TIME
                        +" "+(int)(position/3600)
                        +":"+(int)(position/60)
            ...
            case MESSAGE_BUFFER_END:{
                intent.putExtra(MEDIA_INFO_EXTEND_STRING_MESSAGE_TYPE, MEDIA_INFO_EXTEND_STRING_BUFFER_END);
                intent.putExtra(MEDIA_INFO_EXTEND_STRING_ID, mConnectId);
                intent.putExtra(MEDIA_INFO_EXTEND_STRING_END_TIME, currenttime);
                if (mBufferFlag && !mSeekFlag && bStart) {
                    mBufferFlag = false;
                    Log.d(TAG, "broadcast "+MEDIA_INFO_EXTEND_STRING_MESSAGE_TYPE + ": " +MEDIA_INFO_EXTEND_STRING_BUFFER_END);
                    Log.d(TAG, "--------- "+MEDIA_INFO_EXTEND_STRING_ID + ": " +mConnectId);
                    Log.d(TAG, "--------- "+MEDIA_INFO_EXTEND_STRING_END_TIME + ": " +formater.format(currenttime));
                } else {
                    bSend = false;
            ...
            case MESSAGE_BUFFER_LENGTH:{
                if(10000 < (currenttime - mCountTime) && bStart){
                    mCountTime = currenttime;
                    intent.putExtra(MEDIA_INFO_EXTEND_STRING_MESSAGE_TYPE, MEDIA_INFO_EXTEND_STRING_PLAYABE_REPORT);
                    intent.putExtra(MEDIA_INFO_EXTEND_STRING_ID, mConnectId);
                    if (!mbLocalMediaUrl) {
                        mBufferDuration = 0;
                    }
                    intent.putExtra(MEDIA_INFO_EXTEND_STRING_BUFFER_LENGTH, mBufferDuration);
                    intent.putExtra(MEDIA_INFO_EXTEND_STRING_TIME, currenttime);
                    Log.d(TAG, "broadcast "+MEDIA_INFO_EXTEND_STRING_MESSAGE_TYPE + ": " +MEDIA_INFO_EXTEND_STRING_PLAYABE_REPORT);
                    Log.d(TAG, "--------- "+MEDIA_INFO_EXTEND_STRING_ID + ": " +mConnectId);
                    Log.d(TAG, "--------- "+MEDIA_INFO_EXTEND_STRING_BUFFER_LENGTH
                        + " " +(int)(mBufferDuration/3600)
                        + ":" +(int)(mBufferDuration/60)
                        + ":" +(int)(mBufferDuration%60));
                    Log.d(TAG, "--------- "+MEDIA_INFO_EXTEND_STRING_TIME + ": " +formater.format(currenttime));
                }else {
                    bSend = false;
            ...
            case MESSAGE_PLAY_STOP:{
                if (!mQuitFlag) {
                    ...
                    bPause = false;
                    intent.putExtra(MEDIA_INFO_EXTEND_STRING_MESSAGE_TYPE, MEDIA_INFO_EXTEND_STRING_PLAY_QUIT);
                    intent.putExtra(MEDIA_INFO_EXTEND_STRING_ID, mConnectId);
                    intent.putExtra(MEDIA_INFO_EXTEND_STRING_PLAY_TIME, position);
                    intent.putExtra(MEDIA_INFO_EXTEND_STRING_TIME, currenttime);
                    Log.d(TAG, "broadcast "+MEDIA_INFO_EXTEND_STRING_MESSAGE_TYPE + ": " +MEDIA_INFO_EXTEND_STRING_PLAY_QUIT);
                    Log.d(TAG, "--------- "+MEDIA_INFO_EXTEND_STRING_ID + ": " +mConnectId);
                    Log.d(TAG, "--------- "+MEDIA_INFO_EXTEND_STRING_PLAY_TIME
                        +" "+(int)(position/3600)
                        +":"+(int)(position/60)
        ...
        if (null != mMediaPlayerContext && bSend) {
            mMediaPlayerContext.sendBroadcast(intent);
        } else {
            if (null == mMediaPlayerContext) {
                Log.d(TAG, "error, unkown state Context:"+mMediaPlayerContext);
            }
        }
    }
    private int getIntValueFromInvoke(int cmdId)
    {
        Parcel Request = Parcel.obtain();
        Parcel Reply = Parcel.obtain();
     ...
     private long getLongValueFromInvoke(int cmdId)
    {
        Parcel Request = Parcel.obtain();
        Parcel Reply = Parcel.obtain();
    ...
    private String getStringValueFromInvoke(int cmdId)
    {
        Parcel Request = Parcel.obtain();
        Parcel Reply = Parcel.obtain();
        Request.writeInterfaceToken(IMEDIA_PLAYER);
        Request.writeInt(cmdId);
        invoke(Request, Reply);
        String result = String.valueOf(Reply.readString());
