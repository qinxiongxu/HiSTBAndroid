...
#define POINTER_STATE_FADE 0x10000
#define POINTER_STATE_UNFADE 0x10001
...
public:
    ...
    void setPointerState(int32_t state);
...
void NativeInputManager::setPointerState(int32_t state) {
    if(state == POINTER_STATE_FADE)
    {
        mInputManager->getReader()->requestRefreshConfiguration(
                InputReaderConfiguration::CHANGE_POINTER_FADE);
    }
    else if(state == POINTER_STATE_UNFADE)
    {
        mInputManager->getReader()->requestRefreshConfiguration(
                InputReaderConfiguration::CHANGE_POINTER_UNFADE);
    }
}
...
#endif
	char suspendMode[PROP_VALUE_MAX];
	property_get("service.suspend.type", suspendMode, "0");
        if(!strcmp(suspendMode,"deep_fast"))
	{
		int ret=0;
		ALOGE("majinguang 000099999999");
		HI_SYS_Init();
		...
		{
			HI_UNF_GPIO_SetDirBit(0x2d,HI_FALSE);
			HI_UNF_GPIO_SetDirBit(0x2e,HI_FALSE);
			HI_UNF_GPIO_WriteBit(0x2d, HI_TRUE);
			HI_UNF_GPIO_WriteBit(0x2e, HI_FALSE);
		}
		    HiSysManagerClient sysclient;
    		    String8 device = String8("/sys/devices/system/cpu/cpu0/cpufreq/scaling_governor");
 		    String8 value = String8("userspace");

		sysclient.adjustDevState(device,value);
		device = String8("/proc/msp/pm_cpu");
		value = String8("avs=off");
                sysclient.adjustDevState(device,value);
		ALOGE(" begin sleep");
		usleep(50000);
		ALOGE(" sleep over");
		HI_SYS_WriteRegister(0xf840e520, 0x11111111);
	}
	else
	{
...
static void nativeSetPointerState(JNIEnv* env,
        jclass clazz, jint ptr, jint state) {
    NativeInputManager* im = reinterpret_cast<NativeInputManager*>(ptr);

    im->setPointerState(state);
}
...
static JNINativeMethod gInputManagerMethods[] = {
    ...
    { "nativeSetPointerState", "(II)V",
            (void*) nativeSetPointerState },
