...
import static com.android.server.wifi.WifiController.CMD_WIFI_TOGGLED_FAILED;
import static com.android.server.wifi.WifiController.CMD_SET_AP_FAILED;
/**
...
public final class WifiService extends IWifiManager.Stub {
    ...
    public void checkAndStartWifi() {
        String QbEnable = SystemProperties.get("persist.sys.qb.enable","false");

        if (QbEnable.equals("true")) {
            String QbFlag = SystemProperties.get("persist.sys.qb.flag","true");
            Slog.i(TAG, "WifiService QbFlag =  " + QbFlag);
            /* Do not turn WiFi on when Snap System for qb*/
            if(QbFlag.equals("false")) {
    ...
    /**
     * Check if WiFi-AP needs to be enabled and start
     * if needed
     *
     * This function is used only at boot time
     */
    public void checkAndStartWifiAp() {
        ...
        if (QbEnable.equals("true")) {
            String QbFlag = SystemProperties.get("persist.sys.qb.flag","true");
            Slog.i(TAG, "WifiService QbFlag =  " + QbFlag);
            /* Do not turn WiFi-AP on when Snap System for qb */
            if(QbFlag.equals("false")) {
                return;
            }
        }
        ...
        boolean wifiApEnabled = mSettingsStore.getPersistedWifiApState() == WIFI_ENABLED;
        Slog.i(TAG, "WifiService starting up with WiFi-AP " +
                (wifiApEnabled ? "enabled" : "disabled"));
        if (mSettingsStore.isWifiToggleEnabled()
            || (mSettingsStore.getPersistedWifiApState() != WIFI_ENABLED))
            return;
     ...
     * if needed
     *
     * This function is used only at qb time
     */
    private void checkAndStartWifiAtQb() {
        int wifiStaQbState = WIFI_DISABLED;

        try {
            wifiStaQbState =  Settings.Global.getInt(mContext.getContentResolver(), Settings.Global.WIFI_ON);
        } catch (Settings.SettingNotFoundException e) {
             Slog.e(TAG, "failed to get Settings.Global.WIFI_ON");
             Settings.Global.putInt(mContext.getContentResolver(), Settings.Global.WIFI_ON, WIFI_DISABLED);
             wifiStaQbState =  WIFI_DISABLED;
         }

        Slog.i(TAG, "WifiService qb starting up with Wi-Fi " +
                ((wifiStaQbState ==  WIFI_ENABLED)? "enabled" : "disabled"));

        if (WIFI_ENABLED == wifiStaQbState) {
            setWifiEnabled(true);
        }
    }
     ...
     * This function is used only at qb time
     */
    private void checkAndStartWifiApAtQb() {
        int wifiApQbState = WIFI_DISABLED;
        WifiConfiguration mWifiConfig = getWifiApConfiguration();

        try {
            wifiApQbState =  Settings.Global.getInt(mContext.getContentResolver(), Settings.Global.WIFIAP_ON);
        } catch (Settings.SettingNotFoundException e) {
            Slog.e(TAG, "failed to get Settings.Global.WIFIAP_ON");
            Settings.Global.putInt(mContext.getContentResolver(), Settings.Global.WIFIAP_ON, WIFI_DISABLED);
            wifiApQbState =  WIFI_DISABLED;
        }

        Slog.i(TAG, "WifiService qb starting up with WiFi-AP " +
                ((wifiApQbState ==  WIFI_ENABLED)? "enabled" : "disabled"));

        if (WIFI_ENABLED == wifiApQbState) {
    ...
    /*public void checkAndStartWifiAP() {
        if ((getPersistedWifiState() == WIFI_ENABLED)
              || (getPersistedWifiAPState() != WIFI_ENABLED))
            return;
    ...
    private int getPersistedWifiAPState() {
        ...
        try {
            ...
            Slog.e(TAG, "--->>getPersistedWifiAPState(),not found WIFIAP_ON item");
            Settings.Global.putInt(cr, Settings.Global.WIFIAP_ON, WIFI_DISABLED);
            return WIFI_DISABLED;
        }
    }

    private void persistWifiAPState(boolean enabled) {
        final ContentResolver cr = mContext.getContentResolver();
        Settings.Global.putInt(cr, Settings.Global.WIFIAP_ON,
                                    enabled ? WIFI_ENABLED : WIFI_DISABLED);
    }*/
        ...
        */
        ...
        try {
            if (! mSettingsStore.handleWifiToggled(enable)) {
                ...
                Slog.d(TAG, "airplan Mode on, do not toggle WiFi");
                return true;
            }
        } finally {
            Binder.restoreCallingIdentity(ident);
    ...
    public int getDhcpOption60State() {
        ...
        try {
            ...
            Slog.d(TAG, "getDhcpOption60State: " + option60State);
            return option60State;
        } catch (Settings.SettingNotFoundException e) {
            Slog.e(TAG, "getDhcpOption60State: STATE UNKNOWN");
            return WifiManager.OPTION60_STATE_UNKNOWN;
        }
    }
    ...
    public String getDhcpOption60Login() {
        ...
        try {
            ...
            if("".equals(login))
                login = null;
            Slog.d(TAG, "getDhcpOption60Login: " + login);
            return login;
        } catch (Exception e) {
            Slog.e(TAG, "getDhcpOption60Login Error!");
            return null;
        }
    }
    ...
    public String getDhcpOption60Password() {
        ...
        try {
            ...
            if("".equals(passwd))
                passwd = null;
            Slog.d(TAG, "getDhcpOption60Password: " + passwd);
            return passwd;
        } catch (Exception e) {
            Slog.e(TAG, "getDhcpOption60Password Error!");
            return null;
        }
    }

    public void setDhcpOption60(boolean setEnable, String login, String password) {
        Slog.d(TAG, "setDhcpOption60(" + setEnable + ", " + login + ", " + password + ")");
        final ContentResolver cr = mContext.getContentResolver();
        Settings.Secure.putInt(cr, Settings.Secure.OPTION60_ON,
                setEnable ? WifiManager.OPTION60_STATE_ENABLED: WifiManager.OPTION60_STATE_DISABLED);
    ...
    public int getDhcpOption125State() {
        ...
        try {
            ...
            Slog.d(TAG, "getDhcpOption125State: " + option125State);
            return option125State;
        } catch (Settings.SettingNotFoundException e) {
            Slog.e(TAG, "getWifiDisguiseState: STATE UNKNOWN");
            return WifiManager.OPTION125_STATE_UNKNOWN;
        }
    }
    ...
    public String getDhcpOption125Info() {
        ...
        try {
            ...
            if("".equals(option125Info))
                option125Info = null;
            Slog.d(TAG, "getDhcpOption125Info: " + option125Info);
            return option125Info;
        } catch (Exception e) {
            Slog.e(TAG, "getDhcpOption125Info Error!");
            return null;
        }
    }

    public void setDhcpOption125(boolean setEnable, String option125Info) {
        Slog.d(TAG, "setDhcpOption125(" + setEnable + ", " + option125Info + ")");
        final ContentResolver cr = mContext.getContentResolver();
        Settings.Secure.putInt(cr, Settings.Secure.OPTION125_ON,
                setEnable ? WifiManager.OPTION125_STATE_ENABLED: WifiManager.OPTION125_STATE_DISABLED);
    ...
    /**
     * see {@link android.net.wifi.WifiManager#startWifiWithoutLinkDown}
     *
     */
    public void startWifiWithoutLinkDown() {
        enforceConnectivityInternalPermission();

        mWifiStateMachine.EnableWifiNetwork(true);
    }

    /**
     * see {@link android.net.wifi.WifiManager#stopWifWithoutLinkDown}
     *
     */
    public void stopWifiWithoutLinkDown() {
        enforceConnectivityInternalPermission();

        mWifiStateMachine.EnableWifiNetwork(false);
    }
    ...
    private final BroadcastReceiver mReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(Intent.ACTION_SCREEN_ON)) {
                ...
                mSettingsStore.handleWifiToggled(isWifiEnabled ? true : false);
                if (WifiManager.WIFI_STATE_UNKNOWN == wifiState) {
                    mWifiController.sendMessage(CMD_WIFI_TOGGLED_FAILED);
                }
            } else if(intent.getAction().equals(WifiManager.WIFI_AP_STATE_CHANGED_ACTION)) {
                int wifiApState = intent.getIntExtra(WifiManager.EXTRA_WIFI_AP_STATE, WifiManager.WIFI_AP_STATE_DISABLED);
                boolean isWifiApEnabled = (wifiApState == WifiManager.WIFI_AP_STATE_ENABLED);
                mSettingsStore.handleWifiApToggled(isWifiApEnabled ? true : false);
                if (WifiManager.WIFI_AP_STATE_FAILED == wifiApState) {
                    mWifiController.sendMessage(CMD_SET_AP_FAILED);
                }
            } else if (action.equals(intent.ACTION_QB_PROP_RELOAD)) {
                /* check if WiFi or WiFi-AP needs to be enabled at qb time */
                checkAndStartWifiAtQb();
                checkAndStartWifiApAtQb();
            }
        }
    };
    ...
    private void registerForBroadcasts() {
        ...
        /* register QB broadcast if qb enabled */
        String QbEnable = SystemProperties.get("persist.sys.qb.enable","false");
        if (QbEnable.equals("true"))
