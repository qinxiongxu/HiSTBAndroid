...
final class Settings {
    ...
    // Keys are String(package name), values are apk path. Save the launcher
    // app Packages which installed by user
    public final HashMap<String, String> mPreScanLauncherPackages = new HashMap<String, String>();
    ...
    //for quickboot to Clear PreferredActivities which had been made in mirror
    public void QbClearPreferredActivities()
    {
    ...
    void writeLPr() {
        ...
        try {
            ...
            if(optimize){
                if(mPreScanLauncherPackages.size() > 0) {
                    for(Map.Entry<String, String> e : mPreScanLauncherPackages.entrySet()) {
                        serializer.startTag(null, "prescan-launcher-package");
                        serializer.attribute(null, "name", e.getKey());
                        serializer.attribute(null, "path", e.getValue());
                        serializer.endTag(null, "prescan-launcher-package");
                    }
                }
            }
    ...
    // for qb reload packages(packages.xml)
    void reloadPackages()
    {
        FileInputStream str = null;
        try {
    ...
    boolean readLPw(PackageManagerService service, List<UserInfo> users, int sdkVersion,
        ...
        try {
            ...
            while ((type = parser.next()) != XmlPullParser.END_DOCUMENT
                ...
                } else if (tagName.equals("prescan-launcher-package")) {
                    if(optimize){
                        String name = parser.getAttributeValue(null, "name");
                        String path = parser.getAttributeValue(null, "path");
                        if(name != null && path != null) {
                            mPreScanLauncherPackages.put(name, path);
                        }
                    }
