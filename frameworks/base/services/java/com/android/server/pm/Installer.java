...
public final class Installer {
    ...
    private boolean readReply() {
        ...
        if(optimize){
            if (mReadFull) {
                ...
                buflen = (((int) buf[4]) & 0xff) | ((((int) buf[5]) & 0xff) << 8);
                if ((buflen < 1) || (buflen > 1024)) {
                    Slog.e(TAG, "invalid reply length (" + buflen + ")");
                    disconnect();
                    return false;
                }
            }
            mReadFull = readBytes(buf, buflen);
            return mReadFull;
        }else{
            int len;
            buflen = 0;
    ...
    private synchronized String transaction_Orig(String cmd){
        if (!connect()) {
