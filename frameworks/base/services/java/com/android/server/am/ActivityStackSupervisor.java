...
public final class ActivityStackSupervisor {
    ...
    final Handler mProbeHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case START_HOME_MSG: {
                    if (!mIsHomeActivityStarted) {
                        Slog.i(TAG, "Start home activity");
    ...
    public class ProbeThread extends Thread {
        @Override
        public void run() {
            if (isFastplayStopped() && isBootVideoStopped()) {
    ...
    private boolean isBootVideoStopped() {
        ...
        while (true) {
            bVideoStop = "1".equals(SystemProperties.get("persist.sys.bootvideo.exit", "1"));
            if (bVideoStop) {
                Slog.i(TAG, "persist.sys.bootvideo.exit is 1");
                break;
            }
            Slog.i(TAG, "persist.sys.bootvideo.exit is 0");
            try {
                Thread.sleep(100);
            } catch (InterruptedException e) {
                Slog.e(TAG, "Sleep failed");
