...
#ifdef SKIA_ACCELERATION_ENABLE
        SkHSBlitter * blitter = SkHSBlitter::getInstance();
        blitter->setSurfaceAddress(outBuffer.bits, outBuffer.phyaddr); //skia acceleration
#endif
    } else {
...
#ifdef SKIA_ACCELERATION_ENABLE
    SkHSBlitter * blitter = SkHSBlitter::getInstance();
    blitter->setSurfaceAddress(NULL, 0); //skia acceleration
#endif
    // unlock surface
...
static jintArray getVirtScreenSize(JNIEnv* env, jobject obj) {
    ...
    env->SetIntArrayRegion(array, 0, 1, &w);
    env->SetIntArrayRegion(array, 1, 1, &h);
    return array;
