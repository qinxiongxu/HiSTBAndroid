...
public abstract class Window {
    ...
    public void setWindowManager(WindowManager wm, IBinder appToken, String appName,
        ...
        try
        {
            if (mAppName != null)
            {
                String[] s = new String(mAppName).split("/");
                if((s != null) && (s.length > 1))
                {
                    String hwskiaProcs = SystemProperties.get("persist.sys.hwskia.procs", "");
                    if(hwskiaProcs.contains(s[0])) //proc in hwskia procs list
                    {
