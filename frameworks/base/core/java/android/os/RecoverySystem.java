...
public class RecoverySystem {
    ...
    private static Boolean FOR_SHYD = true;
    /**
    ...
    public static void installPackage(Context context, File packageFile)
        ...
        int index = filename.lastIndexOf('/') + 1;//wanghuanlai modified
        Log.i(TAG,"index : " + index);
        String filepath1 = filename.substring(7, index);
        if (filename.startsWith("/cache/")) {
            if(FOR_SHYD){
                filename = "CACHE:" + filepath1 + filename.substring(index);
            }
        }
        else {
	...
	public static void installPackageWithSdcard(Context context, File packageFile, String mntPoint)
        throws IOException {
        String filename = packageFile.getPath();
        Log.w(TAG, "!!! REBOOTING TO INSTALL " + filename + " !!!");
        String arg = "--update_package=" + filename +
            "\n--locale=" + Locale.getDefault().toString();
        bootCommandWithSdcard(context, arg, mntPoint);
    }
    /**
    ...
    public static void rebootWipeUserData(Context context) throws IOException {
        ...
        context.sendBroadcast(new Intent("com.hisilicon.intent.action.RESET_BASEPARAM"));
    ...
    public static void rebootWipeUserData_Iptv(Context context ,String Path) throws IOException {
        // Reset display params
        context.sendBroadcast(new Intent("com.hisilicon.intent.action.RESET_BASEPARAM"));

        final ConditionVariable condition = new ConditionVariable();
        Log.i("RecoverySystem","enter rebootWipeUserData_Iptv");
                Log.i(TAG, "rebootWipeUserData write 'reset' to misc");
        ...
        int index = Path.lastIndexOf('/') + 1;//wanghuanlai modified
        String FileName = Path.substring(index);
        Log.i(TAG, "PATH = " + Path );
        Log.i(TAG, "FileName = " + FileName );
        HiSysManager hisys = new HiSysManager();
        hisys.recoveryIPTV(Path,FileName);
//        SocketClient socketClient_iptv = new SocketClient();
//        socketClient_iptv.writeMess("system mkdir -p /private/iptv ; echo " + Path + " > /private/iptv/Path; cp -rfp " + Path + " /private/iptv" + " ;echo " + FileName + " > /private/iptv/FileName");
//        socketClient_iptv.readNetResponseSync();

        bootCommand(context, "--wipe_data\n--locale=" + Locale.getDefault().toString());
    }

    /**
    * Reboot into the recovery system to save the user data.
    */
       public static int userDataRestore_Iptv() {
       HiSysManager hisys = new HiSysManager();
       return hisys.userDataRestoreIptv();
       }
...
*/
       ...
       try {
               ...
               command.write(test.getBytes());
               command.getFD().sync();
               command.close();
       } catch (IOException e) {
               e.printStackTrace();
       }
       Log.e(TAG, "write /cache/recovery/commmand in bootCommand");
       HiSysManager hisys = new HiSysManager();
       hisys.reset();
       Log.i(TAG, "enter socketClient.writeMess reset");
	...
	private static void bootCommandWithSdcard(Context context, String arg, String mntPoint) throws IOException {
        RECOVERY_DIR.mkdirs();  // In case we need it
        COMMAND_FILE.delete();  // In case it's not writable
        LOG_FILE.delete();
/*
...
*/
       ...
       Log.e(TAG, "write /cache/recovery/commmand");
       HiSysManager hisys = new HiSysManager();
       hisys.reset();
       Log.i(TAG, "enter socketClient.writeMess reset sdcard");
        // Having written the command file, go ahead and reboot
        PowerManager pm = (PowerManager) context.getSystemService(Context.POWER_SERVICE);
        pm.reboot("recovery");
