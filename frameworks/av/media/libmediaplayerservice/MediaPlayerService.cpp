...
MediaPlayerService::MediaPlayerService()
{
    ...
    HiSysManagerClient().sendMediaStart();
}
...
#ifdef HIMEDIARECORDER_ENABLE
HI_HANDLE MediaPlayerService::getAVPlayHandle()
{
    HI_HANDLE hAVPlayHandle = HI_INVALID_HANDLE;
    int ret = -1;

    for (int i = 0, n = mClients.size(); i < n; ++i)
    {
        sp<Client> c = mClients[i].promote();
        if ((c != 0) && (c->mPlayerType == HIMEDIA_PLAYER))
        {
            ...
            if (p != 0)
            {
                Parcel request;
                Parcel reply;
                request.writeInt32(CMD_GET_AVPLAY_HANDLE);
                request.setDataPosition(0);
                p->invoke(request, &reply);
                reply.setDataPosition(0);
                ret = reply.readInt32();
                if (ret == HI_SUCCESS)
                {
                    hAVPlayHandle = reply.readInt32();
                    ALOGV("[%4d]@MediaplayerService.cpp i=%d hAVPlay_handle=0x%08X", __LINE__, i, hAVPlayHandle);
                    break; //got it, return
                }
            }
        }
    ...
    ALOGV("[%4d]@MediaplayerService.cpp finally hAVPlay_handle=0x%08X", __LINE__, hAVPlayHandle);
    return hAVPlayHandle;
}
#endif
...
//add for destroy static avplayer
int MediaPlayerService::destoryAVPlayer(int hanlde) {
    ALOGI("MediaPlayerService::destoryAVPlayer hanlde = %d", hanlde);
    HiMediaPlayer::destroyStaticAVPlayer();
    return 0;
}
...
void MediaPlayerService::Client::getEffectiveUrl(const char *oriUrl, String8 *destUrl)
{
    if (destUrl == NULL)
    {
        ALOGE("getEffectiveUrl parameter error.");
        return;
    }
    ...
    if (curl == NULL)
    {
        ALOGE("curl_easy_init failure.");
        return;
    }

    curl_easy_setopt(curl, CURLOPT_URL, oriUrl);
    ...
    destUrl->setTo(eff_url);

    if (strcmp(destUrl->string(), oriUrl) != 0)
    {
        ALOGV("effective Url(%s)", destUrl->string());
    }
        ...
        const char *url, const KeyedVector<String8, String8> *headers)
{
    ...
    if (strncmp(url, "content://", 10) == 0) {
        ...
        mURL.clear();
        //getEffectiveUrl(url, &mURL);
        if (mURL.isEmpty())
        {
            mURL.setTo(url);
        }

        player_type playerType = MediaPlayerFactory::getPlayerType(this, mURL.string());
        sp<MediaPlayerBase> p = setDataSource_pre(playerType);
        if (p == NULL) {
            return NO_INIT;
        }

        setDataSource_post(p, p->setDataSource(mURL.string(), headers));
        ...
        const sp<IGraphicBufferProducer>& bufferProducer)
{
    ...
    if (bufferProducer != NULL) {
        ...
        if (err != OK) {
            ...
            if (property_get("ro.product.target", value, "ott")) {
                if (strcmp(value, "shcmcc") == 0) {
                    disconnectNativeWindow();
                    return OK;
                } else {
                    reset();
                                            ...
                                            Parcel *reply)
{
    ...
    switch (InvokeCmd)
    {
        case CMD_INPUT_AUDIO_CODEC_LIB:
        {
            int Ret = HI_SYS_Init();
            if (0 != Ret)
            {
                ALOGE("ERR: input audio lib HI_SYS_Init fail! ");
                return UNKNOWN_ERROR;
            }
            String16 strAudioPath = request.readString16();
            char hichars[2048];
            utf16_to_utf8(strAudioPath.string(), strAudioPath.size(), hichars);
            Ret = HI_UNF_AVPLAY_RegisterAcodecLib(hichars);
            if (0 != Ret)
            {
                ALOGE("input audio lib,register audio lib fail! ");
                return UNKNOWN_ERROR;
            }
            return NO_ERROR;
        }
        case CMD_CONNECT_ID:
            char soft_detector[PROPERTY_VALUE_MAX];
            if (property_get("persist.sys.softdetector.enable", soft_detector, "false") && strcmp("true", soft_detector)== 0)
            {
                if (NULL != reply) {
                    reply->writeInt32(mConnId);
                }
                return NO_ERROR;
            }
        default:
...
status_t MediaPlayerService::Client::setVolume(float leftVolume, float rightVolume)
{
    ...
    sp<MediaPlayerBase> p = getPlayer();
    {
      ...
      if (p != 0 && p->hardwareOutput()) {
          ...
          if (p != 0 && mPlayerType == HIMEDIA_PLAYER) {
              ...
              request.setDataPosition(0);
              request.writeInt32(CMD_SET_TRACK_VOLUME);
              request.writeInt32(left);
              request.writeInt32(right);
              request.setDataPosition(0);
              p->invoke(request, &reply);
          }
          return NO_ERROR;
...
void MediaPlayerService::Client::switchMediaPlayer(player_type playerType)
{
    ALOGV("switchMediaPlayer, player type = %d", playerType);
    sp<MediaPlayerBase> p = setDataSource_pre(playerType);
    if (p == NULL) {
        return;
    }

    setDataSource_post(p, p->setDataSource(mURL.string(), &mHeaders));

    mConnectedWindowBinder = NULL;
    disconnectNativeWindow();
...
/*
    if (MEDIA_ERROR == msg &&
        MEDIA_ERROR_UNKNOWN == ext1) {
        if (client->mPlayerType == STAGEFRIGHT_PLAYER &&
            client->mIsPreparing) {
            client->switchMediaPlayer(NU_PLAYER);
            return;
        }
    }
*/
                                       ...
                                       const sp<IMemoryHeap>& heap, size_t *pSize)
{
    ...
    //soundpool use stagefright
    if(playerType == HIMEDIA_PLAYER)
    {
        playerType = STAGEFRIGHT_PLAYER;
        ALOGV("MediaPlayerService::decode(url) player type from HIMEDIA_PLAYER(%d) --> STAGEFRIGHT_PLAYER(%d)", HIMEDIA_PLAYER, playerType);
    }
    ALOGV("player type = %d", playerType);
                                       ...
                                       const sp<IMemoryHeap>& heap, size_t *pSize)
{
    ...
    {
        playerType = STAGEFRIGHT_PLAYER;
        ALOGV("MediaPlayerService::decode(fd)  player type from HIMEDIA_PLAYER(%d) --> STAGEFRIGHT_PLAYER(%d)", HIMEDIA_PLAYER, playerType);
    }
    ALOGV("player type = %d", playerType);
