...
namespace android {
static bool sEnableLogcatV = false;
#ifdef ALOGV
#undef ALOGV
#endif
#define ALOGV(...) ALOGV_IF(sEnableLogcatV, __VA_ARGS__)
...
private:
    ...
    OMX_U32 getRefreshRate();
    HI_U32 calcRepeatCnt();
    void writeMetadata(HI_U32 repeatCnt, BufferInfo *info);
...
ACodec::ACodec()
    ...
    if (property_get("service.media.codec.logcat", value, NULL)
        && (!strcasecmp("true", value))) {
        sEnableLogcatV = true;
    }
    if (property_get("service.media.codec.savees", value, NULL)
        && (!strcasecmp("true", value))) {
...
ACodec::~ACodec() {
    ...
    if (mBufferStats.size() > 0) {
        ALOGI("Incomplete stats buffers:%d", mBufferStats.size());
        mBufferStats.clear();
    }
    if (mFrcHandle != 0) {
        ALOGI("FrcDestroy(%#x)", mFrcHandle);
        FrcDestroy(mFrcHandle);
        mFrcHandle = 0;
    }
}
...
status_t ACodec::allocateBuffersOnPort(OMX_U32 portIndex) {
    ...
    if (mNativeWindow != NULL && portIndex == kPortIndexOutput) {
        ...
        if (err == OK) {
            ALOGI("[%s] Allocating %lu buffers of size %lu on %s port",
...
status_t ACodec::configureOutputBuffersFromNativeWindow(
    ...
    if (usage & GRALLOC_USAGE_HISI_VDP) {
        char value[PROPERTY_VALUE_MAX];
        if (property_get("service.media.codec.overlay", value, "false")
            && (!strcasecmp("false", value))) {
            ALOGI("Output will queue to original graphics.");
            usage &= ~GRALLOC_USAGE_HISI_VDP;
        } else if (mFastOutput) {
            usage |= GRALLOC_USAGE_HISI_FASTOUTPUT;
        } else {
            if (mFrcHandle > 0) {
                    ALOGI("FRC(%#x) has been created, use it again", mFrcHandle);
            } else {
                HI_S32 ret = FrcCreate(&mFrcHandle);
                if (ret != HI_SUCCESS) {
                    ALOGE("FRC created error");
...
status_t ACodec::allocateOutputBuffersFromNativeWindow() {
    ...
    ALOGI("[%s] Allocating %lu buffers from a native window of size %lu on "
         "output port",
    ...
    for (OMX_U32 i = 0; i < bufferCount; i++) {
        ...
        private_handle_t *p_private_handle = (private_handle_t *)bufferHandle;
        ALOGV("[%s] GraphicBuffer, index:%d, p_private_handle: %#x, ion_phy_addr: %#x, ion_metadata_phy_addr: %#x(%d bytes)",
            mComponentName.c_str(), i, p_private_handle, p_private_handle != NULL ? p_private_handle->ion_phy_addr : 0,
            p_private_handle != NULL ? p_private_handle->ion_metadata_phy_addr : 0,
            p_private_handle != NULL ? p_private_handle->ion_metadata_size : 0);
#endif
        BufferInfo info;
        info.mStatus = BufferInfo::OWNED_BY_US;
...
ACodec::BufferInfo *ACodec::dequeueBufferFromNativeWindow() {
    ...
    if (mStats) {
        int64_t escape = ALooper::GetNowUs() - now;
        ALOGV("dequeueBuffer escape: %lld", escape);
    }
...
status_t ACodec::configureCodec(
    ...
    if ((fastOutput != 0) || (QuickOutputMode != 0)) {
        ...
        if (err == OK) {
            ...
            fastOutput = 1;
            err = mOMX->setParameter(
                mNode, index, &fastOutput, sizeof(int32_t));
        } else {
...
status_t ACodec::setupFFmpegAudioFormat(const sp<AMessage> &msg,const char *mime){
    ALOGE("enter %s()",__FUNCTION__);
    OMX_AUDIO_PARAM_FFMPEGCONFIG profile;
    InitOMXParams(&profile);
    profile.nPortIndex = kPortIndexInput;

    status_t err = mOMX->getParameter(
            mNode, OMX_IndexConfigAudioFfmpeg, &profile, sizeof(profile));
    CHECK_EQ(err, (status_t)OK);

    if (0 == strcmp(mime,MEDIA_MIMETYPE_AUDIO_MPEG_LAYER_II))
    {
       profile.nAvCodecCtxID = 0x15000; // CODEC_ID_MP2 = 0x15000
    }
    ...
    if (err != OK) {
       ALOGE("setParameter('OMX_IndexConfigAudioFfmpeg') failed (err = %d)", err);
       return err;
    }
    ...
    switch (mode) {
        ...
        case RESUBMIT_BUFFERS:
        {
            if (buffer != NULL && !mCodec->mPortEOS[kPortIndexInput]) {
                ...
                if (mCodec->mSaveES) {
                    if (mCodec->mIsVideo) {
                        if (mCodec->mVideoFile == NULL) {
                            char *path = "/mnt/sdcard/acodecVideo.es";
                            mCodec->mVideoFile = fopen(path, "wb");
                            if (mCodec->mVideoFile == NULL) {
                                ALOGE("File %s created error, your APP should add WRITE_EXTERNAL_STORAGE permission", path);
                                mCodec->mSaveES = false;
                            }
                        }
                        if (mCodec->mVideoFile != NULL) {
                            ALOGI("Save %ld bytes data to /sdcard/acodecVideo.es", buffer->size());
                            fwrite((char *)(info->mData->data()), 1, buffer->size(), mCodec->mVideoFile);
                        }
                    } else {
                        if (mCodec->mAudioFile == NULL) {
                            char *path = "/mnt/sdcard/acodecAudio.es";
                            mCodec->mAudioFile = fopen(path, "wb");
                            if (mCodec->mAudioFile == NULL) {
                                ALOGE("File %s created error, your APP should add WRITE_EXTERNAL_STORAGE permission", path);
                                mCodec->mSaveES = false;
                            }
                        ...
                        if (mCodec->mAudioFile != NULL) {
                            ALOGI("Save %ld bytes data to /sdcard/acodecAudio.es", buffer->size());
                            fwrite((char *)(info->mData->data()), 1, buffer->size(), mCodec->mAudioFile);
                        }
                    }
                }
                ...
                if (mCodec->mStats) {
                    int64_t QIBTimeMs = -1;
                    if (buffer->meta()->findInt64("qibtime", &QIBTimeMs)) {
                        ACodec::BufferStats stats;
                        int64_t counter = 0;
                        CHECK(buffer->meta()->findInt64("bufferCounter", &counter));
                        stats.mBufferCounter = counter;
                        stats.mQIBTimeMs = QIBTimeMs;
                        int64_t nowUs = ALooper::GetNowUs();
                        stats.mETBTimeMs = nowUs / 1000ll;
                        stats.mFBDTimeMs = -1ll;
                        mCodec->mBufferStats.add(timeUs, stats);
                    }
                }
                ...
                if (mCodec->mSaveES) {
                    if (mCodec->mVideoFile != NULL) {
                        fclose(mCodec->mVideoFile);
                        mCodec->mVideoFile = NULL;
                    }
                    if (mCodec->mAudioFile != NULL) {
                        fclose(mCodec->mAudioFile);
                        mCodec->mAudioFile = NULL;
                    }
                }
                mCodec->mPortEOS[kPortIndexInput] = true;
                mCodec->mInputEOSResult = err;
...
OMX_U32 ACodec::BaseState::getRefreshRate() {
    OMX_U32 u32DispRate = 30000;
    OMX_U32 s32DispFmt = HI_UNF_ENC_FMT_BUTT;
    DisplayClient dispClient;
    s32DispFmt = dispClient.GetEncFmt();
    switch (s32DispFmt) {
    case HI_UNF_ENC_FMT_1080P_60:     /**<1080p 60 Hz*/
    ...
    case HI_UNF_ENC_FMT_VESA_1680X1050_60_RB:
    case HI_UNF_ENC_FMT_VESA_1920X1080_60:
    case HI_UNF_ENC_FMT_VESA_1920X1200_60:
    case HI_UNF_ENC_FMT_VESA_1920X1440_60:
    ...
    case HI_UNF_ENC_FMT_1080P_29_97:
        u32DispRate = 29970;
        break;
    case HI_UNF_ENC_FMT_PAL:              /* B D G H I PAL */
    case HI_UNF_ENC_FMT_PAL_N:            /* (N)PAL        */
    case HI_UNF_ENC_FMT_PAL_Nc:           /* (Nc)PAL       */
    case HI_UNF_ENC_FMT_NTSC_PAL_M:       /* (M)PAL        */
        u32DispRate = 50000;
        break;
    case HI_UNF_ENC_FMT_NTSC:             /* (M)NTSC       */
    case HI_UNF_ENC_FMT_NTSC_J:           /* NTSC-J        */
        u32DispRate = 60000;
        break;
    case HI_UNF_ENC_FMT_SECAM_SIN:        /**< SECAM_SIN*/
    case HI_UNF_ENC_FMT_SECAM_COS:        /**< SECAM_COS*/
    default:
        u32DispRate = 30000;
        break;
    }
...
HI_U32 ACodec::BaseState:: calcRepeatCnt() {
    HI_U32 repeatCnt = 1;
    OMX_INDEXTYPE index;
    status_t err = mCodec->mOMX->getExtensionIndex(
                    mCodec->mNode,
    ...
    OMX_HISI_PARAM_CHANNELATTRIBUTES params;
    memset(&params, 0, sizeof(params));
    err = mCodec->mOMX->getParameter(mCodec->mNode, index,&params, sizeof(params));
    if (err != OK) {
        return repeatCnt;
    }
    OMX_U32 refreshRate = getRefreshRate();
    ALOGV("[%s] refreshRate: %d, frameRate: %d",mCodec->mComponentName.c_str(), refreshRate, params.xFramerate);
    HI_S32 ret = FrcCalculate(mCodec->mFrcHandle, params.xFramerate, refreshRate, &repeatCnt);
    if (ret != HI_SUCCESS) {
        ALOGW("[%s] Calculate FRC fail, use default value 1",mCodec->mComponentName.c_str());
        repeatCnt = 1;
    }
    return repeatCnt;
}
void ACodec::BaseState:: writeMetadata(HI_U32 repeatCnt, BufferInfo *info) {
    private_handle_t *p_handle = (private_handle_t *)(info->mGraphicBuffer->handle);
    private_metadata_t* metadata_ptr = (private_metadata_t*)mmap(NULL, p_handle->ion_metadata_size,
            PROT_READ | PROT_WRITE, MAP_SHARED, p_handle->metadata_fd, 0);
    if (MAP_FAILED == metadata_ptr) {
        ALOGE("[%s] mmap metadata buffer failed.",mCodec->mComponentName.c_str());
        return;
    }
    metadata_ptr->display_cnt = repeatCnt;
    if (0 != munmap((void *)metadata_ptr, p_handle->ion_metadata_size)) {
        ALOGE("[%s] Failed to munmap metadata buffer", mCodec->mComponentName.c_str());
        return;
    }
}
...
bool ACodec::BaseState::onOMXFillBufferDone(
    ...
    if (mCodec->mStats) {
        ssize_t statsIdx = mCodec->mBufferStats.indexOfKey(timeUs);
        if (statsIdx >= 0) {
            ACodec::BufferStats *stats = &mCodec->mBufferStats.editValueAt(statsIdx);
            int64_t nowUs = ALooper::GetNowUs();
            stats->mFBDTimeMs = nowUs / 1000ll;
        }
    }
    BufferInfo *info =
...
void ACodec::BaseState::onOutputBufferDrained(const sp<AMessage> &msg) {
    ...
    if (mCodec->mStats) {
        int64_t bufferPts = -1;
        CHECK(info->mData->meta()->findInt64("timeUs", &bufferPts));
        ssize_t statsIdx = mCodec->mBufferStats.indexOfKey(bufferPts);
        if (statsIdx >= 0) {
            ACodec::BufferStats *stats = &mCodec->mBufferStats.editValueAt(statsIdx);
            int64_t DOBTimeMs = -1;
            int64_t ROBTimeMs = -1;
            if (msg->findInt64("dobtime", &DOBTimeMs)
                && msg->findInt64("robtime", &ROBTimeMs)) {
                int64_t nowUs = ALooper::GetNowUs();
                int64_t nowMs = nowUs / 1000ll;
                int64_t decodeDelay = nowMs - stats->mQIBTimeMs;
                int64_t app2omx = stats->mETBTimeMs - stats->mQIBTimeMs;
                int64_t omxHold = stats->mFBDTimeMs - stats->mETBTimeMs;
                int64_t omx2app = DOBTimeMs - stats->mFBDTimeMs;
                int64_t appHold = ROBTimeMs - DOBTimeMs;
                int64_t app2Surface = nowMs - ROBTimeMs;
                private_handle_t *p_handle = (private_handle_t *)(info->mGraphicBuffer->handle);
                ALOGI("Idx:%lld, pts:%lld, total:%lld, app2omx:%lld, omxHold:%lld, "
                    "omx2app:%lld, appHold:%lld, app2Surface:%lld, timeUs:%lld, ion_phy_addr:%#x",
                    stats->mBufferCounter, bufferPts, decodeDelay, app2omx, omxHold,
                    omx2app, appHold, app2Surface, nowUs, p_handle->ion_phy_addr);
            }
            mCodec->mBufferStats.removeItemsAt(statsIdx);
            stats = NULL;
        }
    }
    int32_t render;
    if (mCodec->mNativeWindow != NULL
        ...
        bool drop = false;
        if (mCodec->mFrcHandle != 0) {
            HI_U32 repeatCnt = calcRepeatCnt();
            if (repeatCnt > 0) {
                writeMetadata(repeatCnt, info);
            } else {
                ALOGI("drop frame");
                info->mStatus = BufferInfo::OWNED_BY_US;
                drop = true;
    ...
    } else if (mCodec->mLocalNativeWindow != NULL
               && msg->findInt32("render", &render) && render != 0) {
        OMX_PARAM_PORTDEFINITIONTYPE def;
        InitOMXParams(&def);
        def.nPortIndex = kPortIndexOutput;
        ...
        OMX_VIDEO_PORTDEFINITIONTYPE *video_def = &def.format.video;
        //ALOGE("====w %d h %d c %d=====", video_def->nFrameWidth, video_def->nFrameHeight, video_def->eColorFormat);

        if(NULL == mCodec->mRender) {
            sp<MetaData> meta = new MetaData;
            meta->setInt32(kKeyWidth, video_def->nFrameWidth);
            meta->setInt32(kKeyHeight, video_def->nFrameHeight);
            meta->setInt32(kKeyColorFormat, video_def->eColorFormat);

            mCodec->mRender = new SoftwareRenderer(mCodec->mLocalNativeWindow, meta);
        }
        mCodec->mRender->render((const uint8_t *)info->mData->data(), info->mData->size(), NULL);
        info->mStatus = BufferInfo::OWNED_BY_US;
    } else {
        info->mStatus = BufferInfo::OWNED_BY_US;
    }
...
bool ACodec::ExecutingState::onOMXEvent(
    ...
    switch (event) {
        ...
        {
            ALOGI("[%s] OMX_EventPortSettingsChanged 0x%08lx",
    ...
    switch (event) {
        case OMX_EventCmdComplete:
        {
            if (data1 == (OMX_U32)OMX_CommandPortDisable) {
                ...
                ALOGI("[%s] Output port now disabled.",
                        mCodec->mComponentName.c_str());
                ...
                ALOGI("[%s] Output port now reenabled.",
                        mCodec->mComponentName.c_str());
...
bool ACodec::FlushingState::onOMXEvent(
        OMX_EVENTTYPE event, OMX_U32 data1, OMX_U32 data2) {
    ALOGV("[%s] FlushingState onOMXEvent(%d,%ld,%#x)",
            mCodec->mComponentName.c_str(), event, data1, data2);
