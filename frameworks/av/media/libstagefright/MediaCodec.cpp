...
status_t MediaCodec::configure(
    ...
    if(flags == 2) {
        sysclient.setUIAsyncCompose(true);
        ...
        device = String8("/sys/devices/system/cpu/cpu0/cpufreq/scaling_setspeed");
        value = String8("1000000");
        sysclient.adjustDevState(device,value);
    } else {
        sysclient.setUIAsyncCompose(false);
    }
...
status_t MediaCodec::release() {
    ...
    String8 device = String8("/sys/devices/system/cpu/cpu0/cpufreq/scaling_governor");
    String8 value = String8("interactive");
    sysclient.adjustDevState(device,value);
...
status_t MediaCodec::renderOutputBufferAndRelease(size_t index) {
    ...
    if (mStats) {
        int64_t nowUs = ALooper::GetNowUs();
        msg->setInt64("robtime", nowUs / 1000ll);
    }
...
status_t MediaCodec::onReleaseOutputBuffer(const sp<AMessage> &msg) {
    ...
    if (mStats) {
        ssize_t statsIdx = mBufferStats.indexOfKey(index);
        if (statsIdx >= 0) {
            BufferStats *stats = &mBufferStats.editValueAt(statsIdx);
            int64_t bufferPts = -1;
            CHECK(info->mData->meta()->findInt64("timeUs", &bufferPts));
            if (bufferPts == stats->mPtsUs) {
                CHECK(msg->findInt64("robtime", &(stats->mROBTimeMs)));
                info->mNotify->setInt64("dobtime", stats->mDOBTimeMs);
                info->mNotify->setInt64("robtime", stats->mROBTimeMs);
            } else {
                ALOGI("stats error, PTS(%lld) not matched(%lld)", bufferPts, stats->mPtsUs);
            }
            mBufferStats.removeItemsAt(statsIdx);
            stats = NULL;
        }
    }
