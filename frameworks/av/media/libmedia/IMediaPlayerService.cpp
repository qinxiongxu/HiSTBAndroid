...
public:
    ...
    virtual int destoryAVPlayer(int hanlde) {
        Parcel data, reply;
        data.writeInt32(hanlde);
        remote()->transact(DESTROY_AVPLAYER, data, &reply);
        return reply.readInt32();
    }
};
    ...
    uint32_t code, const Parcel& data, Parcel* reply, uint32_t flags)
{
    switch (code) {
        ...
        case DESTROY_AVPLAYER: {
            int32_t avhandle = data.readInt32();
            int32_t result = destoryAVPlayer(avhandle);
