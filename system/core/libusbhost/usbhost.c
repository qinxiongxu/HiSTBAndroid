...
char* usb_device_get_string(struct usb_device *device, int id)
{
    ...
    //for MT7601 and MT7603 WiFi device,max length of usb control message is 0xff(255)
    if (((usb_device_get_vendor_id(device) == 0x0e8d && usb_device_get_product_id(device) == 0x7603)
        || (usb_device_get_vendor_id(device) == 0x148f && usb_device_get_product_id(device) == 0x7601))
        && (sizeof(languages) >= 0x100))
        result = usb_device_control_transfer(device,
                USB_DIR_IN|USB_TYPE_STANDARD|USB_RECIP_DEVICE, USB_REQ_GET_DESCRIPTOR,
                (USB_DT_STRING << 8) | 0, 0, languages, 0xff, 0);
    ...
    for (i = 1; i <= languageCount; i++) {
        ...
        if (((usb_device_get_vendor_id(device) == 0x0e8d && usb_device_get_product_id(device) == 0x7603)
            ...
            result = usb_device_control_transfer(device,
                    USB_DIR_IN|USB_TYPE_STANDARD|USB_RECIP_DEVICE, USB_REQ_GET_DESCRIPTOR,
                    (USB_DT_STRING << 8) | id, languages[i], buffer, 0xff, 0);
