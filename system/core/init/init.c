    ...
    if (fp !=NULL ) {
        if (fgets(buffer,1024,fp) != NULL) {
            if (strstr(buffer,"hinand")) {
                ...
                ERROR("error!!, Can't get Flash Type in Cmdline %s\n",buffer);
            }
        }
        fclose(fp);
    }
...
static int check_ram()
{
    ...
    if (fp !=NULL ) {
        if (fgets(buffer,1024,fp) != NULL) {
            if (strstr(buffer,"mem=512M")) {
                property_set("ro.config.low_ram", "true");
                property_set("ro.config.tde_compose", "false");
                property_set("ro.product.mem.size", "512m");
                property_set("dalvik.vm.jit.codecachesize", "0");
                property_set("persist.sys.screen_density", "160");
                property_set("persist.sys.lmkMinFreeKBytes", "16384");
                property_set("persist.sys.lmkExtraFreeKBytes", "636");
            } else if (strstr(buffer,"mem=768M")) {
                property_set("ro.config.low_ram", "true");
                property_set("ro.config.tde_compose", "false");
                property_set("ro.product.mem.size", "768m");
                property_set("dalvik.vm.jit.codecachesize", "0");
                property_set("persist.sys.screen_density", "160");
                property_set("persist.sys.lmkMinFreeKBytes", "16384");
                property_set("persist.sys.lmkExtraFreeKBytes", "636");
            } else if (strstr(buffer, "mem=1G")) {
                property_set("ro.product.mem.size", "1g");
                if ((property_get("ro.config.low_ram", tmp) != 0) && (strcmp(tmp, "true") == 0)) {
                    property_set("dalvik.vm.jit.codecachesize", "0");
                    property_set("persist.sys.lmkMinFreeKBytes", "16384");
                    property_set("persist.sys.lmkExtraFreeKBytes", "636");
                }
            } else if (strstr(buffer, "mem=2G")) {
                property_set("ro.product.mem.size", "2g");
            } else {
                property_set("ro.product.mem.size", "unknown");
                ERROR("not 512M, 768M, 1G or 2G version\n");
            }
        }
        fclose(fp);
    }
...
int main(int argc, char **argv)
{
    ...
    mount("sysfs", "/sys", "sysfs", 0, NULL);
        /* mkdir usb adb device and mount it */
    ...
    property_get("persist.sys.boot.optimize", value);
    ...
    if (!is_charger) {
        ...
        action_for_each_trigger("fuse-sdcard", action_add_queue_tail);
#endif
    ...
    property_set("ro.mount_tmp_databae.enable","1");
#endif
