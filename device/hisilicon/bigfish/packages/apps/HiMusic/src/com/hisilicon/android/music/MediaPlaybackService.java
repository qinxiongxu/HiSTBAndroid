...
public class MediaPlaybackService extends Service {
                ...
                public void onReceive(Context context, Intent intent) {
                    ...
                    if (action.equals(Intent.ACTION_MEDIA_EJECT)) {
                        ...
                        if (uri.getScheme().equals("file")) {
                            ...
                            Log.i("MediaPlaybackService", "path:" + path);
                            Log.i("MediaPlaybackService", "mFilePathToPlay:"
                                    + mFilePathToPlay);
                            if (path.equals(mFilePathToPlay))
                                if (isPlaying()) {
                                    Log.i("MediaPlaybackService",
                                            "File have be delete:" + path);
                                    next(true);
                                }
                        }
                    }
    ...
    private void openCurrent() {
        synchronized (this) {
            ...
            if (mCursor != null) {
                ...
                try {
                    mFilePathToPlay = mCursor.getString(mCursor
    ...
    public void open(String path) {
        synchronized (this) {
            ...
            //
            // try {
            // mCursor = resolver.query(uri, mCursorCols, where, selectionArgs,
            // null);
            // if (mCursor != null) {
    ...
    public class startLyricThread implements Runnable {
        ...
        public void run() {
            ...
            MediaPlaybackActivity.lyricView.setmLyric(weakRf_lyric.get());
            ...
            MediaPlaybackActivity.lyricView.setCurrentTextSize(20);
            MediaPlaybackActivity.lyricView.setLrcTextSize(20);
            MediaPlaybackActivity.lyricView.setTexttypeface(Typeface.SERIF);
            MediaPlaybackActivity.lyricView
                    .setBrackgroundcolor(Color.TRANSPARENT);
            MediaPlaybackActivity.lyricView.setTextHeight(50);
    ...
    class UIUpdateThread implements Runnable {
        ...
        public void run() {
            while (isPlaying()) {
                try
                {
                    MediaPlaybackActivity.lyricView.updateIndex(position());
    ...
    Runnable mUpdateResults = new Runnable() {
        public void run() {
            MediaPlaybackActivity.lyricView.invalidate();
        }
    };
    ...
    public void prev() {
        synchronized (this) {
            if (null == MediaPlaybackActivity.mediaFileList)
                return;
            if (mShuffleMode == SHUFFLE_NORMAL) {
                String filePath = MediaPlaybackActivity.mediaFileList
                        .getRandomMusicInfo(null).getPath();
                if (null == filePath)
                    return;
                open(filePath);
                play();
            ...
            /*
             * if (mShuffleMode == SHUFFLE_NORMAL) { // go to previously-played
             * track and remove it from the history int histsize =
             * mHistory.size(); if (histsize == 0) { // prev is a no-op return;
             * } Integer pos = mHistory.remove(histsize - 1); mPlayPos =
             * pos.intValue(); } else { if (mPlayPos > 0) { mPlayPos--; } else {
             * mPlayPos = mPlayListLen - 1; } } saveBookmarkIfNeeded();
             * stop(false); openCurrent(); play(); notifyChange(META_CHANGED);
             */
        }
    }
    ...
    public void next(boolean force) {
        synchronized (this) {
            ...
            if (mShuffleMode == SHUFFLE_NORMAL) {
				if(MediaPlaybackActivity.mediaFileList.getRandomMusicInfo(null)==null)
				{
					mIsSupposedToBePlaying = false;
                    notifyChange(PLAYSTATE_CHANGED);
					return;
			    ...
			    MusicModel model = MediaPlaybackActivity.mediaFileList.getNextMusicInfo(null);
				if(model==null)
				{
					mIsSupposedToBePlaying = false;
                    notifyChange(PLAYSTATE_CHANGED);
			    ...
			    MusicModel model = MediaPlaybackActivity.mediaFileList.getNextMusicInfo_NoCycle(null);
				if(model==null)
				{
					mIsSupposedToBePlaying = false;
                    notifyChange(PLAYSTATE_CHANGED);
            ...
            /*
             * if (mPlayListLen <= 0) { Log.d(LOGTAG, "No play queue"); return;
             * }
             ...
             * doAutoShuffleUpdate(); mPlayPos++; } else { if (mPlayPos >=
             * mPlayListLen - 1) { // we're at the end of the list if
             * (mRepeatMode == REPEAT_NONE && !force) { // all done
             * gotoIdleState(); mIsSupposedToBePlaying = false;
             * notifyChange(PLAYSTATE_CHANGED); return; } else if (mRepeatMode
             * == REPEAT_ALL || force) { mPlayPos = 0; } } else { mPlayPos++; }
             * } saveBookmarkIfNeeded(); stop(false); openCurrent(); play();
             * notifyChange(META_CHANGED);
             */
        }
    }
    ...
    private class MultiPlayer {
        ...
        public void setDataSource(String path) {
            ...
            mMediaPlayer.setOnCompletionListener(listener2);
            mMediaPlayer.setOnErrorListener(errorListener2);
            Intent i = new Intent(
        ...
        MediaPlayer.OnErrorListener errorListener2 = new MediaPlayer.OnErrorListener() {
