...
import com.hisilicon.android.gallery3d.app.GalleryActivity;
...
public class PhotoView extends GLView {
    ...
    public interface PhotoTapListener {
        ...
        // Clear AlbumView focus on slider
        public void clearAlbumFocus();

        // draw AlbumView focus on slider
    ...
    // remote control key process
    private final MyCustomKeyListener mCustomKeyListener;
    ...
    private static final int GET_NAIL_TIMEOUT_COUNT = 60;
    ...
    public PhotoView(GalleryActivity activity) {
        ...
        mCustomKeyListener = new MyCustomKeyListener();
    ...
    protected boolean onKeyDown(GLRootView glRootView, KeyEvent event) {
        return mCustomKeyListener.onKey(glRootView, event.getKeyCode(), event);
    }
    ...
    protected void render(GLCanvas canvas) {
        ...
        if (mLoadingState == LOADING_TIMEOUT) {
            // calc time out times
            // if the number is 20 then change to fail
//            mCount++;
//            if (mCount > GET_NAIL_TIMEOUT_COUNT) {
//                mCount = 0;
//                mLoadingState = LOADING_FAIL;
//            }
            // by z00120637 end
    ...
    private class MyGestureListener extends
        ...
        public boolean onSingleTapConfirmed(MotionEvent e) {
            if(e.getAction()==MotionEvent.ACTION_DOWN){
                KeyEvent kevent = new KeyEvent(KeyEvent.ACTION_DOWN, KeyEvent.KEYCODE_ENTER);
                mCustomKeyListener.onKey(mglRootView, kevent.getKeyCode(), kevent);
              }
    ...
    // Two successive click calculation time intervals
    public static Long elapseTime;
    public static Long nowTime;

    public boolean getTime(long sapceTime) {
        long time = 0;
        if (elapseTime == null) {
    ...
    // BEGIN: add player video and left/right process
    public class MyCustomKeyListener {
        public boolean onKey(GLRootView glRootView, int keyCode, KeyEvent event) {
            mglRootView = glRootView;
            Log.d(TAG, "PhotoView . MyCustomKeyListener keyCode=" + keyCode);
            ...
            case KeyEvent.KEYCODE_DPAD_LEFT:
                ...
                mPositionController.startScroll(-getWidth() / 3, 0.0f,
            ...
            case KeyEvent.KEYCODE_DPAD_CENTER:
                mPhotoTapListener.actionSlideshow();
                return true;
            }
