...
static HI_S32 VDEC_FindVdecHandleByPortHandle(HI_HANDLE hPort, HI_HANDLE * phVdec)
{
    ...
    if (i >= HI_VDEC_MAX_INSTANCE_NEW)
    {
        HI_ERR_VDEC("Can't find vdec :%s %d!\n",__FUNCTION__,__LINE__);
        return HI_FAILURE;
    }
    else
    {
    ...
    case EVNT_UNSUPPORT_SPEC:  // UNSUPPORT类型的扩展，特殊处理后然后走EVNT_UNSUPPORT同样分支
        ...
        switch (e_SpecType)
        {
            case SPEC_BIT_DEPTH:
                ...
                pstChan->u32UnSupportStream = 1;
                break;
            case SPEC_OVER_CAP:
                pstChan->u32UnSupportStream = 2;
                break;
            default:
                pstChan->u32UnSupportStream = 1;
                break;
...
static HI_S32 VDEC_SetAttr(VDEC_CHANNEL_S *pstChan)
{
    ...
    else if (HI_UNF_VCODEC_TYPE_HEVC == pstCfg->enType)
    {
        if((1 == pstChan->stOption.u32DynamicFrameStoreAllocEn) && \
    ...
    switch (MemConfig.u32TotalSize)
    {
        case 512:
        {
            ...
            {
                stVdecChanCfg.s32MaxWidth = 3840;
		        stVdecChanCfg.s32MaxHeight = 1088;
            }
...
static HI_S32 VDEC_Chan_VORlsFrame(HI_HANDLE hPort, HI_DRV_VIDEO_FRAME_S *pstVpssFrame)
{
    ...
    if(HI_SUCCESS!=s32Ret)
    {
        HI_ERR_VDEC("VDEC_Chan_VORlsFrame call VDEC_FindVdecHandleByPortHandle ERROR!\n");
        return HI_FAILURE;
    }
    else
    {
...
static HI_S32 VDEC_Chan_CreateVpss(HI_HANDLE hVdec,HI_HANDLE * phVpss)
{
    ...
    #if 0
    s32Ret = (s_stVdecDrv.pVpssFunc->pfnVpssSendCommand)(s_stVdecDrv.astChanEntity[hVdec].stVpssChan.hVpss,HI_DRV_VPSS_USER_COMMAND_START, NULL);
    if (HI_SUCCESS != s32Ret)
    {
...
static HI_S32 VDEC_Chan_SetExtBuffer(HI_HANDLE hVpss,VDEC_BUFFER_ATTR_S *pstBufferAttr)
{
    HI_S32 i;
    BUFMNG_VPSS_NODE_S* pstBufNode;
    VDEC_VPSSCHANNEL_S *pstVpssChan = HI_NULL;
...
static HI_S32 VDEC_Chan_CheckAndDelBuffer(HI_HANDLE hVpss, VDEC_BUFFER_INFO_S stBufInfo)
{
    HI_S32 i;
    struct list_head *pos,*n;
    BUFMNG_VPSS_NODE_S* pstTarget;
    VDEC_VPSSCHANNEL_S *pstVpssChan = HI_NULL;
    BUFMNG_VPSS_INST_S* pstBufVpssInst;
    if ((HI_INVALID_HANDLE == hVpss) || (HI_NULL == stBufInfo.penBufState))
    {
        HI_ERR_VDEC("Bad param!\n");
        return HI_FAILURE;
    }
    ...
    {
         pstTarget = list_entry(pos, BUFMNG_VPSS_NODE_S, node);
         if(stBufInfo.u32PhyAddr == pstTarget->MMZBuf_frm.u32StartPhyAddr)
         {
...
static HI_S32 VDEC_Chan_InitParam(HI_HANDLE hHandle,HI_UNF_AVPLAY_OPEN_OPT_S *pstCapParam)
{
    ...
    pstChan->u32UnSupportStream = 0;//HI_FALSE;
...
static HI_S32 VDEC_Chan_GetMemSize(VDEC_CHANNEL_S *pstChan)
{
    HI_S32 s32Ret;
    HI_S8 as8TmpBuf[128];
...
static HI_S32 VDEC_Chan_Create(HI_HANDLE hHandle)
{
    ...
    if (1 == pstChan->stOption.s32IsSecMode)
    {
        ...
        pstChan->stEOSBuffer.u32StartVirAddr = pstChan->stEOSBuffer.u32StartPhyAddr;
    ...
    if (pstChan->stMemSize.ScdDetailMem > 0)
    {
...
#endif
        {
            ...
            pstChan->stEOSBuffer.u32StartVirAddr = pstChan->stSCDMMZBuf.u32StartVirAddr + pstChan->stSCDMMZBuf.u32Size - HI_VDEC_EOS_MEM_SIZE;
            pstChan->stEOSBuffer.u32Size = HI_VDEC_EOS_MEM_SIZE;
        }
    ...
    if (pstChan->stCurCfg.enType == HI_UNF_VCODEC_TYPE_H263 || pstChan->stCurCfg.enType == HI_UNF_VCODEC_TYPE_SORENSON)
    {
        ...
        //不应减小VDH的大小，否则会导致 chan create failed
...
//#endif
    if (u32VDHSize > 0)
    {
        ...
        if(HI_SUCCESS == s32Ret)
        {
            ...
            pstChan->stVDHMMZBuf.u32StartVirAddr = stMMZBuffer.u32StartVirAddr;
        }
        else
#endif
        {
            if(1 == pstChan->stOption.u32DynamicFrameStoreAllocEn) //如果采用动态帧存的方案,则在这里不需要了整块申请了 l00273086//
            {
                ...
                pstChan->stVDHMMZBuf.u32StartVirAddr = HI_NULL;
            }
            else
            {
...
err2:
#if (1 == PRE_ALLOC_VDEC_VDH_MMZ)
    if (pstChan->stVDHMMZBuf.u32Size > 0
        && (pstChan->stVDHMMZBuf.u32StartPhyAddr >= g_stVDHMMZ.u32StartPhyAddr && pstChan->stVDHMMZBuf.u32StartPhyAddr < (g_stVDHMMZ.u32StartPhyAddr+g_stVDHMMZ.u32Size)))
    {
...
static HI_S32 VDEC_Chan_Destroy(HI_HANDLE hHandle)
{
    ...
    if (HI_INVALID_HANDLE != pstChan->hChan)
    {
        ...
        pstChan->stEOSBuffer.u32StartVirAddr = 0;
...
HI_S32 HI_DRV_VDEC_ChanStart(HI_HANDLE hHandle)
{
    ...
    MMZ_BUFFER_S stMMZBuffer;
    //HI_U32 u32VDHSize = 0;
    //HI_BOOL u32StartWithOptionFlag = HI_FALSE;
    ...
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_VDEC("Chan %d GetMemSize err!\n", hHandle);
        return HI_FAILURE;
    }
    ...
    {
        s32Ret = (s_stVdecDrv.pVfmwFunc->pfnVfmwControl)(pstChan->hChan, VDEC_CID_START_CHAN_WITH_OPTION, &stChanMemVdh);
        if (VDEC_OK != s32Ret)
        {
            VDEC_CHAN_USE_UP(&s_stVdecDrv.astChanEntity[hHandle]);
            HI_ERR_VDEC("Chan %d VDEC_CID_START_CHAN_WITH_OPTION err!\n", pstChan->hChan);
            return HI_FAILURE;
        }
    }
    else
...
HI_S32 HI_DRV_VDEC_ChanStop(HI_HANDLE hHandle)
{
    ...
    if (HI_INVALID_HANDLE != s_stVdecDrv.astChanEntity[hHandle].pstChan->hChan)
    {
        ...
        for (i = 0; i < 50; i++)
        {
            ...
            {
                msleep(10);
                HI_WARN_VDEC("Chan %d DESTROY_CHAN err! time:%d\n", hHandle, i);
                continue;
            }
            else
            {
...
#endif
        {
            if(1 != pstChan->stOption.u32DynamicFrameStoreAllocEn)//如果使用动态帧存方案,也就没有MMZ.l00273086
            {
            HI_DRV_MMZ_UnmapAndRelease(&pstChan->stVDHMMZBuf);
            }
        }
...
#endif
{
    ...
    if(HI_TRUE == bTvp)
    {
        ...
        pstMMZBuf->u32StartVirAddr = pstMMZBuf->u32StartPhyAddr;
        pstMMZBuf->u32Size = u32BufSize;
        pstMMZBuf->bSecureMem = HI_TRUE;
        return HI_SUCCESS;
    }
	else
#endif
...
HI_S32 VDEC_Ioctl(struct inode *inode, struct file  *filp, unsigned int cmd, void *arg)
{
    ...
    case UMAPC_VDEC_CHAN_GETPORTPARAM:
    {
		...
		if(HI_SUCCESS != s32Ret)
		{
			HI_WARN_VDEC("VDEC_Chan_GetPortParam err!:%d\n",s32Ret);
		}
        break;
    }
    case UMAPC_VDEC_CHAN_RESETVPSS:
    ...
    for(i = 0; i < g_VdecPreMMZNodeNum; i++)
    {
           ...
           (st_VdecChanPreUsedMMZInfo[i].u32Size >= pstMMZbuffer->u32Size))
        {
            if(HI_SUCCESS == VDEC_Chan_FindPreMMZ(pstMMZbuffer))
            {
                ...
                pstChan->stVDHMMZBuf.u32StartVirAddr = pstMMZbuffer->u32StartVirAddr;
            }
            else
            {
    ...
    if(i >= g_VdecPreMMZNodeNum)
    {
        for(i = 0; i < g_VdecPreMMZNodeNum; i++)
        {
            if((0 == st_VdecChanPreUsedMMZInfo[i].u32NodeState) && (0 != st_VdecChanPreUsedMMZInfo[i].u32Size))
            {
                ...
                pstChan->stVDHMMZBuf.u32StartPhyAddr = st_VdecChanPreUsedMMZInfo[i].u32StartPhyAddr;
                pstChan->stVDHMMZBuf.u32StartVirAddr = st_VdecChanPreUsedMMZInfo[i].u32StartVirAddr;
                st_VdecChanPreUsedMMZInfo[i].u32NodeState = 1;
                g_VdecPreMMZNodeNum++;
                break;
            }
...
#endif
        {
            ...
            else
            {
                if(HI_SUCCESS == VDEC_FindPreMMZNode(hHandle, &stMMZBuffer)) //找到了合适的预分配节点
                {
                    ...
                    stFsParam.VirAddr = pstChan->stVDHMMZBuf.u32StartVirAddr;
...
static HI_S32 VDEC_OpenDev(HI_VOID)
{
    ...
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_VDEC("BUFMNG_Init err! ret:%d\n",s32Ret);
        goto err0;
    }
...
HI_S32 VDEC_DRV_Init(HI_VOID)
{
...
#if (1 == PRE_ALLOC_VDEC_SCD_MMZ)
    ret = HI_DRV_MMZ_Alloc("VFMW_SCD_PRE", "VFMW", ScdBufSize + HI_VDEC_SCD_EXT_MEM + HI_VDEC_EOS_MEM_SIZE, 0, &g_stSCDMMZ);
    if (HI_SUCCESS != ret)
    {
    ...
    if (VdhBufSize > 0)
    {
        ret = HI_DRV_MMZ_Alloc("VFMW_VDH_PRE", "VFMW", VdhBufSize, 0, &g_stVDHMMZ);
        if (HI_SUCCESS != ret)
        {
            HI_ERR_VDEC("VDH Alloc 0x%x failed\n", VdhBufSize);
            return HI_FAILURE;
        }
...
#endif
#if (1 == PRE_ALLOC_VDEC_ESBUF_MMZ)
    ret = HI_DRV_MMZ_AllocAndMap("VDEC_ESBuf_PRE", "VDEC", ESBUF_MMZ_SIZE, 0, &g_stESBUFMMZ);
    if (HI_SUCCESS != ret)
    {
