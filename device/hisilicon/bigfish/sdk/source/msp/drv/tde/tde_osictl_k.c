...
INLINE HI_S32 TdeOsiCheckSurfaceEX(TDE2_SURFACE_S* pstSurface, TDE2_RECT_S  *pstRect)
{
    ...
    if ((TDE_MAX_RECT_WIDTH_EX< pstRect->u32Width)
        || (TDE_MAX_RECT_HEIGHT_EX< pstRect->u32Height))
    {
...
STATIC INLINE HI_S32 TdeOsiCheckMbSurfaceEX(TDE2_MB_S* pstMbSurface, TDE2_RECT_S  *pstRect)
{
    ...
    if ((pstRect->s32Xpos < 0)
        || (TDE_MAX_RECT_WIDTH_EX< pstRect->u32Width)
        || (TDE_MAX_RECT_HEIGHT_EX< pstRect->u32Height)
        || (0 == pstRect->u32Height)
        || (0 == pstRect->u32Width)
    ...
    if ((pstMbSurface->u32YStride % 4)||(pstMbSurface->u32CbCrStride% 4))
    {
        TDE_TRACE(TDE_KERN_INFO, "stride is not aligned!\n");
        return -1;
    }
                        ...
                        TDE2_SURFACE_S* pstDst, TDE2_RECT_S  *pstDstRect, TDE2_MBOPT_S* pstMbOpt)
{
    ...
    u32CbCrBytePerPixel=1;
    if((pstMB->enMbFmt==TDE2_MB_COLOR_FMT_JPG_YCbCr444MBP)||\
        (pstMB->enMbFmt==TDE2_MB_COLOR_FMT_JPG_YCbCr422MBVP))
    {
        u32CbCrBytePerPixel=2;
    }
    ...
    for(n=0;n<j;n++)
    {
        ...
        for(m=0;m<i;m++)
        {
            ...
            if((1==i)&&(1==j))
            {
                pstDstRect->u32Width = pstDstRect->u32Width;
                pstDstRect->u32Height = pstDstRect->u32Height;
            }
            else
            {
                pstDstRect->u32Width = pstMbRect->u32Width;
