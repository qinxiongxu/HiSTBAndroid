...
static HI_S32 Convert_FrameStructure(HI_UNF_VIDEO_FRAME_INFO_S* pstUnfImage, HI_DRV_VIDEO_FRAME_S* pstDrvImage)
{
    ...
    switch(pstUnfImage->enFramePackingType)
    {
        case HI_UNF_FRAME_PACKING_TYPE_NONE:
			pstDrvImage->eFrmType = HI_DRV_FT_NOT_STEREO;
			break;
		case HI_UNF_FRAME_PACKING_TYPE_SIDE_BY_SIDE:
			pstDrvImage->eFrmType = HI_DRV_FT_SBS;
			break;
		case HI_UNF_FRAME_PACKING_TYPE_TOP_AND_BOTTOM:
			pstDrvImage->eFrmType = HI_DRV_FT_TAB;
			break;
		case HI_UNF_FRAME_PACKING_TYPE_TIME_INTERLACED:
			pstDrvImage->eFrmType = HI_DRV_FT_NOT_STEREO;
			HI_ERR_VENC("can't convert the format HI_UNF_FRAME_PACKING_TYPE_TIME_INTERLACED to Drv Structure\n");
			break;
		case HI_UNF_FRAME_PACKING_TYPE_FRAME_PACKING:
			pstDrvImage->eFrmType = HI_DRV_FT_FPK;
			break;
		case HI_UNF_FRAME_PACKING_TYPE_3D_TILE:
            pstDrvImage->eFrmType = (HI_DRV_FT_FPK + 1);  //HI_DRV_FT_FS;  fix me!
			break;
		default:
			pstDrvImage->eFrmType = HI_DRV_FT_BUTT;
			break;
    }

    pstDrvImage->u32Circumrotate = pstUnfImage->u32Circumrotate;
#ifndef __VENC_S5V100_CONFIG__
...
static HI_S32 Convert_DrvFrameStructure(HI_DRV_VIDEO_FRAME_S* pstDrvImage, HI_UNF_VIDEO_FRAME_INFO_S* pstUnfImage)
{
    ...
    switch (pstDrvImage->ePixFormat)
    {
        ...
        case HI_DRV_PIX_FMT_NV12:
            pstUnfImage->enVideoFormat = HI_UNF_FORMAT_YUV_SEMIPLANAR_420_UV;
            break;
...
#if (defined(__VENC_S40V200_CONFIG__) || defined(__VENC_S5V100_CONFIG__))    //s5v100 & s40v200 have no vedu_chip_id limit!
    g_vedu_chip_id = 2;
                      ...
                      VENC_CHN_INFO_S *pstVeInfo,HI_BOOL bOMXChn, struct file  *pfile)
{
    ...
    switch (pVencUnfAttrs->enVencType)
    {
        case HI_UNF_VCODEC_TYPE_H264:
            ...
            EncCfg.Profile  = (HI_U32)pVencUnfAttrs->enVencProfile;
            EncCfg.Gop      = pVencUnfAttrs->u32Gop;
            EncCfg.QLevel   = 0;
            break;
        ...
        case HI_UNF_VCODEC_TYPE_JPEG:
            ...
            EncCfg.Profile  = VEDU_H264_PROFILE_BUTT;
            EncCfg.QLevel   = pVencUnfAttrs->u32Qlevel;
            EncCfg.Gop      = 0;
            break;
        default:
            EncCfg.Protocol = VEDU_H264;
    ...
    {
        case 0:
            if (pVencUnfAttrs->enCapLevel > HI_UNF_VCODEC_CAP_LEVEL_D1)
            {
                HI_ERR_VENC("vedu_chip_id(%d) not support create this level(%d) channel(should below D1)!\n", g_vedu_chip_id, pVencUnfAttrs->enCapLevel);
	...
	if (EncCfg.DriftRCThr <= 100)
	{
	    EncCfg.bRCSkipFrmEn = HI_TRUE;
	}
	else
	{
    ...
    if (EncCfg.SlcSplitEn)
    {
        ...
        else if (pVencUnfAttrs->u32Width >= 720)
        {
            EncCfg.SplitSize  = 6;
        }
        else
...
#ifdef __VENC_SUPPORT_JPGE__
    if (HI_INVALID_HANDLE != g_stVencChn[u32VeChn].hJPGE)
    {
...
HI_S32 VENC_DRV_SetSrcInfo(VeduEfl_EncPara_S* hVencChn, HI_DRV_VENC_SRC_INFO_S* pstSrcInfo)
{
    HI_S32 s32Ret;
    HI_U32 u32VeChn = 0;
    VeduEfl_SrcInfo_S stSrcInfo;
                              ...
                              VENC_BUF_OFFSET_S* pstBufOffSet)
{
    ...
    if (g_stVencChn[u32VeChn].hJPGE != HI_INVALID_HANDLE) /*JPEG*/
    {
        HI_VOID* pVirAddr = (HI_VOID*)pstEncChnPara->JpgMMZBuf.u32StartVirAddr;

        if (stVeduPacket.SlcLen[0] > 0)
        {
            memcpy(pVirAddr, stVeduPacket.pVirtAddr[0], stVeduPacket.SlcLen[0]);
        }

        if (stVeduPacket.SlcLen[1] > 0)
        {
            memcpy(pVirAddr + stVeduPacket.SlcLen[0], stVeduPacket.pVirtAddr[1], stVeduPacket.SlcLen[1]);
        }

        u32StreamTemp = stVeduPacket.SlcLen[0] + stVeduPacket.SlcLen[1];
        ...
        do
        {
            ...
            s32Ret = VENC_DRV_EflGetBitStream(hVencChn, &stVeduPacket);
            if (s32Ret != HI_SUCCESS) { msleep(5); }
        }
        while ((s32Ret != HI_SUCCESS) || (stVeduPacket.bFrameEnd != HI_TRUE));

        if (stVeduPacket.SlcLen[0] > 0)
        {
            memcpy(pVirAddr + u32StreamTemp , stVeduPacket.pVirtAddr[0], stVeduPacket.SlcLen[0]);
            u32StreamTemp += stVeduPacket.SlcLen[0];
        }
        if (stVeduPacket.SlcLen[1] > 0)
        {
            memcpy(pVirAddr + u32StreamTemp, stVeduPacket.pVirtAddr[1], stVeduPacket.SlcLen[1]);
            u32StreamTemp += stVeduPacket.SlcLen[1];
        }
        ...
        pstStream->pu8Addr      = (PTR_COMPAT_T)pVirAddr;
    ...
    else
    {
        ...
        pstStream->pu8Addr      = (PTR_COMPAT_T)stVeduPacket.pVirtAddr[0];
    ...
    if ((HI_TRUE == g_stVencChn[u32VeChn].stProcWrite.bTimeModeRun) || (HI_TRUE == g_stVencChn[u32VeChn].stProcWrite.bFrameModeRun))
    {
        ...
        if (pstStream->u32SlcLen > 0)
        {
            s32Ret = VENC_DRV_OsalFwrite((char*)pstStream->pu8Addr, pstStream->u32SlcLen ,g_stVencChn[u32VeChn].stProcWrite.fpSaveFile);
...
HI_S32 VENC_DRV_ReleaseStream(VeduEfl_EncPara_S* hVencChn, VENC_INFO_ACQUIRE_STREAM_S* pstAcqStrm)
{
    ...
    pstStream.pu8Addr  = (PTR_COMPAT_T)(pstEncChnPara->stCycBuf.pBase + pstAcqStrm->H264_stream_off);
#if 1
    if ((PTR_COMPAT_T)g_stVencChn[u32VeChn].stChnPacket.pVirtAddr[0] == pstStream.pu8Addr)
    {
    ...
    {
        stVeduPacket.pVirtAddr[0] = (HI_VOID*)pstStream.pu8Addr ;
...
HI_S32 VENC_DRV_ReleaseStreamIntar(VeduEfl_EncPara_S* hVencChn, VENC_STREAM_S* pstStream)
{
    ...
    if (g_stVencChn[u32VeChn].hJPGE != HI_INVALID_HANDLE)
    {
        ...
        if ((pstStream->pu8Addr != (PTR_COMPAT_T)pstEncChnPara->JpgMMZBuf.u32StartVirAddr)
            || (pstEncChnPara->stStat.GetStreamNumOK <= pstEncChnPara->stStat.PutStreamNumOK))
        {
            HI_ERR_VENC("Release stream failed, GetStreamNumOK=%d,PutStreamNumOK=%d,pstStream->pu8Addr = %x,JpegOutBufPhyAddr= %x.\n",
                        pstEncChnPara->stStat.GetStreamNumOK, pstEncChnPara->stStat.PutStreamNumOK, pstStream->pu8Addr, 
                        pstEncChnPara->JpgMMZBuf.u32StartPhyAddr);
            return HI_ERR_VENC_CHN_RELEASE_ERR;
        }
        pstEncChnPara->stStat.PutStreamNumOK++;
...
HI_S32 VENC_DRV_SetAttr(VeduEfl_EncPara_S* EncHandle, HI_UNF_VENC_CHN_ATTR_S* pstAttr, VENC_CHN_INFO_S* pstVeInfo)
{
    ...
    if (HI_TRUE == ErrCfg)
    {
        ...
        if (pstVenc->stChnUserCfg.enCapLevel != pstAttr->enCapLevel)
        {
            HI_ERR_VENC("VENC not support this active change enCapLevel:(%d -> %d)!\n",
                        pstVenc->stChnUserCfg.enCapLevel, pstAttr->enCapLevel);
        }
    ...
    if (pstVenc->stChnUserCfg.enVencType != pstAttr->enVencType)
    {
        ...
        {
            if (HI_INVALID_HANDLE == pstVenc->hJPGE)
            {
                return HI_ERR_VENC_CHN_INVALID_STAT;
            }
            ...
            pstEncChnPara->Protocol = VEDU_H264;
        }
        else                                                  /*h264 --> jpeg*/
        {
            if (HI_INVALID_HANDLE != pstVenc->hJPGE)
            ...
            JpgeEncCfg.FrameWidth  = pstAttr->u32Width;
            JpgeEncCfg.FrameHeight = pstAttr->u32Height;
            JpgeEncCfg.SlcSplitEn  = pstAttr->bSlcSplitEn;
            JpgeEncCfg.SplitSize   = pstVenc->u32SliceSize;
            JpgeEncCfg.Qlevel      = pstAttr->u32Qlevel;

            s32Ret = pJpgeFunc->pfnJpgeCreateChn(&(pstVenc->hJPGE), &JpgeEncCfg );
            if (HI_SUCCESS != s32Ret)
            {
                HI_ERR_VENC("pfnJpgeCreateChn failed!\n");
                return s32Ret;
    ...
    else  /* jpeg*/
    {
        ...
        pstEncChnPara->VoFrmRate = pstAttr->u32TargetFrmRate;
        pstEncChnPara->ViFrmRate = pstAttr->u32InputFrmRate;
        //pstEncChnPara->TrCount   = pstEncChnPara->ViFrmRate;
        pstEncChnPara->VBRflag   = 0;
        pstEncChnPara->PicLevel  = 0;
    ...
    if (pstVenc->stChnUserCfg.u32RotationAngle != pstAttr->u32RotationAngle)
    {
        ...
        {
            switch (pstAttr->u32RotationAngle)
            {
                case 0:
                    pstEncChnPara->RotationAngle = VEDU_ROTATION_0;
                    break;
                case 90:
                    pstEncChnPara->RotationAngle = VEDU_ROTATION_90;
                    break;
                case 270:
                    pstEncChnPara->RotationAngle = VEDU_ROTATION_270;
                    break;
                case 180:
                    pstEncChnPara->RotationAngle = VEDU_ROTATION_180;
                    break;
                default:
                    pstEncChnPara->RotationAngle = VEDU_ROTATION_0;
                    break;
        ...
        {
            HI_ERR_VENC("config u32RotationAngle(%d) not support!.\n", pstAttr->u32RotationAngle);
            return HI_FAILURE;
        }
    }
#endif
