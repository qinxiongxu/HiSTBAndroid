...
HI_S32 VPSS_CTRL_FixTask(VPSS_IP_E enIp, HI_DRV_BUF_ADDR_E enLR, VPSS_TASK_S *pstTask)
{
    ...
    for(i=0; i<DEF_HI_DRV_VPSS_PORT_MAX_NUMBER; i++)
    {
        ...
        if (pstFrmNode!= HI_NULL)
        {
            ...
            pstHalInfo->astPortInfo[i].stOutInfo.bUVInver = pstInst->stPort[i].bUVInver;
...
HI_U32 VPSS_CTRL_Zme2lAndRotateCfg(VPSS_IP_E enIp, VPSS_TASK_S *pstTask)
{
	...
	/*
		根据能力集及version判断是否需要做二级缩放，目前仅hifoneB02支持二级缩放
	*/
	if ((pstInst->stInEntity.enVersion == VPSS_VERSION_V1_0) && 
		(HI_TRUE == pstInst->enCapability.bits.hi3798cv200_a))
	{			
		/*	 检测是否需要做二级缩放操作，记录当前port二级旋转标志 并 申请相应的缓存buffer*/
		DRV_PQ_ZME_2L_Check(pstCur->u32Width, pstCur->u32Height, stZmeWin, &u32DstW, &u32DstH);
		if (HI_SUCCESS != s32Ret)
		{
			VPSS_FATAL("DRV_PQ_ZME_2L_Check failed!! func=%s, line=%d\n",__func__,__LINE__);
			return HI_FAILURE;
		}

		if( u32DstW == pstPort[0]->s32OutputWidth && u32DstH == pstPort[0]->s32OutputHeight)
		{	
			pstPort[0]->u32ZME1LWidth  = 0;
			pstPort[0]->u32ZME1LHeight = 0;

			pstPort[1]->u32ZME1LWidth  = u32DstW;
			pstPort[1]->u32ZME1LHeight = u32DstH;
		}

		else if( u32DstW == 0 && u32DstH == 0)
		{
			pstPort[0]->u32ZME1LWidth  = 0;
			pstPort[0]->u32ZME1LHeight = 0;

			pstPort[1]->u32ZME1LWidth  = 0;
			pstPort[1]->u32ZME1LHeight = 0;
		}

		else 
		{
			pstPort[0]->u32ZME1LWidth  = u32DstW;
			pstPort[0]->u32ZME1LHeight = u32DstH;
...
#else
		u32DstW = pstCur->u32Width /2;
		u32DstH = pstCur->u32Height/2;

		pstTask->bZME2lTaskNeed[0] = HI_TRUE;
		pstPort[0]->bNeedZME2L = HI_TRUE;
		pstPort[0]->u32ZME1LWidth  = u32DstW;
		pstPort[0]->u32ZME1LHeight = u32DstH;
