	...
	
    {
        ...
        if (atomic_read(&stDispWindow.bWindowSytemUpdate[hDst]))
        {
            ...
            for (i = 0; i < WINDOW_MAX_NUMBER; i++)
            {
                ...
                {
                    if (((HI_U32)pWin->stWinLayerOpt.pParam) == HI_DRV_DISP_ZORDER_MOVETOP)
                    {

                        pWin->stVLayerFunc.PF_MovTop(pWin->u32VideoLayer);
                    }
                    else if (((HI_U32)pWin->stWinLayerOpt.pParam) == HI_DRV_DISP_ZORDER_MOVEBOTTOM)
                    {
...
static HI_S32 WIN_SetWinState(HI_VOID)
{
    WINDOW_S *pstWin = HI_NULL;
    HI_U32 i = 0, j = 0;
...
HI_S32 WindowRedistributeProcess(WINDOW_S *tmpWindow)
{
    ...
    for (i = 0; i < WINDOW_MAX_NUMBER; i++)
    {
            ...
            || (pstWinTmp->u32VideoLayerNew == VDP_RM_LAYER_VID0)))
        {
            ...
            pstWinTmp->stWinLayerOpt.pParam = (HI_VOID*)winZorderInfor[k].matchOperation;
        }
    }
 #if 0
    for (i = 0; i < numofWindow; i++)
...
HI_VOID ISR_WinConfigFrameNormal(WINDOW_S *pstWin, HI_DRV_VIDEO_FRAME_S *pstFrame, const HI_DRV_DISP_CALLBACK_INFO_S *pstInfo)
{
    ...
    {
         printk("1in:%d, %d, %d,%d, v:%d,%d, %d,%d,d:%d, %d, %d,%d!\n",
            ...
            * 0010:  00000000 0000040f 00000000 0000040f*/
            ...
            if (HI_NULL != pstDispOpt)
            {
                if ((pstInfo->stDispInfo.bInterlace == HI_TRUE)
                        && (pstInfo->stDispInfo.stFmtResolution.s32Width == 720)
                        && (pstInfo->stDispInfo.stFmtResolution.s32Height == 480))
                {
                    if (u32Fidelity == FIDELITY_MOTO_COMBINATION)
                    {
                        (HI_VOID)pstDispOpt->PF_DATE_SetIRE(pstWin->enDisp, 1);
                    }
                    else
                    {
