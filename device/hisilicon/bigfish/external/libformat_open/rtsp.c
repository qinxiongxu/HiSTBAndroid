...
#define SEEK_DISABLE (-1)    //l00192899 add
#define RTP_READ_EAGAIN_TIME_OUT (30000000) // 30 second
...
#define RTSP_FLAG_OPTS(name, longname) \
    { name, "", OFFSET(rtsp_flags), AV_OPT_TYPE_FLAGS, {0}, INT_MIN, INT_MAX, DEC, "rtsp_flags" }, \
    { "filter_src", "", 0, AV_OPT_TYPE_CONST, {RTSP_FLAG_FILTER_SRC}, 0, 0, DEC, "rtsp_flags" }
#endif
...
const AVOption ff_rtsp_options[] = {
    { "initial_pause", "",  OFFSET(initial_pause), AV_OPT_TYPE_INT, {0}, 0, 1, DEC },
    FF_RTP_FLAG_OPTS(RTSPState, rtp_muxer_flags),
    { "rtsp_transport", "",  OFFSET(lower_transport_mask), AV_OPT_TYPE_FLAGS, {0}, INT_MIN, INT_MAX, DEC|ENC, "rtsp_transport" }, \
    { "udp", "",  0, AV_OPT_TYPE_CONST, {1 << RTSP_LOWER_TRANSPORT_UDP}, 0, 0, DEC|ENC, "rtsp_transport" }, \
    { "tcp", "",  0, AV_OPT_TYPE_CONST, {1 << RTSP_LOWER_TRANSPORT_TCP}, 0, 0, DEC|ENC, "rtsp_transport" }, \
    { "udp_multicast", "",  0, AV_OPT_TYPE_CONST, {1 << RTSP_LOWER_TRANSPORT_UDP_MULTICAST}, 0, 0, DEC, "rtsp_transport" },
    { "http", "",  0, AV_OPT_TYPE_CONST, {(1 << RTSP_LOWER_TRANSPORT_HTTP)}, 0, 0, DEC, "rtsp_transport" },
    RTSP_FLAG_OPTS("rtsp_flags", ""),
    RTSP_MEDIATYPE_OPTS("allowed_media_types", ""),
    { "min_port", "",  OFFSET(rtp_port_min), AV_OPT_TYPE_INT, {RTSP_RTP_PORT_MIN}, 0, 65535, DEC|ENC },
    { "max_port", "", OFFSET(rtp_port_max), AV_OPT_TYPE_INT, {RTSP_RTP_PORT_MAX}, 0, 65535, DEC|ENC },
    { "download_speed_collect_freq_ms", "",  OFFSET(download_speed_collect_freq_ms), AV_OPT_TYPE_INT, {.i64 = 0}, 0, INT_MAX, DEC|ENC},
    { NULL },
};

static const AVOption sdp_options[] = {
    RTSP_FLAG_OPTS("sdp_flags", ""),
    RTSP_MEDIATYPE_OPTS("allowed_media_types", ""),
    { NULL },
};

static const AVOption rtp_options[] = {
    RTSP_FLAG_OPTS("rtp_flags", ""),
    { NULL },
};
                           ...
                           int letter, const char *buf)
{
    ...
    case 'c':
        ...
        if (av_strcmp(buf1, "IN") != 0)
            return;
        get_word(buf1, sizeof(buf1), &p);
        if (av_strcmp(buf1, "IP4") && av_strcmp(buf1, "IP6"))
            return;
        get_word_sep(buf1, sizeof(buf1), "/", &p);
        if (ff_is_multicast_address((struct sockaddr *)buf1))
        {
            rt->is_multicast = 1;
        }
        memset(&sdp_ip, 0, sizeof(sdp_ip));
    ...
    case 'm':
        ...
        if (!av_strcmp(st_type, "audio")) {
            codec_type = AVMEDIA_TYPE_AUDIO;
        } else if (!av_strcmp(st_type, "video")) {
            codec_type = AVMEDIA_TYPE_VIDEO;
        } else if (!av_strcmp(st_type, "application")) {
        ...
        dynarray_add_noverify(&rt->rtsp_streams, &rt->nb_rtsp_streams, rtsp_st, &add_ret);
        if (add_ret < 0){
            av_free(rtsp_st);
            return;
        }
...
#if 1
            /*l00192899 add start find realtime stream*/
            if(end != AV_NOPTS_VALUE && end != start)
            {
                s->bSeekFlag = 0;
            }
            /*l00192899 add end*/
...
static void rtsp_parse_transport(RTSPMessageHeader *reply, const char *p)
{
    ...
    for (;;) {
        ...
        while (*p != '\0' && *p != ',') {
            ...
            if (!av_strcmp(parameter, "port")) {
                if (*p == '=') {
                    p++;
                    rtsp_parse_range(&th->port_min, &th->port_max, &p);
                }
            } else if (!av_strcmp(parameter, "client_port")) {
                if (*p == '=') {
                    p++;
                    rtsp_parse_range(&th->client_port_min,
                                     &th->client_port_max, &p);
                }
            } else if (!av_strcmp(parameter, "server_port")) {
                if (*p == '=') {
                    p++;
                    rtsp_parse_range(&th->server_port_min,
                                     &th->server_port_max, &p);
                }
            } else if (!av_strcmp(parameter, "interleaved")) {
                if (*p == '=') {
                    p++;
                    rtsp_parse_range(&th->interleaved_min,
                                     &th->interleaved_max, &p);
                }
            } else if (!av_strcmp(parameter, "multicast")) {
                if (th->lower_transport == RTSP_LOWER_TRANSPORT_UDP)
                    th->lower_transport = RTSP_LOWER_TRANSPORT_UDP_MULTICAST;
            } else if (!av_strcmp(parameter, "ttl")) {
                if (*p == '=') {
                    p++;
                    th->ttl = strtol(p, (char **)&p, 10);
                }
            } else if (!av_strcmp(parameter, "destination")) {
                if (*p == '=') {
                    p++;
                    get_word_sep(buf, sizeof(buf), ";,", &p);
                    get_sockaddr(buf, &th->destination);
                }
            } else if (!av_strcmp(parameter, "source")) {
                if (*p == '=') {
                    p++;
                    get_word_sep(buf, sizeof(buf), ";,", &p);
                            ...
                            uint32_t seq, uint32_t rtptime)
{
    ...
    for (i = 0; i < rt->nb_rtsp_streams; i++) {
        ...
        if (!rtpctx)
            continue;
        if (!av_strcmp(rtsp_st->control_url, url)) {
            if (rtptime)
                rtpctx->base_timestamp = rtptime;
            if (rt->state == RTSP_STATE_SEEKING){
                rtpctx->statistics.first_seq = (seq & 0xffff);
                /*the next expected pacet seq is (rtpctx->seq + 1)*/
                if (rtpctx->statistics.first_seq >= 1)
                    rtpctx->seq = rtpctx->statistics.first_seq - 1;
                else
                    rtpctx->seq = 0;
            }
            break;
...
static void rtsp_parse_rtp_info(RTSPState *rt, const char *p)
{
    ...
    for (;;) {
        ...
        read++;
        if (!av_strcmp(key, "url"))
            av_strlcpy(url, value, sizeof(url));
        else if (!av_strcmp(key, "seq"))
            seq = strtoul(value, NULL, 10);
        else if (!av_strcmp(key, "rtptime"))
    ...
    for (j = rt->rtp_port_min + port_off, i = 0; i < rt->nb_rtsp_streams; ++i) {
        ...
        if (lower_transport == RTSP_LOWER_TRANSPORT_UDP &&
            ...
            if (i == 0) {
                ...
                for (rtx = 0; rtx < rt->nb_rtsp_streams; rtx++) {
                    ...
                    if (len >= 4 &&
                        !av_strcmp(rt->rtsp_streams[rtx]->control_url + len - 4,
                                "/rtx"))
                        break;
                }
        ...
        have_port:
            snprintf(transport, sizeof(transport) - 1,
                     "%s/UDP;", trans_pref);
           //comment this for helix server must append "unicast" when use udp.
           // if (rt->server_type != RTSP_SERVER_REAL)
...
#if 1
        /*l00192899 add start when server is WMS, and udp can not connect*/
        else if (reply->status_code == 461 && rt->server_type == RTSP_SERVER_WMS
                 && rt->lower_transport != RTSP_LOWER_TRANSPORT_HTTP)
        {
            err = 1;
            goto fail;
        }
...
redirect:
    ...
    if (option_list) {
        ...
        while (option_list) {
            ...
            /* handle the options */
            if (!av_strcmp(option, "udp")) {
                lower_transport_mask |= (1<< RTSP_LOWER_TRANSPORT_UDP);
            } else if (!av_strcmp(option, "multicast")) {
                lower_transport_mask |= (1<< RTSP_LOWER_TRANSPORT_UDP_MULTICAST);
            } else if (!av_strcmp(option, "tcp")) {
                lower_transport_mask |= (1<< RTSP_LOWER_TRANSPORT_TCP);
            } else if(!av_strcmp(option, "http")) {
                lower_transport_mask |= (1<< RTSP_LOWER_TRANSPORT_TCP);
                rt->control_transport = RTSP_MODE_TUNNEL;
            } else if (!av_strcmp(option, "filter_src")) {
    ...
    for (rt->server_type = RTSP_SERVER_RTP;;) {
        ...
        if (rt->server_type != RTSP_SERVER_REAL && reply->real_challenge[0]) {
            ...
            av_strlcpy(real_challenge, reply->real_challenge, sizeof(real_challenge));
        break;
    }
#if 1
    /*l00192899 add start acquiesce Rtsp is not seekable*/
    s->bSeekFlag = SEEK_DISABLE;
    /*l00192899 add end*/
#endif
    ...
    if (err)
        goto fail;
    /*l00192899 modify if stream is multiplay, try UDP_MULTICAST lower_transport first*/
    if (1 == rt->is_multicast)
    {
        lower_transport_mask &= (1<< RTSP_LOWER_TRANSPORT_UDP_MULTICAST);
    }
    do {
        ...
        if (lower_transport_mask == 0 && err == 1) {
            /*l00192899 modify if stream is multiplay, if it is fail to try UDP_MULTICAST lower_transport,
            try other lower_transport from start*/
            if(1 == rt->is_multicast)
            {
                lower_transport_mask = (1 << RTSP_LOWER_TRANSPORT_NB) - 1;
                continue;
...
int ff_rtsp_reset_demux(AVFormatContext *s)
{
    RTSPState *rt = s->priv_data;

    switch(rt->server_type)
    {
        case RTSP_SERVER_WMS:
            if (rt->asf_ctx != NULL &&
                rt->asf_ctx->iformat != NULL &&
                rt->asf_ctx->iformat->read_close != NULL){
                return rt->asf_ctx->iformat->read_close(rt->asf_ctx);
            }
            break;
        case RTSP_SERVER_REAL:
            break;
        case RTSP_SERVER_RTP:
            break;
    }
...
static void* rtsp_udp_recv_thread(void *arg)
{
    ...
    DataPacket rtp_pkt, empty_pkt;
    int buf_size = rt->max_udp_pkt_size;
    int len, fd_max = 0, *fds = NULL;
    fd_set rfds, rfds1;
    struct timeval tv;

    if (rt == NULL)
        return NULL;
    fds = av_malloc(sizeof(int)*2*(rt->nb_rtsp_streams));

    av_log(0, AV_LOG_INFO, "[%s,%d][RTSP] enter rtsp thread\n", __FUNCTION__, __LINE__);
    ...
    for (i = 0; i < rt->nb_rtsp_streams; i++) {
        rtsp_st = rt->rtsp_streams[i];
        if (rtsp_st->rtp_handle) {
            fds[fd_num] = ffurl_get_file_handle(rtsp_st->rtp_handle);
            if (fds[fd_num] > fd_max) {
                fd_max = fds[fd_num];
            }
            FD_SET(fds[fd_num], &rfds);
            av_log(0, AV_LOG_INFO, "[RTSP]fd set fds[%d]=%d, fd_max=%d\n", fd_num, fds[fd_num], fd_max);
            fd_num++;
            fds[fd_num] = ff_rtp_get_rtcp_file_handle(rtsp_st->rtp_handle);
            if (fds[fd_num] > fd_max) {
                fd_max = fds[fd_num];
            }
            FD_SET(fds[fd_num], &rfds);
        ...
        if (n > 0) {
            for (i = 0; i < fd_num; i++) {
                if (FD_ISSET(fds[i], &rfds)) {
                    if (pkt_buf == NULL) {
                        if (pktq_get_pkt(rt->empty_buf_queue, &empty_pkt, 0) < 0) {
                            pkt_buf = av_malloc(rt->max_udp_pkt_size);
                            if (pkt_buf == NULL) {
                                av_log(0, AV_LOG_ERROR, "[%s,%d][RTSP] no memory...\n", __FUNCTION__, __LINE__);
                                rt->is_exit = 1;
                                goto err;
                            }
                        } else {
                            pkt_buf = empty_pkt.data;
                            buf_size = empty_pkt.buf_size;
                        }
                    }
                    ret = recv(fds[i], pkt_buf, buf_size, 0);
                    if (ret > 0) {
                        if (ret >= buf_size) {
                            av_log(0, AV_LOG_ERROR, "[%s,%d] [RTSP] buf size not enough!!\n", __FUNCTION__, __LINE__);
                        }
                        rtp_pkt.data = pkt_buf;
                        rtp_pkt.data_size = ret;
                        rtp_pkt.buf_size = buf_size;
                        rtp_pkt.next = NULL;
                        rtp_pkt.priv = (void *)(i/2);
                        rtp_pkt.rd_idx = 0;
                        if (pktq_put_pkt(rt->rtp_queue, &rtp_pkt, 0) > 0) {
                            pkt_buf = NULL;
                        }
...
err:
    pktq_put_pkt(rt->rtp_queue, NULL, 0);//force to quit
    pktq_put_pkt(rt->empty_buf_queue, NULL, 0);//force to quit
    ...
    av_log(0, AV_LOG_INFO, "[%s,%d][RTSP] exit rtsp thread\n", __FUNCTION__, __LINE__);
...
int create_udp_recv_thread(AVFormatContext *s)
{
    ...
    ret = pktq_create(&rt->rtp_queue, 20971520, 4194304);
    ret |= pktq_create(&rt->empty_buf_queue, -1, -1);
    if (ret == 0) {
        ret = pthread_create(&rt->udp_rcv_thread_id, NULL, rtsp_udp_recv_thread, (void *)s);
        if (ret < 0) {
            av_log(0, AV_LOG_ERROR, "[%s,%d][RTSP] create rtsp_udp_recv_thread return error %d\n",
                __FUNCTION__, __LINE__, ret);
            goto err;
        }
    } else {
        av_log(0, AV_LOG_ERROR, "[%s,%d][RTSP] create pkt queue return error %d\n",
            __FUNCTION__, __LINE__, ret);
        goto err;
    }
    rt->max_udp_pkt_size = RTP_MAX_PACKET_LENGTH;

    return 0;

err:

    if (rt->rtp_queue != NULL)
        pktq_destroy(rt->rtp_queue);
    if (rt->empty_buf_queue != NULL)
        pktq_destroy(rt->empty_buf_queue);
    rt->rtp_queue = NULL;
    rt->empty_buf_queue = NULL;
    if (rt->udp_rcv_thread_id != 0) {
        pthread_join(rt->udp_rcv_thread_id, NULL);
        rt->udp_rcv_thread_id = 0;
    }
...
static int udp_read_packet(AVFormatContext *s, DataPacket *rtp_pkt, int64_t wait_end)
{
    RTSPState *rt = s->priv_data;
    int n, ret, tcp_fd = -1;
    ...
    if (rt->rtsp_hd) {
        ...
        if (tcp_fd >= 0) {
            ...
            if (n > 0 && FD_ISSET(tcp_fd, &rfds)) {
                RTSPMessageHeader reply;

                ret = ff_rtsp_read_reply(s, &reply, NULL, 0, NULL);
                if (ret < 0) {
                    av_log(0, AV_LOG_ERROR, "[%s,%d][RTSP] ff_rtsp_read_reply return error %d!\n",
                        __FUNCTION__, __LINE__, ret);
                    return ret;
                }
                /* XXX: parse message */
                if (rt->state != RTSP_STATE_STREAMING) {
                    av_log(0, AV_LOG_ERROR, "[%s,%d][RTSP] ff_rtsp_read_reply return %d, but state is %d\n",
                        __FUNCTION__, __LINE__, ret, rt->state);
                    return 0;
                }
            }
        }
    ...
    do {
        ...
        ret = pktq_get_pkt(rt->rtp_queue, rtp_pkt, 100);
        if (ret > 0) {
            return rtp_pkt->data_size;
        } else if (ret == 0) {//empty packet?
            if (rtp_pkt->data != NULL) {
                av_free(rtp_pkt->data);
                rtp_pkt->data = NULL;
            }
    ...
    } while(wait_end == 0 || (wait_end != 0 && wait_end - av_gettime() >= 0));
 ...
 redo:
...
#if HAVE_UDP_RCV_THREAD
        rtp_pkt.data = NULL;
        len = udp_read_packet(s, &rtp_pkt, wait_end);
        if (len > 0) {
            rtsp_stream_index = (int)rtp_pkt.priv;
            if (rtsp_stream_index >= rt->nb_rtsp_streams) {
                av_free(rtp_pkt.data);
                rtp_pkt.data = NULL;
                rtp_pkt.data_size = 0;
                rtp_pkt.buf_size = 0;
                av_log(0, AV_LOG_ERROR, "[%s,%d] invalid stream index=%d, max=%d\n",
                    __FUNCTION__, __LINE__, rtsp_stream_index, (rt->nb_rtsp_streams - 1));
                return -1;
            }
            rtsp_st = rt->rtsp_streams[rtsp_stream_index];
        }
#else
    ...
    if (rt->transport == RTSP_TRANSPORT_RDT) {
        ...
        if (rt->lower_transport == RTSP_LOWER_TRANSPORT_UDP ||
            ...
            ret = ff_rdt_parse_packet(rtsp_st->transport_priv, pkt, &(rtp_pkt.data), len);
            if (rtp_pkt.data != NULL) {
                rtp_pkt.data_size = 0;
                if (pktq_put_pkt(rt->empty_buf_queue, &rtp_pkt, 0) < 0) {
                    av_free(rtp_pkt.data);
                    rtp_pkt.data = NULL;
                    rtp_pkt.data_size = 0;
                    rtp_pkt.buf_size = 0;
        ...
        if (rt->lower_transport == RTSP_LOWER_TRANSPORT_UDP ||
            rt->lower_transport == RTSP_LOWER_TRANSPORT_UDP_MULTICAST) {
            ret = ff_rtp_parse_packet(rtsp_st->transport_priv, pkt, &(rtp_pkt.data), len);
            if (rtp_pkt.data != NULL) {
                rtp_pkt.data_size = 0;
                if (pktq_put_pkt(rt->empty_buf_queue, &rtp_pkt, 0) < 0) {
                    av_free(rtp_pkt.data);
...
static int sdp_read_header(AVFormatContext *s, AVFormatParameters *ap)
{
    ...
    av_log(NULL, AV_LOG_INFO, "Func:%s, Line:%d, nb_rtsp_streams: %d\n", __FUNCTION__, __LINE__, rt->nb_rtsp_streams);
    /* open each RTP stream */
    for (i = 0; i < rt->nb_rtsp_streams; i++) {
        char namebuf[50] = {0};
        rtsp_st = rt->rtsp_streams[i];

        is_multicast = ff_is_multicast_address((struct sockaddr*) &rtsp_st->sdp_ip);
        av_log(NULL, AV_LOG_INFO, "Func:%s, Line:%d, is_multicast: %d\n", __FUNCTION__, __LINE__, is_multicast);

        if (is_multicast) {
            sin = (struct sockaddr_in *)&rtsp_st->sdp_ip;
            ip_addr = ntohl(sin->sin_addr.s_addr);
            snprintf(namebuf, sizeof(namebuf), "%d.%d.%d.%d",
        ...
        av_log(NULL, AV_LOG_INFO, "Func:%s, Line:%d, hostname: %s\n", __FUNCTION__, __LINE__, namebuf);
        ...
        if (ffurl_open(&rtsp_st->rtp_handle, url, AVIO_FLAG_READ_WRITE,
                       &s->interrupt_callback, NULL) < 0) {
            err = AVERROR_INVALIDDATA;
            av_log(NULL, AV_LOG_ERROR, "Func:%s, Line:%d, err:0x%x\n", __FUNCTION__, __LINE__, err);
            goto fail;
        }
        if ((err = rtsp_open_transport_ctx(s, rtsp_st))) {
            av_log(NULL, AV_LOG_ERROR, "Func:%s, Line:%d, err:0x%x\n", __FUNCTION__, __LINE__, err);
...
static int rtp_probe(AVProbeData *p)
{
    ...
    {
        av_log(NULL, AV_LOG_ERROR, "use rtp demuxer\n");
                           ...
                           AVFormatParameters *ap)
{
    ...
    av_log(NULL, AV_LOG_INFO, "Func:%s, Line:%d\n", __FUNCTION__, __LINE__);
    ...
    while (1) {
        if (llabs(av_gettime() - start_time) > RTP_READ_EAGAIN_TIME_OUT) {
            av_log(NULL, AV_LOG_ERROR, "Func:%s, Line:%d, RTP read header data timeout !\n", __FUNCTION__, __LINE__);
            ret = -1;
            goto fail;
        }
    ...
    ffurl_close(in);
    in = NULL;
    av_log(NULL, AV_LOG_INFO, "Func:%s, Line:%d, payload_type:%d\n", __FUNCTION__, __LINE__, payload_type);
    ...
    av_log(NULL, AV_LOG_INFO, "Func:%s, Line:%d, ret:0x%x \n", __FUNCTION__, __LINE__, ret);
    return ret;
