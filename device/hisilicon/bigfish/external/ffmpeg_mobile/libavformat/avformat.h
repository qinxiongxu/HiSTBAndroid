...
#define MAX_LIVE_CACHE_DURATION (60 * 90) //1.5 hour
typedef enum
{
    HLS_EVENT_UNKNOWN = NETWORK_DISCONNECT + 5000,
...
typedef enum
{
    HLS_STREAM_MODE_QUALITY_FIRST = 0,
    HLS_STREAM_MODE_FLUENT_FIRST = 1,
    HLS_STREAM_MODE_AUTO = 2,
}HLSStreamMode;
...
typedef struct hls_segment_s {
    ...
    int     total_time;                 /* segment duration (seconds) */
...
typedef struct HLSContext_s {
    ...
    HLSStreamMode           stream_mode;
    int64_t                 cur_seg_download_start_time;
