...
// if in diagnose,net condition restored,request long old hls segment,
// the server will not response data until 30s or 60s,the abnormal situation
...
{
    if (NULL == src || NULL == src->priv_data)
        return;
...
static int http_open_cnx(URLContext *h)
{
    ...
    char hostname[URL_SIZE], hoststr[URL_SIZE], proto[10];
    char auth[URL_SIZE], proxyauth[URL_SIZE] = "";
    char path1[URL_SIZE];
    char buf[URL_SIZE], urlbuf[URL_SIZE];
    char old_location[URL_SIZE];
    ...
    memset(old_location, 0, URL_SIZE);
 ...
 redo:
    ...
    if (err < 0)
    {
            ...
            && (s->http_code == 416 || s->http_code == 406 || s->http_code == 403 || s->http_code == 400 || s->http_code == 0 || s->http_code == 500))
        {
            if (s->http_code == 500)
            {
                av_log(0, AV_LOG_ERROR, "http_connect return 500, s->off:%lld, filesize: %lld ", s->discard_off, s->filesize);
            }
            s->seek_flag = 0;
            url_close(hd);
            s->hd = NULL;
        ...
        {
            av_log(0, AV_LOG_WARNING, "retry http connect");
            s->willtry = 1;
            try_max_count = 5;
            goto retry;
        }
...
retry:
    if (s->willtry && willtry < try_max_count)
    {
        ...
        memcpy(s->location, old_location, URL_SIZE);
        s->buf_ptr = s->buffer;
        s->buf_end = s->buffer;
        ffurl_close(hd);
        s->hd = NULL;
    ...
    if (s->buf_ptr >= s->buf_end) {
        ...
        if (len < 0) {
           av_log(NULL, AV_LOG_ERROR, "[%s:%d] url_read='%d'\n", __FILE_NAME__, __LINE__, len);
            ...
            av_log(NULL, AV_LOG_ERROR, "tcp read error, len:%d\n", len);
...
*/
    ...
    for (;;) {
        ...
        if (err < 0) {
            av_log(h, AV_LOG_ERROR, "http process line failed, error:%d\n", err);
            return err;
        }
...
static int http_reconnect(URLContext *h,HTTPContext *old, int flag)
{
    ...
    if (!s) {
        url_errorcode_cb(h->interrupt_callback.opaque, 12,"http");
        old->errtag   = 1;
        return AVERROR(ENOMEM);
    }
    ...
    av_strlcpy(s->location, old->location, URL_SIZE);
