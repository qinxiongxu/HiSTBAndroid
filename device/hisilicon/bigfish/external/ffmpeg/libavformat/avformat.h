...
#define INITIAL_URL_SIZE        (1024)
#define HLS_DECRYPT_MAX_BUFFER_BLOCKS 150
#define HLS_DECRYPT_BLOCKSIZE 16
...
/**begin add by d00198887 for extra binary metadata,e.g. picture info of id3v2 2010-04-09**/
typedef struct ExtraMetadata {
    const char *tag;
    void *data;
    struct ExtraMetadata *next;
    void (*free)(void *obj);
} ExtraMetadata;
/**end add by d00198887 for extra binary metadata,e.g. picture info of id3v2 2010-04-09**/
...
#define AVFIND_INFO_PARSE_BYTES_MAX 1536*1024   ///< maximum bytes, avformat_find_stream_info will parse
#define AVFIND_INFO_PARSE_BYTES_MIN 2*1024   ///< minimum bytes, avformat_find_stream_info will parse
...
typedef struct AVProcinfo {
//trace seek
    uint32_t u32DoReadSeek;
    uint32_t u32ReadSeekDone;
    uint32_t u32DoSeekFrameBinary;
    uint32_t u32SeekFrameBinaryDone;
    uint32_t u32DoSeekFrameGeneric;
    uint32_t u32SeekFrameGenericDone;
//trace switch-pg
    uint32_t u32DoInitInput;
    uint32_t u32DoAvioOpenH;
    uint32_t u32AvioOpenHDone;
    uint32_t u32DoAvProbeInputBuffer;
    uint32_t u32AvProbeInputBufferDone;
    uint32_t u32FindEndPtsStart;
    uint32_t u32FindEndPtsDone;
    uint32_t u32EstimateDurationStart;
    uint32_t u32EstimateDurationDone;
    uint32_t u32FirstVidFrameRead;
    uint32_t u32FirstAudFrameRead;
} AVProcinfo;
...
//z00180556 add begin
    ...
    //int close_file;
    /*! Raw packet(m1v or m2v) fps */
    float MpegVideoFPS;
    /*! If the video support seek operation */
    int bSeekFlag ;
    /*! Save Mpeg video last packet time stamp */
    int64_t dwLastTimeStamp;
...
#define HLS_TAG_LEN (32)

typedef struct PresentationInfo_s {
    char   dmx_name[16];
    int     stream_index;//adaptationset index for dash, redition index  for hls?
    int     presentation_type;//type is AVMediaType
    int     presentation_index;//a stream for a different bandwidth, i.e. representation index for dash,
    int     bandwidth;//
    int     maxPlayoutRate;//for dash
}PresentationInfo_s;

typedef enum AVEvent {
    AVEVENT_UNKNOWN = 1000,
    AVEVENT_PRESENTATION_CHANGED,//hls/dash 's presentation(a different bps stream) changed, param type is PresentationInfo_s
    AVEVENT_WAITING_FOR_STREAMREADY,
    AVEVENT_STREAM_NOT_AVAIABLE,
}AVEvent;

/* add for m3u8 and m3u9 info get begin*/
...
{
    int     duration;
    int64_t start_time;               //current segment start time
    int64_t cur_segment_start_time;  // alone segment start time

    char    url[INITIAL_URL_SIZE];
    int     is_opened;
} m3u9_segment_t;
...
typedef struct m3u9_variant_s
{
    ...
    AVPacket        pkt;
    int             stream_offset;
    int             finished;                   //livestream will use
    int             need_reload;                //to update the list
    int             start_seq_no;               //Start of segment sequences
    int             n_segments;                 //total segments number
    m3u9_segment_t  **segments;      //every segments
    int             cur_seq_no;                 //current play segment number
    int64_t         *pts_offset;            //correct pts or dts timestamp
    int64_t         *seek_offset;           //correct pts or dts timestamp
    int64_t         *rem_last_time;         //correct pts or dts timestamp
    int             * flags ;
    int64_t         last_load_time;
    int             judge_open;             //Whether update timestamp
    int             target_duration;            //file total duration
    int             do_onlyone_start;           //livestream will use
    int             num_streams;                //Record the current file with stream's number
} m3u9_variant_t;

typedef struct SeamlessPlayContext
{
    int             n_variants;
    m3u9_variant_t  **variants;
    AVIOInterruptCB interrupt_callback;
    char            *user_agent;
    char            *referer;
    char            *cookies;
...
/* add for m3u8 and m3u9 info get end*/
 ...
 */
AVInputFormat *av_probe_input_format(AVProbeData *pd, int is_opened, AVIOInterruptCB *interrupt_callback);
 ...
 */
AVInputFormat *av_probe_input_format2(AVProbeData *pd, int is_opened, int *score_max, AVIOInterruptCB *interrupt_callback);
 ...
 */
AVInputFormat *av_probe_input_format3(AVProbeData *pd, int is_opened, int *score_ret, AVIOInterruptCB *interrupt_callback);
...
int av_probe_input_buffer(AVIOContext *pb, AVInputFormat **fmt,
                          ...
                          unsigned int offset, unsigned int max_probe_size, AVIOInterruptCB *interrupt_callback);
 ...
 * Get the current time in microseconds.
 */
unsigned int av_getsystemtime(void);
