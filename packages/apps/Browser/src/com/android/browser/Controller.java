...
public class Controller
        implements WebViewController, UiController, ActivityController , ControlPanelController{
    ...
    protected TabSwitcher mTabSwitcher;
    protected ControlPanelUi mControlPanelUi;
    protected ControlPanelEx mControlPanelEx;
    protected ZoomSetting mZoomSetting;
    private static final int PROGRESS_MAX = 100;

    // public message ids
    public final static int LOAD_URL = 1001;
    public final static int STOP_LOAD = 1002;
    public final static int FREE_TAB = 1003;
    public final static int FREE_TAB_TIMEOUT = 1000; // 1 second
    ...
    public void start(final Intent intent) {
        if (com.android.browser.Browser.CRASH_RECOVERY_ENABLED) {
            // mCrashRecoverHandler has any previously saved state.
            Log.d(LOGTAG, "start - do startRecovery -- " + intent);
            mCrashRecoveryHandler.startRecovery(intent);
        } else {
            Log.d(LOGTAG, "start - do start without recovery -- " + intent);
            doStart(null, intent);
        }
    }
    ...
    int getMaxTabs() {
        if (ActivityManager.isLowRamDeviceStatic()
            ...
            LogCat.d(true, LOGTAG, "low deviece max tab is 2!");
            return 2;
        }
            ...
            public void handleMessage(Message msg) {
                switch (msg.what) {
                    ...
                    case UPDATE_BOOKMARK_THUMBNAIL:
                        ...
                        break;
                    case FREE_TAB:
                        // TODO: Free the previous tab
                        LogCat.d(true, LOGTAG, "Trying to free the previous tab...");
                        mTabControl.freeTab(mTabControl.tabToBeFreed());
                        break;
                }
            }
        };
    ...
    public void onPageStarted(Tab tab, WebView view, Bitmap favicon) {
        ...
        if (mControlPanelUi != null) {
            mControlPanelUi.updateView(tab);
        }
        String url = tab.getUrl();

        if (false) {
            if (url.equals(mSettings.getNavigationPage())) {
                Log.d(LOGTAG, "controller - onPageStarted -- url==NavigationPage!! reset");
                browser_url_is_navigation = true;
                com.android.browser.Browser.resetCursorMode(true);
            } else {
                browser_url_is_navigation = false;
                com.android.browser.Browser.setCursorMode();
            }
        }

        if (mControlPanelUi != null && mControlPanelUi.isShowing())
            mControlPanelUi.dismiss();
        ...
        //TODO: Free the previous tab here should be safe!
        mHandler.sendMessageDelayed(mHandler.obtainMessage(FREE_TAB), FREE_TAB_TIMEOUT);
    }

    private boolean browser_url_is_navigation = false;
    public boolean isNavigationPage() {
        return browser_url_is_navigation;
    }
    ...
    public void onProgressChanged(Tab tab) {
        ...
        // Just for testing ANR / death exceptions
        if (false && newProgress > 50) {
        ...
        if (mControlPanelUi != null) {
            mControlPanelUi.updateStop(tab);
        }
    }
    ...
    public void onReceivedHttpAuthRequest(Tab tab, WebView view,
        ...
        // If we have a http proxy setting in environment, just apply it.
        ProxyParser pp = ProxyParser.getInstance();
        while (pp != null) {
            username = pp.getProxy_username();
            password = pp.getProxy_password();
        ...
        // No http proxy set in system, reuse the saved or give a prompt to user!
    ...
    public void showCustomView(Tab tab, View view, int requestedOrientation,
        ...
        LogCat.d(true, LOGTAG, "Contorller showCustomView begin...");
        if (tab.inForeground()) {
            if (mUi.isCustomViewShowing()) {
    ...
    public void hideCustomView() {
        LogCat.d(true, LOGTAG, "Contorller hideCustomView - begin...");
        String stack = Log.getStackTraceString(new Throwable());
        if (mUi.isCustomViewShowing()) {
            LogCat.d(true, LOGTAG, "Contorller hideCustomView - ui isCustomViewShowing...");
        ...
        // Force to reset main menu when html5 video is ending
        if (stack.contains("HTML5") || stack.contains("Controller.onBackKey")) {
            LogCat.d(true, LOGTAG, "Contorller hideCustomView - force to reset MAIN_MENU...");
            // Reset the old menu state.
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        if (mMenuState == EMPTY_MENU) {
            LogCat.d(true, LOGTAG, "Contorller onCreateOptionsMenu mMenuState is EMPTY_MENU...");
            return false;
        }
    ...
    public void onCreateContextMenu(ContextMenu menu, View v,
        ...
        switch (type) {
            ...
            case WebView.HitTestResult.IMAGE_TYPE:
                ...
                /*
                //because the launcher isn't protosomatic,so this item is invalid
    ...
    private Tab createNewTab(boolean incognito, boolean setActive,
            boolean useCurrent) {
        Log.d(LOGTAG, "Controller - createNewTab...");
        Tab tab = null;
        if (mTabControl.canCreateNewTab()) {
            Log.d(LOGTAG, "Controller::createNewTab - Create new Tab!");
            ...
            && ("512m".equals(ActivityManager.getMemorySizeStatic()) || "768m".equals(ActivityManager.getMemorySizeStatic()))) {
            Log.d(LOGTAG, "Controller::createNewTab - <low deviece> -- closeOtherTabs!!...");
            closeOtherTabs();
            Log.d(LOGTAG, "Controller::createNewTab - <low deviece> -- closeOtherTabs && createNew...!!");
            tab = mTabControl.createNewTab(incognito);
            addTab(tab);
            if (setActive) {
                setActiveTab(tab);
            ...
            if (true || useCurrent) {
                Log.d(LOGTAG, "Controller::createNewTab - Always using current tab when in max!");
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        ...
        if (false && !noModifiers && isMenuOrCtrlKey(keyCode)) {
            mMenuIsDown = true;
            return false;
        }
        ...
        if (KeyEvent.KEYCODE_MENU == keyCode) {
            Log.d(LOGTAG, "onKeyDown - MENU --> F1...");
        ...
        // Extra Key Event processing for better performances.
        if ( (mUi instanceof XLargeUi) ) {
            TabBar tb = ((XLargeUi)mUi).getTabBar();
            boolean isTabBarHasFocus = ((XLargeUi)mUi).getTabBar().hasFocus();
            if (KeyEvent.KEYCODE_DPAD_RIGHT == keyCode && isTabBarHasFocus) {
                if ( tb != null && tb.isNewTabImageFocused()) {
                    ((XLargeUi)mUi).setActionBarMenuFocus(true);
                    return true;
                }
            }
            if (KeyEvent.KEYCODE_DPAD_LEFT == keyCode) {
                if ( ((XLargeUi)mUi).getMenuViewFocus() ) {
                    if (tb != null) {
                        tb.setNewTabImageFocus(true);
                        return true;
                    }
                }
            }
            if (KeyEvent.KEYCODE_DPAD_UP == keyCode) {
                NavigationBarTablet nbt = ((XLargeUi)mUi).getNavigationBar();
                if (nbt != null && nbt.getNavigationBarFocusStatus()) {
                    ((XLargeUi)mUi).setActionBarMenuFocus(true);
                    return true;
                }
            }
            if (KeyEvent.KEYCODE_DPAD_DOWN == keyCode) {
                if ( ((XLargeUi)mUi).getMenuViewFocus() ) {
                    NavigationBarTablet nbt = ((XLargeUi)mUi).getNavigationBar();
                    if (nbt != null) {
                        nbt.setAllButtonFocus(true);
                        return true;
                    }
                }
            }

            // TODO: another key more convenient -2
            if (KeyEvent.KEYCODE_F1 == keyCode) {
                if (mControlPanelUi == null) {
                    mControlPanelUi = new ControlPanelUi(mActivity, this);
                } else {
                    Tab curr = getCurrentTab();
                    mControlPanelUi.updateView(curr);
                }
                if (!mControlPanelUi.isShowing()) {
                    mControlPanelUi.show();
                    WindowManager.LayoutParams params = mControlPanelUi.getWindow().getAttributes();
                    Display display = mActivity.getWindowManager().getDefaultDisplay();
                    params.width = display.getWidth();
                    mControlPanelUi.getWindow().setAttributes(params);
                } else {
                    mControlPanelUi.dismiss();
                }
                return true;
            ...
            // TODO: another key more convenient - 1
            if (KeyEvent.KEYCODE_F2 == keyCode) {
                if (mTabSwitcher == null) {
                    mTabSwitcher = new TabSwitcher(mActivity, this);
                } else {
                    mTabSwitcher.initImgs();
                }
                if (!mTabSwitcher.isShowing()) {
                    mTabSwitcher.show();
                    WindowManager.LayoutParams params = mTabSwitcher.getWindow()
                            .getAttributes();
                    Display display = mActivity.getWindowManager()
                            .getDefaultDisplay();
                    params.width = display.getWidth();
                    mTabSwitcher.getWindow().setAttributes(params);
                } else {
                    mTabSwitcher.dismiss();
                }
                return true;
            }
        }
...
//          case KeyEvent.KEYCODE_Z:    // unused
    ...
    public void stopOrRefresh() {
        ...
        // if progress is equal to PROGRESS_MAX, the page has been loaded.
    ...
    public void webViewZoom() {
        //mControlPanelEx.dismiss();
        mControlPanelUi.dismiss();
        if (mZoomSetting == null) {
            mZoomSetting = new ZoomSetting(mActivity, this);
        } else {
            mZoomSetting.initView(getCurrentTab().getZoomLevel());
        }
        mZoomSetting.show();
    }

    /**
     * web page jump to the home page
     *
     * @param null
     * @return null
     */
    ...
    public void openHomePage() {
        Log.d(LOGTAG, "controller - openHomePage...");
        mControlPanelUi.dismiss();
        openTabToHomePage();
    }

    /**
     * open more dialog, you can use zoom, add collection, open setting, open
     * history method in this dialog.
     *
     * @param null
     * @return null
     */
    ...
    public void openMoreDialog() {
        if (mControlPanelEx == null) {
            mControlPanelEx = new ControlPanelEx(mActivity, this);
        }
        mControlPanelEx.show();
        // set the diaplay height and width
        WindowManager.LayoutParams params = mControlPanelEx.getWindow()
                .getAttributes();
        Display display = mActivity.getWindowManager().getDefaultDisplay();
        params.width = 350;
        mControlPanelEx.getWindow().setAttributes(params);
    }
    ...
    /**
     * open tab thumbnail switcher page
     *
     * @param null
     * @return null
     */
