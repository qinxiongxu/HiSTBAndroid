...
class TabControl {
    ...
    /**
     * Close some tabs to save zoom for new window or tab.
     */
    void closeSomeTabs() {
        // null check
        if (getTabCount() == 0) return;
        ...
        Vector<Tab> tabs = getHalfLeastUsedTabs(getCurrentTab());
        if (tabs.size() > 0) {
            Log.w(LOGTAG, "Close " + tabs.size() + " tabs in the browser");
            for (Tab t : tabs) {
                Log.w(LOGTAG, "Closing tab(title:" + t.getTitle() + "; url:"+ t.getUrl() + ")...");
                removeTab(t);
            }
            return;
        }
    ...
    void recreateWebView(Tab t) {
        ...
        if (w != null) {
            LogCat.d(true, "browser", "recreateWebView -- t.destroy -- " + t);
            t.destroy();
        }
        // Create a new WebView. If this tab is the current tab, we need to put
        // back all the clients so force it to be the current tab.
        LogCat.d(true, "browser", "recreateWebView -- t.setWebView(createNewWebView(), false)...");
    ...
    private Tab tab_to_be_free = null;
    public Tab tabToBeFreed() {
        return tab_to_be_free;
    }
    ...
    public boolean freeTab(Tab t) {
        LogCat.d(true, LOGTAG, "preparing freeTab(" + t + ")...");
        ...
        if (view != null) {
            ...
            boolean is_html5_mode = (ua != null && ua.contains("Mac OS"));
            // If not in html5 mode, we will free this tab
            if ((ActivityManager.isLowRamDeviceStatic() && ("512m".equals(ActivityManager.getMemorySizeStatic()) || "768m".equals(ActivityManager.getMemorySizeStatic())))
                || !is_html5_mode) {
                LogCat.d(true, LOGTAG, "FlashPlayer mode!! || low deviece --> free tab(" + t.getTitle() + ")...");
                // store the WebView's state.
                t.saveState();
                // destroy the tab
                t.destroy();
